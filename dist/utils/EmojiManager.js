class EmojiManager {
     ...
     */
    resolveEmoji(config, animatedOnly, staticOnly) {
        if (!this.client)
    ...
    listRequiredEmojis() {
        ...
        for (const [category, emojis] of Object.entries(this.registry)) {
            for (const [, config] of Object.entries(emojis)) {
    ...
    registerCustomEmoji(category, key, config) {
