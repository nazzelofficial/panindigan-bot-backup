...
export function manilaTimestampFull() {
    return new Date().toLocaleString('en-PH', {
        timeZone: 'Asia/Manila',
        weekday: 'short',
        year: 'numeric',
...
function styleLevel(level) {
    ...
    if (!cfg)
        return chalk.gray(` ${raw.toUpperCase().padEnd(7)} `);
    return cfg.badge(raw);
}
function styleModule(mod) {
    if (!mod)
        return chalk.gray('  ' + '─'.padEnd(20));
...
// ─── Formats ──────────────────────────────────────────────────────────────────
const jsonFormat = winston.format.combine(winston.format.timestamp({ format: () => manilaTimestamp() }), winston.format.errors({ stack: true }), winston.format((info) => {
...
const consoleFormat = winston.format.combine(winston.format.timestamp({ format: () => manilaTimestampShort() }), winston.format.printf(({ timestamp, level, message, module: mod, shardId, durationMs, correlationId, requestId, guild, user, channel, command, ...meta }) => {
    ...
    const ctx = [];
    if (guild)
        ctx.push(chalk.cyan(`guild:${guild}`));
    if (user)
        ctx.push(chalk.green(`user:${user}`));
    if (channel)
        ctx.push(chalk.yellow(`ch:${channel}`));
    if (command)
        ctx.push(chalk.magenta(`cmd:${command}`));
...
export function logInteraction(type, opts) {
...
export function logVoiceEvent(event, opts) {
...
export function logMusicEvent(event, opts) {
...
export function logAIRequest(provider, opts) {
    if (opts.success) {
...
export function logCacheEvent(type, opts) {
...
export function logAPIRequest(method, url, opts) {
...
export function logSecurityEvent(event, opts) {
...
export function logPerformance(operation, durationMs, opts) {
