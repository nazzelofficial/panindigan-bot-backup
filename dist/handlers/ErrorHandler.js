...
export class ErrorHandler {
    ...
    static async send(source, options) {
        const { title = 'Error Occurred', description, whatHappened, why, howToFix, suggestedActions, showTimestamp = true, ephemeral = true, } = options;
     ...
     */
    static async permission(source, missingPermissions) {
     ...
     */
    static async botPermission(source, missingPermissions) {
        await this.send(source, {
     ...
     * Send cooldown error
     */
    static async cooldown(source, remainingTime) {
        await this.send(source, {
     ...
     */
    static async rateLimit(source, resetTime) {
        await this.send(source, {
     ...
     */
    static async invalidArgument(source, argumentName, expected) {
        await this.send(source, {
     ...
     */
    static async notFound(source, resourceType, identifier) {
        await this.send(source, {
     ...
     * Send generic error
     */
    static async generic(source, error) {
        await this.send(source, {
     ...
     * Send music error
     */
    static async music(source, errorType) {
        const errorMessages = {
            not_in_voice: {
     ...
     * Send economy error
     */
    static async economy(source, errorType) {
        const errorMessages = {
            insufficient_funds: {
     ...
     */
    static async moderation(source, errorType) {
        const errorMessages = {
            hierarchy: {
