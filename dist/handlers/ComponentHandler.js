...
async function handleMarriageButton(interaction) {
    ...
    if (action === 'marry_accept') {
        ...
        const acceptEmbed = EmbedManager.success('💒 Sila na!', `🎊 <@${proposerId}> at <@${targetId}> ay opisyal na isang couple!\n\n*Mahabang pagmamahal sa inyong dalawa!* 💕`, {
            fields: [
        ...
        const declineEmbed = EmbedManager.error('Proposal Declined', `💔 <@${targetId}> ay tumanggi sa proposal ni <@${proposerId}>.\n\n*It's okay — there are plenty of fish in the sea!*`);
...
async function handleGiveawayEntry(interaction) {
    ...
    if (giveaway.requiredLevel) {
        ...
        if (!leveling || leveling.level < giveaway.requiredLevel) {
            await interaction.reply({ content: `❌ You need to be level ${giveaway.requiredLevel}+ to enter this giveaway.`, ephemeral: true });
            return;
        }
    }
...
async function handleTicketButton(interaction, client) {
    ...
    if (action === 'ticket_create') {
        ...
        const existingChannel = interaction.guild?.channels.cache.find(ch => ch.name === `ticket-${interaction.user.username.toLowerCase()}`);
        if (existingChannel) {
        ...
        if (channel) {
            ...
            const closeRow = new (await import('discord.js')).ActionRowBuilder()
                .addComponents(new (await import('discord.js')).ButtonBuilder()
...
async function handleTagCreateModal(interaction) {
    ...
    await collection.updateOne({ guildId, name }, { $set: { guildId, name, content, createdBy: interaction.user.id, updatedAt: new Date() }, $setOnInsert: { createdAt: new Date() } }, { upsert: true });
    ...
    if (guild?.reportChannelId) {
        ...
        if (channel?.isTextBased()) {
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: '🎯 Reported User', value: `<@${targetId}> (${targetId})`, inline: true }, { name: '👤 Reporter', value: `<@${interaction.user.id}>`, inline: true }, { name: '📝 Reason', value: reason, inline: false })
                .setTimestamp();
