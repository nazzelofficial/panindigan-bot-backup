...
import { detectHosting } from './HostingDetector.js';
...
function checkAuth(req, authToken, allowedIps) {
    if (!authToken && (!allowedIps || allowedIps.length === 0))
        return true;
    if (authToken) {
...
function statusToHttpCode(status, isReady) {
    if (status === 'healthy' && isReady)
        return 200;
    if (status === 'degraded')
        return 200; // degraded = running but impaired
    return 503;
}
// ─── Health Server ────────────────────────────────────────────────────────────
...
<tr><td><a href="/info">/info</a></td><td>Runtime information</td><td>${this.authToken ? 'Yes' : 'No'}</td></tr>
    ...
    cachedReport = null;
    lastReportMs = 0;
    REPORT_CACHE_MS = parseInt(process.env.HEALTH_CACHE_MS ?? '10000', 10);
    getCachedReport() {
        const now = Date.now();
