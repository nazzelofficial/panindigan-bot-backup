...
export class StockService {
    ...
    async getStockFallback(symbol) {
        ...
        if (!known)
            throw new Error(`Stock symbol "${symbol}" not found. Configure ALPHAVANTAGE_API_KEY for live data.`);
    ...
    async getCryptoPrice(coinId) {
        ...
        if (!res.ok) {
            if (res.status === 404)
                throw new Error(`Crypto "${coinId}" not found. Try the CoinGecko ID (e.g., "bitcoin", "ethereum").`);
