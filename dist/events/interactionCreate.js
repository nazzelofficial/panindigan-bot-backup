    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            const premiumTier = interaction.guild
                ...
                : 'free';
            const hasAccess = command.premiumTier === 'free' || await hasPremiumAccess(interaction.user.id, interaction.guild?.id ?? 'global', command.premiumTier);
            if (!hasAccess) {
            ...
            // ── Cooldown ──────────────────────────────────────────────────────────
            const cooldownCheck = await checkCooldown(client, interaction.user.id, interaction.guild?.id ?? 'dm', command.name, premiumTier);
            ...
            logCommandExecution(client.shardId, interaction.guild?.id ?? 'dm', interaction.user.id, command.name, [], executionTime, true);
        }
        catch (error) {
            const executionTime = Date.now() - startTime;
            ...
            logCommandExecution(client.shardId, interaction.guild?.id ?? 'dm', interaction.user.id, command.name, [], executionTime, false);
            const errorEmbed = EmbedManager.error('Command Error', 'An unexpected error occurred while running this command.');
            errorEmbed.addFields({ name: '📋 Command', value: `\`/${command.name}\``, inline: true }, { name: '🔧 Details', value: error instanceof Error ? `\`${error.message.slice(0, 200)}\`` : 'Unknown error', inline: false }, { name: '💡 What to do', value: 'Try the command again. If the problem persists, join our [support server](https://discord.gg/panindigan).', inline: false });
            try {
                if (interaction.replied || interaction.deferred) {
