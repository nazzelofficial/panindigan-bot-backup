...
export class CoupleConsentService {
    ...
    requestKey(toUserId, guildId) {
        return `${config.databases.redis.keyPrefix}couple_req:${guildId}:${toUserId}`;
    }
    async sendRequest(fromUserId, toUserId, guildId) {
        // Check if they're already coupled
        const existing = await this.getCouple(fromUserId, guildId);
        if (existing)
            return { success: false, error: 'You are already in a couple. Use `p!divorce` first.' };
        const targetExisting = await this.getCouple(toUserId, guildId);
        if (targetExisting)
            return { success: false, error: 'That user is already in a couple.' };
        if (fromUserId === toUserId)
            return { success: false, error: 'You cannot send a couple request to yourself.' };
        const reqData = {
            fromUserId,
            toUserId,
            guildId,
    ...
    async getPendingRequest(toUserId, guildId) {
        try {
    ...
    async acceptRequest(toUserId, guildId) {
        const request = await this.getPendingRequest(toUserId, guildId);
        if (!request)
            return { success: false, error: 'No pending couple request found.' };
    ...
    async declineRequest(toUserId, guildId) {
        try {
            await this.redis.del(this.requestKey(toUserId, guildId));
            return true;
        }
    ...
    async cancelRequest(fromUserId, guildId) {
    ...
    async divorce(userId, guildId) {
        const user = await this.prisma.user.findUnique({
            where: { userId_guildId: { userId, guildId } },
            select: { spouseId: true },
        });
        if (!user?.spouseId)
            return { success: false, error: 'You are not in a couple.' };
    ...
    async getCouple(userId, guildId) {
        try {
    ...
    async getCoupleStatus(userId, guildId) {
...
//# sourceMappingURL=CoupleConsentService.js.map