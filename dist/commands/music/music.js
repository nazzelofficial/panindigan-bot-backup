...
import { SlashCommandBuilder, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ComponentType, ModalBuilder, TextInputBuilder, TextInputStyle, StringSelectMenuBuilder, } from 'discord.js';
...
import { MusicUI } from '../../structures/MusicUI.js';
...
export class MusicCommand extends BaseCommand {
    ...
    async executeSlash(i) {
        ...
        if (subcommandGroup === 'player') {
            switch (subcommand) {
                case 'play':
                    await this.handlePlay(i);
                    break;
                case 'pause':
                    await this.handlePause(i);
                    break;
                case 'resume':
                    await this.handleResume(i);
                    break;
                case 'stop':
                    await this.handleStop(i);
                    break;
                case 'skip':
                    await this.handleSkip(i);
                    break;
                case 'previous':
                    await this.handlePrevious(i);
                    break;
                case 'seek':
                    await this.handleSeek(i);
                    break;
                case 'replay':
                    await this.handleReplay(i);
                    break;
                case 'shuffle':
                    await this.handleShuffle(i);
                    break;
                case 'loop':
                    await this.handleLoop(i);
                    break;
                case 'volume':
                    await this.handleVolume(i);
                    break;
                case 'nowplaying':
                    await this.handleNowPlaying(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'show':
                    await this.handleQueueShow(i);
                    break;
                case 'clear':
                    await this.handleQueueClear(i);
                    break;
                case 'remove':
                    await this.handleQueueRemove(i);
                    break;
                case 'move':
                    await this.handleQueueMove(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'create':
                    await this.handlePlaylistCreate(i);
                    break;
                case 'list':
                    await this.handlePlaylistList(i);
                    break;
                case 'show':
                    await this.handlePlaylistShow(i);
                    break;
                case 'add':
                    await this.handlePlaylistAdd(i);
                    break;
                case 'remove':
                    await this.handlePlaylistRemove(i);
                    break;
                case 'delete':
                    await this.handlePlaylistDelete(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'bassboost':
                    await this.handleFilterBassboost(i);
                    break;
                case 'nightcore':
                    await this.handleFilterNightcore(i);
                    break;
                case 'vaporwave':
                    await this.handleFilterVaporwave(i);
                    break;
                case '8d':
                    await this.handleFilter8D(i);
                    break;
                case 'reset':
                    await this.handleFilterReset(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'join':
                    await this.handleVoiceJoin(i);
                    break;
                case 'leave':
                    await this.handleVoiceLeave(i);
                    break;
                case 'disconnect':
                    await this.handleVoiceDisconnect(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'search':
                    await this.handleSearch(i);
                    break;
                case 'lyrics':
                    await this.handleLyrics(i);
                    break;
                case 'stats':
    ...
    async handlePlay(i) {
        ...
        try {
            ...
            await ErrorHandler.generic(i, error);
        }
    }
    async handlePause(i) {
        if (!i.guild || !i.member)
        ...
        try {
            ...
            const embed = EmbedManager.music('Paused', `⏸️ **${currentTrack.title}** has been paused.`, {
                fields: [
    ...
    async handleStop(i) {
        ...
        try {
            ...
            player.destroy();
            const embed = EmbedManager.music('Playback Stopped', `⏹️ Stopped **${trackTitle}** and cleared the queue.`, {
                fields: [
    ...
    async handleSkip(i) {
        ...
        try {
            ...
            const embed = EmbedManager.music('Track Skipped', `⏭️ Skipped **${skippedTrack.title}**`, {
                fields: [
    ...
    async handlePrevious(i) {
        ...
        try {
            ...
            await player.seek(0);
            const embed = EmbedManager.music('Replaying from Start', `⏮️ Replaying **${currentTrack.title}** from the beginning`, {
                fields: [
    ...
    async handleSeek(i) {
        ...
        try {
            ...
            const embed = EmbedManager.music('Seeked', `⏩ Jumped to **${this.formatDuration(seconds)}** in the track`, {
                fields: [
                    { name: '🎵 Track', value: currentTrack.title, inline: true },
    ...
    async handleReplay(i) {
        ...
        try {
            ...
            await player.seek(0);
            const embed = EmbedManager.music('Replaying Track', `🔄 Replaying **${currentTrack.title}** from the beginning`, {
                fields: [
                    { name: '🎵 Track', value: currentTrack.title, inline: true },
                    { name: '🎤 Artist', value: currentTrack.author || 'Unknown', inline: true },
                    { name: '👤 Requested by', value: `<@${i.user.id}>`, inline: true },
    ...
    async handleShuffle(i) {
        ...
        try {
            ...
            const embed = EmbedManager.music('Queue Shuffled', `🔀 Shuffled **${player.queue.size}** tracks in the queue`, {
                fields: [
    ...
    async handleVolume(i) {
        ...
        try {
            ...
            const embed = EmbedManager.music('Volume Changed', `${volEmoji} Volume set to **${level}%**`, {
                fields: [
    ...
    async handleNowPlaying(i) {
        ...
        try {
            ...
            if (!player || !player.queue.current) {
                const embed = EmbedManager.music('Nothing Playing', '🎵 No track is currently playing.\n\n> Use `/music player play` to start listening!', {
                    fields: [
            ...
            embed.addFields({ name: '📋 Queue', value: `${player.queue.size} track${player.queue.size !== 1 ? 's' : ''} remaining`, inline: true }, { name: `${volEmoji} Volume`, value: `${player.volume}%`, inline: true }, { name: '🔁 Loop', value: loopLabel, inline: true });
            const controls = ComponentBuilder.musicControlRow(i.guild.id, player.paused, player.loop);
            const volRow = ComponentBuilder.musicVolumeRow(i.guild.id);
            await i.editReply({ embeds: [embed], components: [controls, volRow] });
        }
        catch (error) {
    ...
    async handleQueueShow(i) {
        ...
        try {
            ...
            const player = client.kazagumo.players.get(i.guild.id);
            if (!player) {
                const embed = EmbedManager.music('Queue Empty', '📋 No active music session found.\n\n> Use `/music player play` to start listening!', { fields: [{ name: '💡 Tip', value: 'Try `/music search` to browse songs.', inline: false }] });
                await i.editReply({ embeds: [embed] });
                return;
            }
            const currentTrack = player.queue.current;
            ...
            if (queueSize === 0 && !currentTrack) {
                const embed = EmbedManager.music('Queue is Empty', '📋 The music queue is currently empty.\n\n> Add songs with `/music player play`!', {
                    fields: [
            ...
            const createEmbed = (page) => {
                const loopLabel = player.loop === 'none' ? '➡️ Off' : player.loop === 'track' ? '🔂 Track' : '🔁 Queue';
                return EmbedManager.music('📜 Music Queue', getQueuePage(page) || '*No tracks in queue*', {
                    fields: [
            ...
            collector.on('collect', async (interaction) => {
                try {
                    ...
                    await ErrorHandler.generic(interaction, error);
                }
            });
    ...
    async handleQueueClear(i) {
        ...
        try {
            ...
            const row = new ActionRowBuilder()
                ...
                .setLabel('Cancel')
                .setStyle(ButtonStyle.Secondary));
            const warnEmbed = EmbedManager.error('Confirm Queue Clear', `⚠️ This will remove **${player.queue.size} tracks** from the queue. This action cannot be undone.`);
                ...
                if (interaction.customId === 'confirm_clear') {
                    try {
                        ...
                        const embed = new EmbedBuilder()
                            ...
                            .addFields({ name: '👤 Cleared by', value: i.user.tag, inline: true }, { name: '🖥️ Server', value: i.guild.name, inline: true })
                            .setTimestamp();
    ...
    async handleQueueRemove(i) {
        ...
        try {
            ...
            const embed = EmbedManager.music('Track Removed', `🗑️ Removed **${removedTrack.title}** from position **#${position}**`, {
                fields: [
    ...
    async handleQueueMove(i) {
        ...
        try {
            ...
            const embed = EmbedManager.music('Track Moved', `🔄 Moved **${trackToMove.title}** from position **#${from}** → **#${to}**`, {
                fields: [
    ...
    async handlePlaylistCreate(i) {
        ...
        try {
            ...
            const embed = EmbedManager.music('Playlist Created', `📁 Created playlist **${name}**!`, {
                fields: [
    ...
    async handlePlaylistList(i) {
        ...
        try {
            ...
            if (playlists.length === 0) {
                const embed = EmbedManager.music('Your Playlists', '📁 You have no playlists yet.', {
                    fields: [
    ...
    async handlePlaylistShow(i) {
        ...
        try {
            ...
            const trackList = tracks.map((track, index) => `${index + 1}. **${track.title}** - ${track.artist || 'Unknown'} (${this.formatDuration(track.duration || 0)})`).join('\n');
            ...
            const embed = EmbedManager.music(`📁 ${playlist.name}`, playlist.description || '*No description*', { fields: playlistFields });
            await i.editReply({ embeds: [embed] });
        }
        catch (error) {
            await ErrorHandler.generic(i, error);
    ...
    async handlePlaylistAdd(i) {
        ...
        try {
            ...
            const exists = tracks.some((t) => t.url === currentTrack.uri);
            if (exists) {
            ...
            const embed = EmbedManager.music('Track Added to Playlist', `➕ Added **${currentTrack.title}** to **${name}**`, {
                fields: [
                    { name: '🎵 Track', value: currentTrack.title, inline: true },
    ...
    async handlePlaylistRemove(i) {
        ...
        try {
            ...
            const embed = EmbedManager.music('Track Removed from Playlist', `🗑️ Removed **${removedTrack.title}** from **${name}**`, {
                fields: [
                    { name: '🎵 Track', value: removedTrack.title, inline: true },
                    { name: '📍 Position', value: `#${position}`, inline: true },
                    { name: '📁 Playlist', value: name, inline: true },
    ...
    async handlePlaylistDelete(i) {
        ...
        const row = new ActionRowBuilder()
            ...
            .setLabel('Cancel')
            .setStyle(ButtonStyle.Secondary));
        const warnEmbed = EmbedManager.error('Confirm Playlist Delete', `⚠️ This will permanently delete **${name}** and its **${trackCount} tracks**. This cannot be undone.`);
        await i.editReply({ embeds: [warnEmbed], components: [row] });
        const collector = i.channel.createMessageComponentCollector({
            componentType: ComponentType.Button,
            time: 30000,
        ...
        collector.on('collect', async (interaction) => {
            if (interaction.customId === 'confirm_delete') {
                try {
                    ...
                    const embed = EmbedManager.music('Playlist Deleted', `🗑️ **${name}** has been permanently deleted.`, {
                        fields: [
                            { name: '📁 Playlist', value: name, inline: true },
    ...
    async handleFilterBassboost(i) {
        ...
        try {
            ...
            const hasBassBoost = currentFilters.equalizer?.some((eq) => eq.band === 0 && eq.gain > 0);
            if (hasBassBoost) {
                // Remove bass boost
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: '🎵 Filter', value: 'Bass Boost', inline: true }, { name: '🔊 Status', value: 'Disabled', inline: true }, { name: '👤 Toggled by', value: i.user.tag, inline: true })
                    .setTimestamp();
                await i.editReply({ embeds: [embed] });
            }
            else {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: '🎵 Filter', value: 'Bass Boost', inline: true }, { name: '🔊 Status', value: 'Enabled', inline: true }, { name: '👤 Toggled by', value: i.user.tag, inline: true })
                    .setTimestamp();
                await i.editReply({ embeds: [embed] });
            }
        }
    ...
    async handleFilterNightcore(i) {
        ...
        try {
            ...
            if (hasNightcore) {
                // Remove nightcore
                await player.setFilters({});
                const embed = EmbedManager.music('Nightcore Disabled', `🎚️ **Nightcore** filter has been **turned off**.`, {
                    fields: [
                ...
                const embed = EmbedManager.music('Nightcore Enabled', `🎚️ **Nightcore** filter is now **active**! Speeds up the track and raises the pitch — anime energy!`, {
                    fields: [
                        { name: '🎵 Filter', value: 'Nightcore', inline: true },
    ...
    async handleFilterVaporwave(i) {
        ...
        try {
            ...
            if (hasVaporwave) {
                // Remove vaporwave
                await player.setFilters({});
                const embed = EmbedManager.music('Vaporwave Disabled', `🎚️ **Vaporwave** filter has been **turned off**.`, {
                    fields: [
                ...
                const embed = EmbedManager.music('Vaporwave Enabled', `🎚️ **Vaporwave** filter is now **active**! Slows the track down and lowers the pitch — retro vibes.`, {
                    fields: [
                        { name: '🎵 Filter', value: 'Vaporwave', inline: true },
                        { name: '🔊 Status', value: '✅ On', inline: true },
                        { name: '👤 Toggled by', value: `<@${i.user.id}>`, inline: true },
        ...
        try {
            ...
            if (has8D) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: '🎵 Filter', value: '8D Audio', inline: true }, { name: '🔊 Status', value: 'Disabled', inline: true }, { name: '👤 Toggled by', value: i.user.tag, inline: true })
                    .setTimestamp();
                await i.editReply({ embeds: [embed] });
            }
            else {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: '🎵 Filter', value: '8D Audio', inline: true }, { name: '🔊 Status', value: 'Enabled', inline: true }, { name: '👤 Toggled by', value: i.user.tag, inline: true })
                    .setTimestamp();
                await i.editReply({ embeds: [embed] });
            }
        }
    ...
    async handleFilterReset(i) {
        ...
        try {
            ...
            // Reset all filters
            await player.setFilters({});
            const embed = EmbedManager.music('All Filters Reset', '🎚️ All audio filters have been removed. The track is back to its original sound.', {
                fields: [
    ...
    async handleVoiceJoin(i) {
        ...
        try {
            ...
            const embed = EmbedManager.music('Joined Voice Channel', `🔊 Joined **${voiceChannel.name}**! Use \`/music player play\` to start listening.`, {
                fields: [
    ...
    async handleVoiceLeave(i) {
        ...
        try {
            ...
            player.destroy();
            const embed = EmbedManager.music('Left Voice Channel', `👋 Left **${channelName}**. Use \`/music voice join\` to bring me back!`, {
                fields: [
    ...
    async handleVoiceDisconnect(i) {
        ...
        try {
            ...
            player.destroy();
            const embed = EmbedManager.music('Disconnected', `🔌 Disconnected from **${channelName}** and cleared the queue.`, {
                fields: [
                    { name: '🎤 Channel', value: channelName, inline: true },
    ...
    async handleSearch(i) {
        ...
        try {
            ...
            if (isPlaylist) {
                ...
                const embed = EmbedManager.music('Playlist Queued', `📋 Added **${result.tracks.length} tracks** to the queue!`, {
                    fields: [
            ...
            const trackList = tracks.map((track, index) => `${index + 1}. **${track.title}** - ${track.author || 'Unknown'} (${this.formatDuration(track.length || 0)})`).join('\n');
            const embed = EmbedManager.music('🔍 Search Results', `Results for **${query}**\n\n${trackList}`, {
                fields: [
            ...
            const selectMenu = new StringSelectMenuBuilder()
                ...
                .setMaxValues(1)
                .setOptions(tracks.slice(0, 25).map((track, index) => ({
    ...
    async handleLyrics(i) {
        ...
        try {
            ...
            const row = new ActionRowBuilder()
                ...
                .setLabel('▶️')
                .setStyle(ButtonStyle.Secondary)
                .setDisabled(currentPage === pages.length - 1), new ButtonBuilder()
    ...
    async handleStats(i) {
        ...
        try {
            ...
            const embed = EmbedManager.music('Music Statistics', `📊 Server music stats for **${i.guild.name}**`, {
                fields: [
