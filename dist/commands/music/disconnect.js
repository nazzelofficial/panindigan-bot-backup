...
export class DisconnectCommand extends BaseCommand {
    constructor() {
        const options = {
            ...
            prefixCommand: true,
            aliases: ['dc', 'leavevc'],
            examples: ['/disconnect', 'p!disconnect'],
        };
        super(options);
    }
    async executeSlash(interaction) {
        ...
        if (!voiceChannel) {
            await interaction.reply({ content: '❌ You need to be in a voice channel to disconnect the bot.', ephemeral: true });
            return;
        }
        try {
            ...
            await interaction.reply({ content: '❌ Failed to disconnect.', ephemeral: true });
        }
    }
    async executePrefix(message) {
        ...
        if (!voiceChannel) {
            await message.reply('❌ You need to be in a voice channel to disconnect the bot.');
            return;
        }
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(COLORS.error)
                .addFields([
                { name: 'Disconnected by', value: message.author.tag, inline: true },
            ])
                .setTimestamp();
            await message.reply({ embeds: [embed] });
        }
        catch (error) {
            await message.reply('❌ Failed to disconnect.');
        }
    }
}
