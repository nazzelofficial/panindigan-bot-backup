...
export class MassRoleCommand extends BaseCommand {
    ...
    async executeSlash(interaction) {
        ...
        if (!interaction.guild)
            return;
        const membersToRole = interaction.guild.members.cache.filter(m => !m.user.bot && !m.roles.cache.has(role.id) && (!excludeRole || !m.roles.cache.has(excludeRole.id)));
    ...
    async executePrefix(message, _args) {
        ...
        if (!message.guild)
            return;
        const membersToRole = message.guild.members.cache.filter(m => !m.user.bot && !m.roles.cache.has(role.id) && (!excludeRole || !m.roles.cache.has(excludeRole.id)));
        if (membersToRole.size === 0) {
