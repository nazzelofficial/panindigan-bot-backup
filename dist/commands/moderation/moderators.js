...
export class ModeratorsCommand extends BaseCommand {
    ...
    async showModerators(interaction) {
        if (!interaction.guild)
            return;
        const moderators = interaction.guild.members.cache.filter(member => member.permissions.has(PermissionFlagsBits.ModerateMembers) && !member.user.bot);
        const admins = moderators.filter(member => member.permissions.has(PermissionFlagsBits.Administrator));
        const mods = moderators.filter(member => !member.permissions.has(PermissionFlagsBits.Administrator));
