...
export class NewsCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addChoices({ name: 'General', value: 'general' }, { name: 'Technology', value: 'technology' }, { name: 'Business', value: 'business' }, { name: 'Science', value: 'science' }, { name: 'Sports', value: 'sports' }, { name: 'Entertainment', value: 'entertainment' }, { name: 'Health', value: 'health' }));
    }
    async executeSlash(interaction) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(articles.map((a, i) => `**${i + 1}. [${a.title}](${a.url})**\n${a.description?.slice(0, 100) || ''}...\n📰 ${a.source} • ${newsService.formatTimeAgo(a.publishedAt)}`).join('\n\n').slice(0, 4000))
    ...
    async executePrefix(message, _args) {
        ...
        try {
            ...
            if (!articles.length) {
                await thinking.edit(`${EMOJIS.error} No news articles found.`);
                return;
            }
            const embed = new EmbedBuilder()
