...
export class CryptoCommand extends BaseCommand {
    ...
    async fetchCrypto(coin) {
        ...
        if (res.status === 404)
            throw new Error(`Coin "${coin}" not found. Try the full name (e.g., "bitcoin").`);
        if (!res.ok)
    ...
    async executeSlash(interaction) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: '💵 Price (USD)', value: priceUSD ? `$${priceUSD.toLocaleString()}` : 'N/A', inline: true }, { name: '📊 24h Change', value: this.formatChange(market.price_change_percentage_24h), inline: true }, { name: '📅 7d Change', value: this.formatChange(market.price_change_percentage_7d), inline: true }, { name: '🏦 Market Cap', value: market.market_cap?.usd ? `$${market.market_cap.usd.toLocaleString()}` : 'N/A', inline: true }, { name: '🔝 24h High', value: market.high_24h?.usd ? `$${market.high_24h.usd.toLocaleString()}` : 'N/A', inline: true }, { name: '🔻 24h Low', value: market.low_24h?.usd ? `$${market.low_24h.usd.toLocaleString()}` : 'N/A', inline: true }, { name: '📦 Volume (24h)', value: market.total_volume?.usd ? `$${market.total_volume.usd.toLocaleString()}` : 'N/A', inline: true }, { name: '🏅 Market Rank', value: data.market_cap_rank ? `#${data.market_cap_rank}` : 'N/A', inline: true })
    ...
    async executePrefix(message, _args) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(market.price_change_percentage_24h >= 0 ? COLORS.success : COLORS.error)
                .setThumbnail(data.image?.small)
                .addFields({ name: '💵 Price (USD)', value: priceUSD ? `$${priceUSD.toLocaleString()}` : 'N/A', inline: true }, { name: '📊 24h Change', value: this.formatChange(market.price_change_percentage_24h), inline: true }, { name: '🏦 Market Cap', value: market.market_cap?.usd ? `$${market.market_cap.usd.toLocaleString()}` : 'N/A', inline: true })
