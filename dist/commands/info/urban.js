...
export class UrbanCommand extends BaseCommand {
    ...
    async lookup(term) {
        ...
        if (!res.ok)
            throw new Error(`Urban Dictionary API error: ${res.status}`);
    ...
    async executeSlash(interaction) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(0x1d3461)
                .setURL(result.link)
                .addFields({ name: '📝 Definition', value: result.definition.slice(0, 1024), inline: false }, { name: '💬 Example', value: result.example.slice(0, 512) || 'No example.', inline: false }, { name: '👤 Author', value: result.author, inline: true }, { name: '👍', value: result.thumbsUp.toLocaleString(), inline: true }, { name: '👎', value: result.thumbsDown.toLocaleString(), inline: true })
    ...
    async executePrefix(message, _args) {
        ...
        if (!term)
            return void message.reply(`${EMOJIS.error} Please provide a term to look up.`);
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(0x1d3461)
                .setURL(result.link)
                .addFields({ name: '📝 Definition', value: result.definition.slice(0, 1024), inline: false }, { name: '💬 Example', value: result.example.slice(0, 512) || 'No example.', inline: false })
