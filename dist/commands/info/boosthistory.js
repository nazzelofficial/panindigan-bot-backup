...
export class BoostHistoryCommand extends BaseCommand {
    ...
    async executeSlash(interaction) {
        ...
        const boosters = guild.members.cache.filter(m => m.premiumSince !== null).sort((a, b) => (a.premiumSince?.getTime() || 0) - (b.premiumSince?.getTime() || 0));
        const embed = new EmbedBuilder()
            .setTitle(`🚀 Server Boosts — ${guild.name}`)
            .setColor(0xff73fa)
            .addFields({ name: '📊 Boost Stats', value: [
    ...
    async executePrefix(message, _args) {
        ...
        const embed = new EmbedBuilder()
            .setTitle(`🚀 Server Boosts — ${guild.name}`)
            .setColor(0xff73fa)
            .addFields({ name: '📊 Stats', value: `Tier **${guild.premiumTier}** • **${guild.premiumSubscriptionCount || 0}** boosts • **${boosters.size}** boosters`, inline: false })
            .setTimestamp();
