...
export class KeygenCommand extends BaseCommand {
    constructor() {
        super({ name: 'keygen', description: 'Generate premium activation keys (Owner only)', category: 'owner', premiumTier: 'free', cooldown: 0, guildOnly: false, ownerOnly: true, slashCommand: true, prefixCommand: true, aliases: ['generatekey', 'genkey'], examples: ['p!keygen gold 30', 'p!keygen diamond 365 5'] });
    }
    buildSlashCommand() {
        return (new SlashCommandBuilder().setName(this.name).setDescription(this.description)
            ...
            .addIntegerOption(o => o.setName('count').setDescription('Number of keys to generate').setRequired(false).setMinValue(1).setMaxValue(50)));
    }
    generateKey(tier, days) {
    ...
    async executePrefix(m, _args) {
        ...
        if (!TIERS.includes(tier)) {
            await m.reply(`❌ Invalid tier. Use: ${TIERS.join(', ')}`);
            return;
        }
    ...
    async doGenerate(tier, days, count, createdBy) {
