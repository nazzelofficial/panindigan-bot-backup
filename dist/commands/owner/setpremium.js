...
export class SetPremiumCommand extends BaseCommand {
    constructor() {
        super({ name: 'setpremium', description: 'Manually set a user\'s premium tier (Owner only)', category: 'owner', premiumTier: 'free', cooldown: 0, guildOnly: false, ownerOnly: true, slashCommand: true, prefixCommand: true, aliases: ['givepremium', 'forcepremium'], examples: ['p!setpremium @user gold 30'] });
    }
    buildSlashCommand() {
        return (new SlashCommandBuilder().setName(this.name).setDescription(this.description)
            ...
            .addIntegerOption(o => o.setName('days').setDescription('Duration in days (0 = permanent)').setRequired(false).setMinValue(0)));
    }
    ...
    async executePrefix(m, _args) {
        ...
        if (!target || !TIERS.includes(tier)) {
            await m.reply('❌ Usage: `p!setpremium @user <tier> [days]`');
            return;
        }
        const handler = new PremiumHandler();
