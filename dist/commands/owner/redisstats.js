...
export class RedisstatsCommand extends BaseCommand {
    ...
    async run(interaction, message) {
        ...
        try {
            ...
            const get = (key) => info.match(new RegExp(`${key}:(.+)`))?.[1]?.trim() ?? 'N/A';
            const embed = new EmbedBuilder()
                .setColor(COLORS.default)
                .setTitle('📊 Redis Statistics')
                .addFields({ name: '🖥️ Version', value: get('redis_version'), inline: true }, { name: '⏱️ Uptime', value: `${get('uptime_in_seconds')}s`, inline: true }, { name: '🔌 Clients', value: get('connected_clients'), inline: true }, { name: '💾 Memory Used', value: get('used_memory_human'), inline: true }, { name: '✅ Cache Hits', value: get('keyspace_hits'), inline: true }, { name: '❌ Cache Misses', value: get('keyspace_misses'), inline: true }, { name: '🔑 Total Keys', value: get('db0')?.match(/keys=(\d+)/)?.[1] ?? '0', inline: true }, { name: '📤 Total Commands', value: get('total_commands_processed'), inline: true });
            await send(embed);
        }
        catch (err) {
