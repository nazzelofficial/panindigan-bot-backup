...
export class UserstatsCommand extends BaseCommand {
    constructor() {
        super({ name: 'userstats', description: 'Show all stats for any user by ID', category: 'owner', premiumTier: 'free', cooldown: 0, guildOnly: false, ownerOnly: true, slashCommand: true, prefixCommand: true, aliases: ['ustat'], examples: ['p!userstats 123456789'] });
    }
    async run(i, m, userId) {
        ...
        try {
            ...
            const [economy, level, cmdCount] = await Promise.all([
                prisma.economy?.findFirst({ where: { userId } }).catch(() => null),
                prisma.userLevel?.findFirst({ where: { userId } }).catch(() => null),
            ...
            const embed = new EmbedBuilder().setColor(COLORS.default).setTitle(`📊 User Stats: ${user?.tag ?? userId}`)
                ...
                .addFields({ name: '💰 Wallet', value: economy?.wallet?.toLocaleString() ?? '0', inline: true }, { name: '🏦 Bank', value: economy?.bank?.toLocaleString() ?? '0', inline: true }, { name: '⭐ Level', value: level?.level?.toString() ?? '0', inline: true }, { name: '💫 XP', value: level?.xp?.toLocaleString() ?? '0', inline: true }, { name: '🤖 Commands Used', value: cmdCount.toString(), inline: true }, { name: '💎 Premium', value: economy?.premiumTier ?? 'free', inline: true });
            await send(embed);
        }
        catch (err) {
