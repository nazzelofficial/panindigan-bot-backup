...
export class MassmuteCommand extends BaseCommand {
    ...
    async run(interaction, message, _args) {
        ...
        if (_args.length < 2)
            return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Usage: `massmute <role_id> <duration>`\nExample: `massmute @role 1h`'));
        ...
        if (!targets.size)
            return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ No moderatable members with that role.'));
        ...
        for (const [, member] of targets) {
            try {
                await member.timeout(duration, 'Mass mute by bot owner');
                success++;
            }
            catch {
                failed++;
