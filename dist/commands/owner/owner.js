...
export class OwnerCommand extends BaseCommand {
    ...
    async handleBotReload(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: '📝 Commands', value: 'Reloaded', inline: true }, { name: '⏱️ Cooldowns', value: 'Cleared', inline: true }, { name: '🕐 Reload Time', value: `<t:${Math.floor(Date.now() / 1000)}:T>`, inline: true })
                .setTimestamp();
    ...
    async handleBotStats(i) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(COLORS.success)
            .addFields({ name: '🏠 Servers', value: `${client.guilds.cache.size}`, inline: true }, { name: '👥 Users', value: `${client.users.cache.size}`, inline: true }, { name: '📺 Channels', value: `${client.channels.cache.size}`, inline: true }, { name: '⏱️ Uptime', value: `${days}d ${hours}h ${minutes}m ${seconds}s`, inline: true }, { name: '💾 Memory', value: `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB`, inline: true }, { name: '📡 Ping', value: `${client.ws.ping}ms`, inline: true }, { name: '🎮 Shards', value: `${client.shard?.count || 1}`, inline: true }, { name: '📝 Node Version', value: process.version.slice(1), inline: true })
            .setTimestamp();
        await i.reply({ embeds: [embed], ephemeral: true });
    }
    async handleBotHealth(i) {
        await i.deferReply({ ephemeral: true });
    ...
    async handleBotVersion(i) {
        const embed = new EmbedBuilder()
            .setTitle('🤖 Bot Version')
            .setColor(COLORS.info)
            .addFields({ name: 'Version', value: '1.0.0', inline: true }, { name: 'Discord.js', value: '14.x', inline: true }, { name: 'Node.js', value: process.version.slice(1), inline: true }, { name: 'TypeScript', value: '5.x', inline: true })
            .setTimestamp();
        await i.reply({ embeds: [embed], ephemeral: true });
    }
    async handleBotMaintenance(i) {
        await i.deferReply({ ephemeral: true });
        const prisma = getPrismaClient();
    ...
    async handleDbStats(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(COLORS.info)
                .addFields({ name: '👥 Users', value: `${userCount}`, inline: true }, { name: '🏠 Guilds', value: `${guildCount}`, inline: true }, { name: '📝 Commands Used', value: `${commandCount}`, inline: true }, { name: '💾 Database', value: 'PostgreSQL', inline: true }, { name: '🔗 Status', value: '✅ Connected', inline: true })
                .setTimestamp();
            await i.editReply({ embeds: [embed] });
        }
        catch (error) {
    ...
    async handleDbStatus(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(COLORS.success)
                .addFields({ name: '🔗 Connection', value: 'Connected', inline: true }, { name: '⚡ Latency', value: `${latency}ms`, inline: true }, { name: '💾 Type', value: 'PostgreSQL', inline: true })
                .setTimestamp();
            await i.editReply({ embeds: [embed] });
        }
        catch (error) {
    ...
    async handleRedisStats(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(COLORS.info)
                .addFields({ name: '🔗 Status', value: 'Connected', inline: true }, { name: '📦 Total Keys', value: info.match(/keyspace_hits:(\d+)/)?.[1] || 'N/A', inline: true }, { name: '💾 Memory Used', value: memory.match(/used_memory_human:(.+)/)?.[1] || 'N/A', inline: true }, { name: '⚡ Hits', value: info.match(/keyspace_hits:(\d+)/)?.[1] || 'N/A', inline: true }, { name: '❌ Misses', value: info.match(/keyspace_misses:(\d+)/)?.[1] || 'N/A', inline: true })
                .setTimestamp();
            await i.editReply({ embeds: [embed] });
        }
        catch (error) {
    ...
    async handleRedisInfo(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(COLORS.info)
                .addFields({ name: '🖥️ Version', value: info.match(/redis_version:(.+)/)?.[1] || 'N/A', inline: true }, { name: '⏱️ Uptime', value: info.match(/uptime_in_days:(\d+)/)?.[1] + ' days' || 'N/A', inline: true }, { name: '🏠 OS', value: info.match(/os:(.+)/)?.[1] || 'N/A', inline: true }, { name: '🔗 Connected Clients', value: info.match(/connected_clients:(\d+)/)?.[1] || 'N/A', inline: true })
                .setTimestamp();
            await i.editReply({ embeds: [embed] });
        }
        catch (error) {
    ...
    async handleRedisGet(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                .setTitle('📥 Redis Get')
                .setColor(COLORS.info)
                .addFields({ name: '🔑 Key', value: `\`${key}\``, inline: true }, { name: '📦 Value', value: value || 'null', inline: false })
                .setTimestamp();
            await i.editReply({ embeds: [embed] });
        }
        catch (error) {
    ...
    async handleGuildInfo(i) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: '🆔 ID', value: guild.id, inline: true }, { name: '👑 Owner', value: ownerTag, inline: true }, { name: '👥 Members', value: `${guild.memberCount}`, inline: true }, { name: '📺 Channels', value: `${guild.channels.cache.size}`, inline: true }, { name: '🎭 Roles', value: `${guild.roles.cache.size}`, inline: true }, { name: '📅 Created', value: `<t:${Math.floor(guild.createdAt.getTime() / 1000)}:R>`, inline: true }, { name: '🔒 Verification', value: guild.verificationLevel.toString(), inline: true }, { name: '💎 Premium', value: 'Checking...', inline: true })
            .setTimestamp();
        await i.editReply({ embeds: [embed] });
    }
    async handleGuildLeave(i) {
        await i.deferReply({ ephemeral: true });
        const id = i.options.getString('id', true);
        const guild = i.client.guilds.cache.get(id);
        if (!guild) {
    ...
    async handleGuildUnblacklist(i) {
        await i.deferReply({ ephemeral: true });
        const id = i.options.getString('id', true);
        const prisma = getPrismaClient();
        try {
    ...
    async handleGuildRevokePremium(i) {
        await i.deferReply({ ephemeral: true });
        const id = i.options.getString('id', true);
        const prisma = getPrismaClient();
        try {
    ...
    async handleUserInfo(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: '🆔 ID', value: id, inline: true }, { name: '📅 Account Created', value: user ? `<t:${Math.floor(user.createdAt.getTime() / 1000)}:R>` : 'Unknown', inline: true }, { name: '💎 Premium', value: dbUser?.premiumTier || 'free', inline: true }, { name: '🏠 Guilds', value: `${i.client.guilds.cache.filter(g => g.members.cache.has(id)).size}`, inline: true }, { name: '🚫 Blacklisted', value: dbUser?.blacklisted ? 'Yes' : 'No', inline: true })
                .setTimestamp();
            await i.editReply({ embeds: [embed] });
        }
        catch (error) {
    ...
    async handleKeyGenerate(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(COLORS.success)
                .addFields({ name: '🔑 Key', value: `\`\`${key}\`\``, inline: false }, { name: '💎 Tier', value: tier, inline: true }, { name: '👤 Created By', value: i.user.tag, inline: true })
                .setTimestamp();
            await i.editReply({ embeds: [embed] });
        }
        catch (error) {
    ...
    async handleKeyInfo(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(COLORS.info)
                .addFields({ name: '🔑 Key', value: `\`\`${keyData.key}\`\``, inline: false }, { name: '💎 Tier', value: keyData.tier, inline: true }, { name: '📊 Status', value: keyData.used ? '✅ Used' : '⬜ Available', inline: true }, { name: '👤 Used By', value: keyData.usedBy || 'N/A', inline: true }, { name: '📅 Created', value: `<t:${Math.floor(keyData.createdAt.getTime() / 1000)}:R>`, inline: true })
                .setTimestamp();
            await i.editReply({ embeds: [embed] });
        }
        catch (error) {
        ...
        collector.on('collect', async (interaction) => {
            if (interaction.customId === 'confirm_exec') {
                try {
                    ...
                    await ErrorHandler.generic(interaction, err);
                }
            }
            else {
                await interaction.update({ content: '❌ Cancelled.', components: [] });
    ...
    async executeSlash(i) {
        ...
        switch (subcommandGroup) {
            case 'bot':
                switch (subcommand) {
                    case 'restart':
                        await this.handleBotRestart(i);
                        break;
                    case 'shutdown':
                        await this.handleBotShutdown(i);
                        break;
                    case 'reload':
                        await this.handleBotReload(i);
                        break;
                    case 'stats':
                        await this.handleBotStats(i);
                        break;
                    case 'health':
                        await this.handleBotHealth(i);
                        break;
                    case 'version':
                        await this.handleBotVersion(i);
                        break;
                    case 'maintenance':
                        await this.handleBotMaintenance(i);
                        break;
                }
                break;
            case 'db':
                switch (subcommand) {
                    case 'stats':
                        await this.handleDbStats(i);
                        break;
                    case 'status':
                        await this.handleDbStatus(i);
                        break;
                    case 'backup':
                        await this.handleDbBackup(i);
                        break;
                    case 'restore':
                        await this.handleDbRestore(i);
                        break;
                    case 'clean':
                        await this.handleDbClean(i);
                        break;
                    case 'vacuum':
                        await this.handleDbVacuum(i);
                        break;
                    case 'tables':
                        await this.handleDbTables(i);
                        break;
                    case 'query':
                        await this.handleDbQuery(i);
                        break;
                }
                break;
            case 'redis':
                switch (subcommand) {
                    case 'stats':
                        await this.handleRedisStats(i);
                        break;
                    case 'info':
                        await this.handleRedisInfo(i);
                        break;
                    case 'keys':
                        await this.handleRedisKeys(i);
                        break;
                    case 'get':
                        await this.handleRedisGet(i);
                        break;
                    case 'set':
                        await this.handleRedisSet(i);
                        break;
                    case 'delete':
                        await this.handleRedisDelete(i);
                        break;
                    case 'flush':
                        await this.handleRedisFlush(i);
                        break;
                }
                break;
            case 'guild':
                switch (subcommand) {
                    case 'list':
                        await this.handleGuildList(i);
                        break;
                    case 'info':
                        await this.handleGuildInfo(i);
                        break;
                    case 'leave':
                        await this.handleGuildLeave(i);
                        break;
                    case 'blacklist':
                        await this.handleGuildBlacklist(i);
                        break;
                    case 'unblacklist':
                        await this.handleGuildUnblacklist(i);
                        break;
                    case 'premium':
                        await this.handleGuildPremium(i);
                        break;
                    case 'revokepremium':
                        await this.handleGuildRevokePremium(i);
                        break;
                }
                break;
            case 'user':
                switch (subcommand) {
                    case 'info':
                        await this.handleUserInfo(i);
                        break;
                    case 'blacklist':
                        await this.handleUserBlacklist(i);
                        break;
                    case 'unblacklist':
                        await this.handleUserUnblacklist(i);
                        break;
                    case 'reset':
                        await this.handleUserReset(i);
                        break;
                    case 'warn':
                        await this.handleUserWarn(i);
                        break;
                }
                break;
            case 'key':
                switch (subcommand) {
                    case 'generate':
                        await this.handleKeyGenerate(i);
                        break;
                    case 'list':
                        await this.handleKeyList(i);
                        break;
                    case 'info':
                        await this.handleKeyInfo(i);
                        break;
                    case 'revoke':
                        await this.handleKeyRevoke(i);
                        break;
                    case 'delete':
                        await this.handleKeyDelete(i);
                        break;
                }
                break;
            case 'owners':
                switch (subcommand) {
                    case 'list':
                        await this.handleOwnersList(i);
                        break;
                    case 'add':
                        await this.handleOwnersAdd(i);
                        break;
                    case 'remove':
                        await this.handleOwnersRemove(i);
                        break;
                }
                break;
            case 'announce':
                switch (subcommand) {
                    case 'send':
                        await this.handleAnnounceSend(i);
                        break;
                    case 'dm':
                        await this.handleAnnounceDm(i);
                        break;
                }
                break;
            case 'dev':
                switch (subcommand) {
                    case 'eval':
                        await this.handleDevEval(i);
                        break;
                    case 'exec':
                        await this.handleDevExec(i);
                        break;
                    case 'test':
                        await this.handleDevTest(i);
                        break;
                    case 'logs':
                        await this.handleDevLogs(i);
                        break;
                    default:
