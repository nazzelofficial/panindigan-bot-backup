...
export class UserblacklistCommand extends BaseCommand {
    constructor() {
        super({ name: 'userblacklist', description: 'Add a user to the global bot blacklist', category: 'owner', premiumTier: 'free', cooldown: 0, guildOnly: false, ownerOnly: true, slashCommand: true, prefixCommand: true, aliases: ['ublk'], examples: ['p!userblacklist 123456789 spamming'] });
    }
    async run(i, m, userId, reason) {
    ...
    async executeSlash(i) { await this.run(i, null, i.options.getString('user_id', true), i.options.getString('reason') ?? ''); }
