...
export class GlobalAnnounceCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addStringOption(o => o.setName('message').setDescription('Announcement message (add --confirm to execute)').setRequired(true));
    }
    async sendToAllGuilds(client, announcementText) {
        let sent = 0;
        let failed = 0;
    ...
    async executeSlash(interaction) {
        ...
        if (!confirmed) {
            ...
            const warningEmbed = new EmbedBuilder()
                ...
                .setDescription(`**This will send a message to ALL ${guildCount} servers the bot is in!**\n\n` +
                `**Preview:**\n${announcementText}\n\n` +
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: 'Message', value: announcementText.slice(0, 1024), inline: false }, { name: 'Sent', value: String(sent), inline: true }, { name: 'Failed', value: String(failed), inline: true })
            .setTimestamp();
