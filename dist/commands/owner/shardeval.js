        ...
        try {
            ...
            if (!client.shard) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: 'Code', value: codeBlock('js', code.slice(0, 512)), inline: false }, { name: 'Output', value: codeBlock('js', String(result).slice(0, 900)), inline: false })
                    .setTimestamp();
                await m.reply({ embeds: [embed] });
                return;
            }
            const results = await client.shard.broadcastEval(async (c, { evalCode }) => {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: 'Code', value: codeBlock('js', code.slice(0, 512)), inline: false }, { name: 'Output', value: codeBlock('js', shardResult.output || 'undefined'), inline: false })
                .setTimestamp();
            await m.reply({ embeds: [embed] });
        }
        catch (err) {
