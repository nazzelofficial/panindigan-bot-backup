...
export class MasskickCommand extends BaseCommand {
    ...
    async run(interaction, message, _args) {
        ...
        if (!args[0])
            return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Provide a role ID.'));
        ...
        if (!targets.size)
            return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ No kickable members with that role.'));
        ...
        for (const [, member] of targets) {
            try {
                await member.kick('Mass kick by bot owner');
                success++;
            }
            catch {
                failed++;
