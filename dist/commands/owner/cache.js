...
export class CacheCommand extends BaseCommand {
    ...
    getStatsEmbed(requesterTag) {
        const mem = process.memoryUsage();
        const cacheSize = Object.keys(require.cache).length;
        return new EmbedBuilder()
            .setColor(COLORS.info)
            .setTitle(`${EMOJIS.info} Cache & Memory Stats`)
            .addFields({ name: '📦 Module Cache Size', value: `\`${cacheSize}\` modules`, inline: true }, { name: '🧠 Heap Used', value: this.formatBytes(mem.heapUsed), inline: true }, { name: '📐 Heap Total', value: this.formatBytes(mem.heapTotal), inline: true }, { name: '💾 RSS', value: this.formatBytes(mem.rss), inline: true }, { name: '🔗 External', value: this.formatBytes(mem.external), inline: true }, { name: '📋 Array Buffers', value: this.formatBytes(mem.arrayBuffers), inline: true })
            .setFooter({ text: `Requested by ${requesterTag}` })
            .setTimestamp();
    }
    getClearEmbed(cleared, requesterTag) {
        const mem = process.memoryUsage();
        return new EmbedBuilder()
            .setColor(COLORS.success)
            .setTitle(`${EMOJIS.success} Cache Cleared`)
            .setDescription(`Module cache has been cleared.`)
            .addFields({ name: '🗑️ Modules Removed', value: `\`${cleared}\``, inline: true }, { name: '🧠 Heap After', value: this.formatBytes(mem.heapUsed), inline: true })
            .setFooter({ text: `Requested by ${requesterTag}` })
            .setTimestamp();
    }
