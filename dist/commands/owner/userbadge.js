...
export class UserBadgeCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addStringOption(o => o.setName('badge_name').setDescription('Badge name to add').setRequired(true));
    }
    async addBadge(userId, badgeName) {
        const mongo = await getMongoClient();
        const db = mongo.db();
        const collection = db.collection('user_profiles');
        const result = await collection.findOneAndUpdate({ userId }, {
    ...
    async executeSlash(interaction) {
        ...
        try {
            ...
            const badges = result?.badges ?? [badgeName];
            const embed = new EmbedBuilder()
                .setTitle(`${EMOJIS.premium} Badge Added`)
                .setColor(COLORS.success)
                .addFields({ name: 'User ID', value: userId, inline: true }, { name: 'Badge Added', value: `\`${badgeName}\``, inline: true }, { name: 'All Badges', value: badges.map(b => `\`${b}\``).join(', ') || 'None', inline: false })
                .setTimestamp();
