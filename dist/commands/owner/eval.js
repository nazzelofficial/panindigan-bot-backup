...
export class EvalCommand extends BaseCommand {
    constructor() {
        super({ name: 'eval', description: 'Execute arbitrary JavaScript code (Owner only)', category: 'owner', premiumTier: 'free', cooldown: 0, guildOnly: false, ownerOnly: true, slashCommand: true, prefixCommand: true, aliases: ['ev', 'evaluate'], examples: ['p!eval client.users.cache.size'] });
    }
    buildSlashCommand() {
    ...
    async runEval(code, context, isAsync) {
    ...
    async executeSlash(i) {
        ...
        if (silent) {
            await i.editReply({ content: success ? '✅ Executed.' : '❌ Error occurred.' });
            return;
        }
        const embed = new EmbedBuilder()
            .setTitle(success ? '✅ Eval Result' : '❌ Eval Error')
            .setColor(success ? COLORS.success : COLORS.error)
            .addFields({ name: '📥 Input', value: codeBlock('js', code.slice(0, 512)), inline: false }, { name: '📤 Output', value: codeBlock('js', output.slice(0, 900)), inline: false }, { name: '⏱️ Time', value: `${timeTaken}ms`, inline: true }).setTimestamp();
        await i.editReply({ embeds: [embed] });
    }
    async executePrefix(m, _args) {
        const code = _args.join(' ');
        if (!code) {
            await m.reply('❌ Provide code to evaluate.');
            return;
        }
