...
export class FunCommand extends BaseCommand {
    ...
    async executeSlash(i) {
        ...
        if (subcommandGroup === 'humor') {
            switch (subcommand) {
                case 'joke':
                    await this.handleJoke(i);
                    break;
                case 'dadjoke':
                    await this.handleDadJoke(i);
                    break;
                case 'meme':
                    await this.handleMeme(i);
                    break;
                case '8ball':
                    await this.handle8Ball(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'cat':
                    await this.handleCat(i);
                    break;
                case 'dog':
                    await this.handleDog(i);
                    break;
                case 'fox':
                    await this.handleFox(i);
                    break;
                case 'duck':
                    await this.handleDuck(i);
                    break;
                case 'panda':
                    await this.handlePanda(i);
                    break;
                case 'shiba':
                    await this.handleShiba(i);
                    break;
                case 'capybara':
                    await this.handleCapybara(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case '8ball':
                    ...
                    break;
                case 'tarot':
                    await this.handleTarot(i);
                    break;
                case 'horoscope':
                    await this.handleHoroscope(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'ship':
                    await this.handleShip(i);
                    break;
                case 'rate':
                    await this.handleRate(i);
                    break;
                case 'choose':
                    await this.handleChoose(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'ascii':
                    await this.handleAscii(i);
                    break;
                case 'quote':
                    await this.handleQuote(i);
                    break;
                case 'fact':
                    await this.handleFact(i);
                    break;
                case 'birthday':
    ...
    async handle8Ball(i) {
        ...
        try {
            ...
            const embed = EmbedManager.fun('🎱 Magic 8-Ball', `> ${question}\n\n${responseEmoji} **${response}**`, { timestamp: true });
            await i.editReply({ embeds: [embed] });
        }
        catch (error) {
            await ErrorHandler.generic(i, error);
  ...
  ${text.split('').map(() => '█').join('  ')}
    ...
    async handleQuote(i) {
        ...
        try {
            ...
            const embed = EmbedManager.fun('💬 Inspirational Quote', `> *"${quote.text}"*\n\n**— ${quote.author}**`, {
