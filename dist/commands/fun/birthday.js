...
export class BirthdayCommand extends BaseCommand {
    ...
    async setOrUpdateBirthday(userId, guildId, username, month, day) {
        const db = await connectMongoDB();
        const col = db.collection('birthdays');
        await col.updateOne({ userId, guildId }, { $set: { userId, guildId, month, day, username } }, { upsert: true });
    }
    ...
    async executeSlash(interaction) {
        ...
        if (sub === 'set') {
            ...
            await this.setOrUpdateBirthday(interaction.user.id, interaction.guildId, interaction.user.username, parsed.month, parsed.day);
            const embed = new EmbedBuilder()
            ...
            const lines = birthdays.slice(0, 20).map((b, i) => `${i + 1}. **${b.username}** — ${this.monthName(b.month)} ${b.day}`);
            const embed = new EmbedBuilder()
    ...
    async executePrefix(message, _args) {
        ...
        if (sub === 'set') {
            ...
            await this.setOrUpdateBirthday(message.author.id, message.guildId, message.author.username, parsed.month, parsed.day);
            const embed = new EmbedBuilder()
                .setTitle('🎂 Birthday Set!')
                .setDescription(`Your birthday has been set to **${this.monthName(parsed.month)} ${parsed.day}**!`)
                .setColor(COLORS.success)
