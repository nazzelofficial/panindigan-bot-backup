...
export class StarboardCommand extends BaseCommand {
    ...
    async executeSlash(i) {
        ...
        if (sub === 'setup') {
            ...
            embed.addFields({ name: 'Emoji', value: guild?.starboardEmoji || '⭐', inline: true }, { name: 'Threshold', value: `${guild?.starboardThreshold || 3} stars`, inline: true }, { name: 'Status', value: guild?.starboardLocked ? '🔒 Locked' : '✅ Active', inline: true });
    ...
    async handleStats(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: '🌟 Starred Messages', value: `${starredMessages}`, inline: true }, { name: '🚫 Ignored Channels', value: `${ignoredChannels}`, inline: true }, { name: '⭐ Threshold', value: `${guild?.starboardThreshold || 3}`, inline: true }, { name: '🔒 Status', value: guild?.starboardLocked ? 'Locked' : 'Active', inline: true })
                .setTimestamp();
            await i.editReply({ embeds: [embed] });
        }
        catch (error) {
    ...
    async handleRandom(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: '🔗 Original', value: `[Jump to message](${random.originalMessageUrl})`, inline: true })
                .setTimestamp();
            await i.editReply({ embeds: [embed] });
        }
        catch (error) {
