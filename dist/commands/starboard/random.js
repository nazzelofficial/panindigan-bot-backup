...
export class StarboardRandomCommand extends BaseCommand {
    constructor() {
        super({ name: 'starboard-random', description: 'Get a random starred message', category: 'starboard', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['sb-random', 'sbrandom', 'sbrnd'], examples: ['/starboard-random'] });
    }
    async getRandom(guildId) {
        const col = getCollection('starboard');
        const count = await col.countDocuments({ guildId });
        if (!count)
            return '📭 No starred messages in this server yet.';
        const skip = Math.floor(Math.random() * count);
        const doc = await col.findOne({ guildId }, { skip });
        if (!doc)
            return '📭 Could not find a starred message.';
