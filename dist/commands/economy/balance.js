...
export class BalanceCommand extends BaseCommand {
    ...
    async fetchBalance(userId, guildId) {
    ...
    async executeSlash(interaction) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: `👛 Wallet`, value: `${symbol}${wallet.toLocaleString()}`, inline: true }, { name: `🏦 Bank`, value: `${symbol}${bank.toLocaleString()}`, inline: true }, { name: `💰 Total`, value: `${symbol}${total.toLocaleString()}`, inline: false }, { name: `📊 Net Worth`, value: `${symbol}${Math.max(total, networth).toLocaleString()}`, inline: true })
    ...
    async executePrefix(message, _args) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(COLORS.info)
                .setThumbnail(target.displayAvatarURL({ size: 128 }))
                .addFields({ name: `👛 Wallet`, value: `${symbol}${wallet.toLocaleString()}`, inline: true }, { name: `🏦 Bank`, value: `${symbol}${bank.toLocaleString()}`, inline: true }, { name: `💰 Total`, value: `${symbol}${total.toLocaleString()}`, inline: false })
                .setFooter({ text: `${config.economy.currencyName} • Panindigan Economy` })
                .setTimestamp();
