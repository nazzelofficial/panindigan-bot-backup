...
export class EconomyCommand extends BaseCommand {
    ...
    async executeSlash(i) {
        ...
        if (subcommandGroup === 'currency') {
            switch (subcommand) {
                case 'balance':
                    await this.handleBalance(i);
                    break;
                case 'wallet':
                    await this.handleWallet(i);
                    break;
                case 'bank':
                    await this.handleBank(i);
                    break;
                case 'deposit':
                    await this.handleDeposit(i);
                    break;
                case 'withdraw':
                    await this.handleWithdraw(i);
                    break;
                case 'transfer':
                    await this.handleTransfer(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'daily':
                    await this.handleDaily(i);
                    break;
                case 'weekly':
                    await this.handleWeekly(i);
                    break;
                case 'monthly':
                    await this.handleMonthly(i);
                    break;
                case 'work':
                    await this.handleWork(i);
                    break;
                case 'beg':
                    await this.handleBeg(i);
                    break;
                case 'crime':
                    await this.handleCrime(i);
                    break;
                case 'invest':
                    await this.handleInvest(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'view':
                    await this.handleShopView(i);
                    break;
                case 'buy':
                    await this.handleShopBuy(i);
                    break;
                case 'sell':
                    await this.handleShopSell(i);
                    break;
                case 'inventory':
            ...
            switch (subcommand) {
                case 'coinflip':
                    await this.handleCoinflip(i);
                    break;
                case 'dice':
                    await this.handleDice(i);
                    break;
                case 'slots':
            ...
            switch (subcommand) {
                case 'start':
                    await this.handleBusinessStart(i);
                    break;
                case 'upgrade':
                    await this.handleBusinessUpgrade(i);
                    break;
                case 'sell':
                    await this.handleBusinessSell(i);
                    break;
                case 'info':
                    await this.handleBusinessInfo(i);
                    break;
            }
        }
        ...
        try {
            ...
            if (!economy) {
                ...
                if (Date.now() - lastDaily.getTime() < cooldown) {
                    ...
                    const embed = EmbedManager.economy('Daily Reward on Cooldown', `⏰ You already claimed your daily reward!\n\n**Available again:** <t:${Math.floor(resetTime.getTime() / 1000)}:R>`, {
                        fields: [
        ...
        try {
            ...
            if (!economy) {
                ...
                if (Date.now() - lastWeekly.getTime() < cooldown) {
                    ...
                    const embed = EmbedManager.economy('Weekly Reward on Cooldown', `⏰ You already claimed your weekly reward!\n\n**Available again:** <t:${Math.floor(resetTime.getTime() / 1000)}:R>`, {
                        fields: [
                            { name: '📅 Resets at', value: `<t:${Math.floor(resetTime.getTime() / 1000)}:F>`, inline: false },
        ...
        try {
            ...
            if (!economy) {
                ...
                if (Date.now() - lastMonthly.getTime() < cooldown) {
                    ...
                    const embed = EmbedManager.economy('Monthly Reward on Cooldown', `⏰ You already claimed your monthly reward!\n\n**Available again:** <t:${Math.floor(resetTime.getTime() / 1000)}:R>`, {
                        fields: [
                            { name: '📅 Resets at', value: `<t:${Math.floor(resetTime.getTime() / 1000)}:F>`, inline: false },
