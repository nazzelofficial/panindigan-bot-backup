...
export class AnalyticsAccessCommand extends BaseCommand {
    constructor() {
        super({ name: 'analyticsaccess', description: 'Access your personal analytics dashboard (Gold+)', category: 'premium', premiumTier: 'gold', cooldown: 10, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['myanalytics', 'personalanalytics'], examples: ['/analyticsaccess'] });
    }
    async executeSlash(i) {
        ...
        const embed = new EmbedBuilder().setTitle('📊 Personal Analytics').setColor(COLORS.gold)
            ...
            .addFields({ name: '📋 Commands Today', value: `${cmdToday}`, inline: true }, { name: '📅 Commands This Month', value: `${cmdMonth}`, inline: true }, { name: '🤖 Total AI Requests', value: `${aiTotal}`, inline: true }, { name: '⭐ Total XP', value: `${totalXp.toLocaleString()}`, inline: true }, { name: '📈 Avg Level', value: `${avgLevel.toFixed(1)}`, inline: true }, { name: '💰 Total Balance', value: `₱${totalBalance.toLocaleString()}`, inline: true }, { name: '🌐 Servers Tracked', value: `${userProfiles.length}`, inline: true })
            .setTimestamp();
