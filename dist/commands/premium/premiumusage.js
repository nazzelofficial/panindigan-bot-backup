...
export class PremiumUsageCommand extends BaseCommand {
    ...
    async buildEmbed(userId) {
        const prisma = getPrismaClient();
        const redis = getRedisClient();
        let tier = 'free';
        try {
            const prem = await prisma.premium?.findUnique({ where: { userId } }).catch(() => null);
            if (prem?.active)
                tier = prem.tier || 'free';
        }
        catch { }
        ...
        const formatLimit = (used, limit) => {
            if (limit === -1)
                return `${used} / ♾️ Unlimited`;
            const bar = Math.round((used / limit) * 10);
            return `${used} / ${limit} \`${'█'.repeat(bar)}${'░'.repeat(10 - bar)}\``;
        };
        const tierLabels = { free: '🆓 Free', bronze: '🥉 Bronze', silver: '⭐ Silver', gold: '💎 Gold', diamond: '👑 Diamond' };
        return new EmbedBuilder()
            .setTitle(`${EMOJIS.premium} Premium Usage`)
            .setColor(COLORS.gold)
            .addFields({ name: '💎 Your Tier', value: tierLabels[tier] || '🆓 Free', inline: true }, { name: '🔄 Resets', value: 'Daily at midnight UTC', inline: true }, { name: '\u200B', value: '\u200B', inline: false }, { name: '🖼️ AI Image Generation', value: formatLimit(imagesUsed, imageLimit), inline: false }, { name: '💬 AI Chat Messages', value: formatLimit(chatUsed, chatLimit), inline: false })
