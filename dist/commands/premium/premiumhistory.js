...
export class PremiumHistoryCommand extends BaseCommand {
    constructor() {
        super({ name: 'premium-history', description: 'View your premium subscription history', category: 'premium', premiumTier: 'free', cooldown: 5, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['premiumlog', 'myhistory'], examples: ['/premium-history'] });
    }
    ...
    async executePrefix(m) {
        ...
        if (!premium.length) {
            await m.reply('📭 No premium history.');
            return;
        }
        const embed = new EmbedBuilder().setTitle('📜 Premium History').setColor(COLORS.gold)
