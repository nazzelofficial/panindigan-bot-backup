...
export class PriorityQueueCommand extends BaseCommand {
    constructor() {
        super({ name: 'priorityqueue', description: 'Add a song to the front of the music queue (Bronze+ perk) 🥉', category: 'premium', premiumTier: 'bronze', cooldown: 3, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['pq', 'priority', 'frontsong'], examples: ['/priorityqueue <song URL or name>', 'p!priorityqueue Never Gonna Give You Up'] });
    }
    buildSlashCommand() {
    ...
    async handle(userId, guildId, query, send) {
        const prisma = getPrismaClient();
        // Look up current queue for this guild
        const queue = await prisma.musicQueue?.findFirst?.({
            where: { guildId },
        ...
        if (!queue) {
            ...
            await prisma.musicQueueRequest?.create?.({
            ...
            const maxPos = await prisma.musicQueueSong?.findFirst?.({
                where: { queueId: queue.id },
                orderBy: { position: 'asc' },
                select: { position: true },
            }).catch(() => null);
            const frontPos = (maxPos?.position ?? 1) - 1;
            await prisma.musicQueueSong?.create?.({
    ...
    async executePrefix(m, _args) {
        if (!args.length) {
            await m.reply('❌ Usage: `p!priorityqueue <song name or URL>`');
            return;
        }
