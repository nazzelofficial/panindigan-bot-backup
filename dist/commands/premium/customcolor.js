...
export class CustomColorCommand extends BaseCommand {
    constructor() {
        super({ name: 'customcolor', description: 'Set a custom embed color for your profile (Silver+ perk)', category: 'premium', premiumTier: 'silver', cooldown: 5, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['embedcolor', 'mycolor'], examples: ['/customcolor #ff6699', 'p!customcolor #ff6699'] });
    }
    buildSlashCommand() {
    ...
    async executeSlash(i) {
        ...
        if (color === null) {
            await i.reply({ content: '❌ Invalid hex color. Example: `#ff6699`', ephemeral: true });
            return;
        }
    ...
    async executePrefix(m, _args) {
        ...
        if (!colorStr) {
            await m.reply('❌ Usage: `p!customcolor <#hexcode>`');
            return;
        }
        const color = this.parseColor(colorStr);
        if (color === null) {
            await m.reply('❌ Invalid hex color.');
            return;
        }
        const prisma = getPrismaClient();
