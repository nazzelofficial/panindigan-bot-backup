...
export class CooldownCheckCommand extends BaseCommand {
    constructor() {
        super({ name: 'cooldowncheck', description: 'Check your current cooldowns on commands', category: 'premium', premiumTier: 'free', cooldown: 3, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['cd', 'cooldowns'], examples: ['/cooldowncheck'] });
    }
    async executeSlash(i) {
        ...
        if (!keys.length) {
            ...
            for (const key of keys.slice(0, 15)) {
                ...
                if (ttl > 0)
                    cooldowns.push(`• \`${command}\` — ${ttl}s remaining`);
            }
    ...
    async executePrefix(m) {
        const redis = getRedisClient();
        if (!redis) {
            await m.reply('❌ Redis not connected.');
            return;
        }
        const keys = await redis.keys(`cooldown:${m.author.id}:*`);
        if (!keys.length) {
            await m.reply('✅ No active cooldowns!');
            return;
        }
        const lines = [];
