...
export class RoadmapVoteCommand extends BaseCommand {
    constructor() {
        super({ name: 'roadmap-vote', description: 'Vote on upcoming bot features (Diamond perk)', category: 'premium', premiumTier: 'diamond', cooldown: 86400, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['votepoll', 'featurevote'], examples: ['/roadmap-vote voicechat'] });
    }
    async executeSlash(i) {
        ...
        if (!featureId) {
            ...
            const voteCounts = Object.fromEntries(votes.map((v) => [v._id, v.count]));
        ...
        if (!feature) {
            await i.reply({ content: '❌ Invalid feature ID.', ephemeral: true });
            return;
        }
        const existing = await col.findOne({ userId: i.user.id, featureId });
        if (existing) {
            await i.reply({ content: `⚠️ You already voted for **${feature.name}**!`, ephemeral: true });
            return;
        }
