...
export class ExportDataCommand extends BaseCommand {
    constructor() {
        super({ name: 'export-data', description: 'Export your personal bot data as JSON (Gold+ perk)', category: 'premium', premiumTier: 'gold', cooldown: 60, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['mydata', 'dataexport'], examples: ['/export-data'] });
    }
    async executeSlash(i) {
        ...
        const exportData = {
            ...
            premium,
            leveling,
            aiConversations: conversations.map((c) => ({ guildId: c.guildId, messageCount: c.messages?.length || 0, lastUpdated: c.updatedAt })),
        };
