...
export class PremiumTransferCommand extends BaseCommand {
    constructor() {
        super({ name: 'premium-transfer', description: 'Transfer your premium subscription to another server (Diamond)', category: 'premium', premiumTier: 'diamond', cooldown: 86400, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['transferpremium', 'movepremium'], examples: ['/premium-transfer'] });
    }
    buildSlashCommand() {
    ...
    async executeSlash(i) {
        ...
        if (!premium) {
            await i.reply({ content: '❌ You don\'t have an active premium subscription on this server.', ephemeral: true });
            return;
        }
        const embed = new EmbedBuilder().setTitle('⚠️ Premium Transfer').setColor(COLORS.warning)
        ...
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`premium_transfer:${i.guildId}:${targetGuildId}:${premium.id}`).setLabel('✅ Confirm Transfer').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId('cancel').setLabel('❌ Cancel').setStyle(ButtonStyle.Secondary));
        await i.reply({ embeds: [embed], components: [row], ephemeral: true });
    }
    async executePrefix(m, _args) {
        if (!args[0]) {
            await m.reply('❌ Usage: `p!premium-transfer <guild_id>`');
            return;
        }
        const prisma = getPrismaClient();
        const premium = await prisma.premium.findFirst({ where: { userId: m.author.id, guildId: m.guildId, active: true } });
        if (!premium) {
            await m.reply('❌ No active premium on this server.');
            return;
        }
