...
export class HdImageCommand extends BaseCommand {
    constructor() {
        super({ name: 'hdimage', description: 'Generate a high-definition AI image (Diamond perk)', category: 'premium', premiumTier: 'diamond', cooldown: 30, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['hd-image', 'imagine-hd', 'imagehd'], examples: ['/hdimage a majestic dragon', 'p!hdimage a city at night'] });
    }
    buildSlashCommand() {
    ...
    async executeSlash(i) {
        ...
        await i.deferReply();
        try {
            const result = await aiEngine.generateImage(prompt, { size: '1792x1024', quality: 'hd', style: style });
            if (!result.imageUrl) {
    ...
    async executePrefix(m, _args) {
        if (!args.length) {
            await m.reply('❌ Usage: `p!hdimage <prompt>`');
            return;
        }
        ...
        try {
            ...
            if (!result.imageUrl) {
                await msg.edit('❌ Failed to generate image.');
                return;
            }
