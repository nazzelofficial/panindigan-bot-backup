...
export class PremiumCommand extends BaseCommand {
    ...
    async executeSlash(i) {
        ...
        if (sub === 'info') {
            ...
            if (result.success) {
                await SuccessHandler.premium(i, result.tier || 'Bronze', TIER_INFO[result.tier]?.perks || []);
            }
            else {
            ...
            if (!giverStatus?.active || giverStatus.tier === 'free') {
                await ErrorHandler.generic(i, new Error('You need an active premium subscription to gift premium.'));
                return;
            }
            // Apply gift
            await handler.setUserPremium(target.id, tier, 30); // 30 days
            await SuccessHandler.premium(i, TIER_INFO[tier]?.label || tier, TIER_INFO[tier]?.perks || []);
        }
    }
