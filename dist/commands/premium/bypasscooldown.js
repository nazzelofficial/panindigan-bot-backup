...
export class BypassCooldownCommand extends BaseCommand {
    constructor() {
        super({ name: 'bypass-cooldown', description: 'Bypass cooldown for a specific command (Gold+)', category: 'premium', premiumTier: 'gold', cooldown: 30, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['bypasscd', 'skipcd'], examples: ['/bypass-cooldown play', 'p!bypass-cooldown play'] });
    }
    ...
    async executePrefix(m, _args) {
        ...
        const redis = getRedisClient();
        if (redis) {
            await redis.del(`cooldown:${m.author.id}:${command}`);
        }
