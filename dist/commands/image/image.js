...
export class ImageCommand extends BaseCommand {
    constructor() {
        super({ name: 'image', description: 'Generate images and apply image effects', category: 'image', premiumTier: 'free', cooldown: 10, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['img', 'generate', 'imagegen'], examples: ['/image generate a cute cat', '/image avatar @user', '/image blur @user'] });
    }
    buildSlashCommand() {
        return (new SlashCommandBuilder().setName(this.name).setDescription(this.description)
            ...
            .addSubcommand(s => s.setName('flip').setDescription('Flip avatar').addUserOption(o => o.setName('user').setDescription('Target user').setRequired(false))));
    }
    async fetchAvatarBuffer(user) {
    ...
    async executeSlash(i) {
        ...
        try {
            if (sub === 'generate') {
                ...
                if (!imageUrl) {
                    await ErrorHandler.generic(i, 'Failed to generate image.');
                    return;
                }
                ...
                const bannerUrl = target.bannerURL?.({ extension: 'png', size: 4096 });
                if (!bannerUrl) {
                    await ErrorHandler.generic(i, 'This user has no banner.');
                    return;
                }
