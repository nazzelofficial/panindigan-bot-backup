...
export class ImageCommand extends BaseCommand {
    constructor() {
        super({ name: 'image', description: 'Generate images and apply image effects', category: 'image', premiumTier: 'gold', cooldown: 10, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['img', 'generate', 'imagegen'], examples: ['/image generate a cute cat', '/image avatar @user', '/image blur @user'] });
    }
    buildSlashCommand() {
    ...
    async executeSlash(i) {
        ...
        try {
            if (sub === 'generate') {
                ...
                if (!imageUrl) {
                    await i.editReply({ content: '❌ Failed to generate image.' });
                    return;
                }
                ...
                if (!bannerUrl) {
                    await i.editReply({ content: '❌ This user has no banner.' });
                    return;
                }
