...
export class GifCommand extends BaseCommand {
    constructor() {
        super({ name: 'gif', description: 'Create a custom animated GIF from an avatar or image (Diamond) 🎬', category: 'image', premiumTier: 'diamond', cooldown: 15, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['makegif', 'creategif', 'animategif'], examples: ['/gif @user spin', 'p!gif @user pulse'] });
    }
    buildSlashCommand() {
        return (new SlashCommandBuilder().setName(this.name).setDescription(this.description)
            ...
            .addChoices({ name: 'Spin', value: 'spin' }, { name: 'Pulse', value: 'pulse' }, { name: 'Shake', value: 'shake' }, { name: 'Bounce', value: 'bounce' })))
            .setDMPermission(false));
    }
    async generateFrames(avatarUrl, effect, frameCount) {
        const size = 200;
        const frames = [];
    ...
    async handle(avatarUrl, effect, send) {
        const frameCount = 16;
    ...
    async executeSlash(i) {
        ...
        if (sub !== 'create') {
            await i.reply({ content: '❌ Use `/gif create`', ephemeral: true });
            return;
        }
