...
export class ImagineUpscaleCommand extends BaseCommand {
    constructor() {
        super({ name: 'imagineupscale', description: 'AI image upscaling — increase resolution up to 4x (Diamond) 🔍', category: 'image', premiumTier: 'diamond', cooldown: 20, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['upscale', 'aiupscale', 'imagine-upscale'], examples: ['/imagineupscale [attach image] 4x', 'p!imagineupscale [attach image]'] });
    }
    buildSlashCommand() {
    ...
    async upscale(imageBuffer, scale) {
    ...
    async executeSlash(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: '📐 Original', value: `${meta.width}×${meta.height}`, inline: true }, { name: '📐 Upscaled', value: `${outMeta.width}×${outMeta.height}`, inline: true }, { name: '🔍 Scale', value: `${scale}x`, inline: true })
    ...
    async executePrefix(m, _args) {
        ...
        if (!attachment) {
            await m.reply('❌ Please attach an image to upscale.');
            return;
        }
        const scale = parseInt(args.find(a => a === '2' || a === '4') || '2');
        try {
            const res = await fetch(attachment.url);
            const buf = Buffer.from(await res.arrayBuffer());
            const upscaled = await this.upscale(buf, scale);
