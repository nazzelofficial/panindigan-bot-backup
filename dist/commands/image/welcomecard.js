...
export class WelcomeCardCommand extends BaseCommand {
    constructor() {
        super({ name: 'welcomecard', description: 'Preview the welcome card for a user', category: 'image', premiumTier: 'free', cooldown: 8, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['welcome-card', 'welcomepreview'], examples: ['/welcomecard @user', 'p!welcomecard @user'] });
    }
    buildSlashCommand() {
        return (new SlashCommandBuilder().setName(this.name).setDescription(this.description).addUserOption(o => o.setName('user').setDescription('User to preview for').setRequired(false)).setDMPermission(false));
    }
    async executeSlash(i) {
        ...
        try {
            ...
            await i.editReply({ content: '❌ Failed to generate welcome card.' });
        }
    }
