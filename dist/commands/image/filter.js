...
export class FilterCommand extends BaseCommand {
    constructor() {
        super({ name: 'filter', description: 'Apply a color filter to a user\'s avatar', category: 'image', premiumTier: 'free', cooldown: 5, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['colorfilter', 'imagefilter'], examples: ['/filter sepia @user', 'p!filter warm @user'] });
    }
    buildSlashCommand() {
    ...
    async executePrefix(m, _args) {
        ...
        if (!filterName || !FILTERS[filterName]) {
            await m.reply(`❌ Valid filters: ${Object.keys(FILTERS).join(', ')}`);
            return;
        }
