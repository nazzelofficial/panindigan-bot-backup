...
export class ImagineStyleCommand extends BaseCommand {
    constructor() {
        super({ name: 'imaginestyle', description: 'AI image with style transfer (Diamond) 🎭', category: 'image', premiumTier: 'diamond', cooldown: 30, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['aistyle', 'imagestyle', 'imagine-style'], examples: ['/imaginestyle anime a warrior princess', 'p!imaginestyle oilpainting a sunrise over mountains'] });
    }
    buildSlashCommand() {
    ...
    async executePrefix(m, _args) {
        ...
        if (!prompt) {
            await m.reply('❌ Provide a prompt after the style.');
            return;
        }
