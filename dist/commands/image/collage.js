...
export class CollageCommand extends BaseCommand {
    constructor() {
        super({ name: 'collage', description: 'Create an avatar collage of multiple users', category: 'image', premiumTier: 'free', cooldown: 10, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['avatar-collage', 'members-grid'], examples: ['/collage @user1 @user2 @user3'] });
    }
    buildSlashCommand() {
    ...
    async executeSlash(i) {
        const users = [i.options.getUser('user1'), i.options.getUser('user2'), i.options.getUser('user3'), i.options.getUser('user4')].filter(Boolean);
        await i.deferReply();
        try {
            ...
            await i.editReply({ content: '❌ Failed to create collage.' });
        }
    }
    async executePrefix(m, _args) {
        const users = m.mentions.users.first(4);
        if (!users.length) {
            await m.reply('❌ Mention at least 1 user.');
            return;
        }
        try {
            const size = 128;
