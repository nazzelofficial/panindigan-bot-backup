        ...
        try {
            if (sub === 'color') {
                ...
                if (!this.isValidHex(hex)) {
                    await i.reply({ content: `${EMOJIS.error} Invalid hex color. Use format #RRGGBB or #RGB.`, ephemeral: true });
                    return;
                }
                ...
                if (url && !this.isValidUrl(url)) {
                    await i.reply({ content: `${EMOJIS.error} Invalid URL.`, ephemeral: true });
                    return;
                }
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: 'Color', value: lv?.rankCardColor ?? '#5865F2', inline: true }, { name: 'Style', value: lv?.rankCardStyle ?? 'default', inline: true }, { name: 'Background', value: lv?.rankCardBg ? '[Custom](' + lv.rankCardBg + ')' : 'None', inline: true })
        ...
        try {
            if (!sub || sub === 'preview') {
                ...
                if (!hex || !this.isValidHex(hex)) {
                    await m.reply(`${EMOJIS.error} Invalid hex. Usage: \`p!rankcard color #RRGGBB\``);
                    return;
                }
                await prisma.leveling.upsert({ where: { userId_guildId: { userId, guildId } }, create: { userId, guildId, rankCardColor: hex }, update: { rankCardColor: hex } });
                ...
                const url = args[1];
                if (url && !this.isValidUrl(url)) {
                    await m.reply(`${EMOJIS.error} Invalid URL.`);
                    return;
                }
                await prisma.leveling.upsert({ where: { userId_guildId: { userId, guildId } }, create: { userId, guildId, rankCardBg: url ?? null }, update: { rankCardBg: url ?? null } });
                ...
                if (!style || !VALID_STYLES.includes(style)) {
                    await m.reply(`${EMOJIS.error} Valid styles: ${VALID_STYLES.join(', ')}`);
                    return;
                }
                await prisma.leveling.upsert({ where: { userId_guildId: { userId, guildId } }, create: { userId, guildId, rankCardStyle: style }, update: { rankCardStyle: style } });
