...
export class XpBlacklistCommand extends BaseCommand {
    ...
    async toggleChannel(guildId, channelId) {
        const prisma = getPrismaClient();
        const guild = await this.getGuild(guildId);
        const list = Array.isArray(guild.xpBlacklistChannels) ? guild.xpBlacklistChannels : [];
    ...
    async toggleRole(guildId, roleId) {
        const prisma = getPrismaClient();
        const guild = await this.getGuild(guildId);
        const list = Array.isArray(guild.xpBlacklistRoles) ? guild.xpBlacklistRoles : [];
    ...
    async buildListEmbed(guildId) {
        const guild = await this.getGuild(guildId);
        const channels = Array.isArray(guild.xpBlacklistChannels) ? guild.xpBlacklistChannels : [];
        const roles = Array.isArray(guild.xpBlacklistRoles) ? guild.xpBlacklistRoles : [];
        return new EmbedBuilder()
            .setTitle(`${EMOJIS.leveling} XP Blacklist`)
            .setColor(COLORS.default)
            .addFields({ name: '🚫 Blacklisted Channels', value: channels.length ? channels.map(c => `<#${c}>`).join('\n') : 'None', inline: true }, { name: '🚫 Blacklisted Roles', value: roles.length ? roles.map(r => `<@&${r}>`).join('\n') : 'None', inline: true })
            .setTimestamp();
    }
    ...
    async executePrefix(m, _args) {
        ...
        try {
            if (!sub || sub === 'list') {
                ...
                if (!ch) {
                    await m.reply(`${EMOJIS.error} Please mention a channel.`);
                    return;
                }
                ...
                if (!role) {
                    await m.reply(`${EMOJIS.error} Please mention a role.`);
                    return;
                }
