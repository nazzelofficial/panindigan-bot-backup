...
export class XpMultiplierCommand extends BaseCommand {
    ...
    async getMultipliers(guildId) {
        ...
        const raw = guild?.xpMultipliers;
        if (!raw || !Array.isArray(raw))
            return [];
        return raw;
    }
    async saveMultipliers(guildId, entries) {
        const prisma = getPrismaClient();
        await prisma.guild.upsert({
            where: { guildId },
            create: { guildId, ...{ xpMultipliers: entries } },
            update: { ...{ xpMultipliers: entries } },
        });
    }
    buildListEmbed(guildId, entries) {
    ...
    async executeSlash(i) {
        ...
        try {
            if (sub === 'list') {
                ...
                if (idx >= 0)
                    entries[idx].multiplier = multiplier;
                else
                    entries.push({ roleId: role.id, multiplier });
                ...
                if (filtered.length === entries.length) {
                    await i.reply({ content: `${EMOJIS.error} No multiplier found for that role.`, ephemeral: true });
                    return;
                }
    ...
    async executePrefix(m, _args) {
        ...
        try {
            if (!sub || sub === 'list') {
                ...
                const filtered = entries.filter(e => e.roleId !== role.id);
                if (filtered.length === entries.length) {
                    await m.reply(`${EMOJIS.error} No multiplier found for that role.`);
                    return;
                }
                await this.saveMultipliers(guildId, filtered);
                ...
                if (!role) {
                    await m.reply(`${EMOJIS.error} Usage: \`p!xpmultiplier @role <multiplier>\``);
                    return;
                }
                if (isNaN(multiplier) || multiplier < 0.1) {
                    await m.reply(`${EMOJIS.error} Provide a valid multiplier (0.1–10).`);
                    return;
                }
                const entries = await this.getMultipliers(guildId);
                const idx = entries.findIndex(e => e.roleId === role.id);
