...
export class SetLevelUpCommand extends BaseCommand {
    ...
    async handleSub(sub, guildId, opts) {
        ...
        if (sub === 'message') {
            ...
            return new EmbedBuilder()
                ...
                .setColor(COLORS.success)
                .addFields({ name: '✅ Message Set', value: msg, inline: false }, { name: '📝 Variables', value: '`{user}` — mention\n`{level}` — new level\n`{xp}` — total XP\n`{username}` — display name', inline: false })
                .setTimestamp();
        }
        if (sub === 'disable') {
            await prisma.guild.upsert({
        ...
        if (sub === 'status') {
            ...
            const chId = guild?.levelUpChannelId;
            const customMsg = guild?.levelUpMessage || 'Default: 🎉 {user} leveled up to **Level {level}**!';
            return new EmbedBuilder()
                .setTitle(`${EMOJIS.leveling} Level-Up Notification Status`)
                .setColor(COLORS.info)
                .addFields({ name: '📢 Channel', value: chId ? `<#${chId}>` : 'Not set (uses current channel)', inline: true }, { name: '✅ Enabled', value: chId ? 'Yes' : 'Using default behavior', inline: true }, { name: '💬 Message', value: customMsg, inline: false })
                .setTimestamp();
        }
