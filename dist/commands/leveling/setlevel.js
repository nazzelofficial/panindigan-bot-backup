...
export class SetLevelCommand extends BaseCommand {
    constructor() {
        super({ name: 'setlevel', description: 'Set a user\'s level directly (Admin)', category: 'leveling', premiumTier: 'free', cooldown: 3, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['setlvl'], examples: ['/setlevel @user 50', 'p!setlevel @user 50'] });
    }
    buildSlashCommand() {
    ...
    async executePrefix(m, _args) {
        ...
        if (!target || isNaN(level)) {
            await m.reply('❌ Usage: `p!setlevel @user <level>`');
            return;
        }
        const prisma = getPrismaClient();
        const xpForLevel = Math.floor(5 * Math.pow(level, 2) + 50 * level + 100);
