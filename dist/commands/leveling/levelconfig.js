...
export class LevelConfigCommand extends BaseCommand {
    constructor() {
        super({ name: 'levelconfig', description: 'Configure the leveling system', category: 'leveling', premiumTier: 'bronze', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['xpconfig', 'levelsetup', 'rankconfig'], examples: ['/levelconfig toggle', '/levelconfig channel #general', '/levelconfig multiplier 2'] });
    }
    buildSlashCommand() {
    ...
    async executeSlash(i) {
        ...
        if (sub === 'toggle') {
            ...
            const embed = new EmbedBuilder().setTitle('⚙️ Leveling Config').setColor(COLORS.default)
                .addFields({ name: 'Status', value: guild?.levelingEnabled !== false ? '✅ Enabled' : '❌ Disabled', inline: true }, { name: 'XP Multiplier', value: `${guild?.xpMultiplier || 1}x`, inline: true }, { name: 'Announce Channel', value: guild?.levelUpChannelId ? `<#${guild.levelUpChannelId}>` : 'Message channel', inline: true }, { name: 'Level-up Message', value: guild?.levelUpMessage || 'Default', inline: false });
            await i.reply({ embeds: [embed] });
        }
    }
    async executePrefix(m, _args) { await m.reply('Please use `/levelconfig` slash commands.'); }
}
