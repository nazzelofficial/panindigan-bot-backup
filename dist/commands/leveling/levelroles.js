...
export class LevelRolesCommand extends BaseCommand {
    ...
    async handleAdd(guildId, role, level) {
        ...
        if (existing) {
            await prisma.levelRole.update({ where: { id: existing.id }, data: { level } });
            return `✅ Updated level role: <@&${role.id}> → Level **${level}**`;
        }
        await prisma.levelRole.create({ data: { guildId, roleId: role.id, level } });
        return `✅ Added level role: <@&${role.id}> → Level **${level}**`;
    }
    async handleRemove(guildId, role) {
        const prisma = getPrismaClient();
    ...
    async handleList(guildId) {
        ...
        if (!roles.length)
            embed.setDescription('No level roles configured. Use `/levelroles add` to add some.');
        else
            embed.setDescription(roles.map(r => `Level **${r.level}** → <@&${r.roleId}>`).join('\n'));
        return embed;
    }
    async executeSlash(i) {
        const sub = i.options.getSubcommand();
        if (sub === 'list') {
            const e = await this.handleList(i.guildId);
        ...
        if (sub === 'add') {
            ...
            await i.reply(await this.handleAdd(i.guildId, role, level));
        }
        else if (sub === 'remove') {
            await i.reply(await this.handleRemove(i.guildId, role));
        }
    }
    async executePrefix(m, _args) {
        const [sub] = _args;
        if (!sub || sub === 'list') {
            const e = await this.handleList(m.guildId);
            await m.reply({ embeds: [e] });
            return;
        }
        const role = m.mentions.roles.first();
        if (!role) {
            await m.reply('❌ Please mention a role.');
            return;
        }
        if (sub === 'add') {
            const level = parseInt(args[2]);
            if (!level) {
                await m.reply('❌ Provide a level number.');
                return;
            }
            await m.reply(await this.handleAdd(m.guildId, role, level));
        }
        else if (sub === 'remove') {
            await m.reply(await this.handleRemove(m.guildId, role));
        }
    }
}
