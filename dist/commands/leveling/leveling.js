...
export class LevelingCommand extends BaseCommand {
    ...
    async executeSlash(i) {
        ...
        if (subcommandGroup === 'user') {
            switch (subcommand) {
                case 'rank':
                    await this.handleRank(i);
                    break;
                case 'level':
                    await this.handleLevel(i);
                    break;
                case 'xp':
                    await this.handleXp(i);
                    break;
                case 'leaderboard':
            ...
            switch (subcommand) {
                case 'toggle':
                    await this.handleConfigToggle(i);
                    break;
                case 'channel':
                    await this.handleConfigChannel(i);
                    break;
                case 'multiplier':
                    await this.handleConfigMultiplier(i);
                    break;
                case 'message':
                    await this.handleConfigMessage(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'add':
                    await this.handleRolesAdd(i);
                    break;
                case 'remove':
                    await this.handleRolesRemove(i);
                    break;
                case 'list':
                    await this.handleRolesList(i);
                    break;
                case 'sync':
                    await this.handleRolesSync(i);
                    break;
            }
        }
    }
    async handleRank(i) {
        ...
        const embed = EmbedManager.leveling('Rank Card', `**${user.tag}**`)
            ...
            .addFields({ name: '📊 Rank', value: `#${rank}`, inline: true }, { name: '⭐ Level', value: level.toString(), inline: true }, { name: '💎 XP', value: xp.toString(), inline: true }, { name: '📈 Progress', value: `${progress}% to next level`, inline: false }, { name: '🎯 Next Level', value: nextLevelXp.toString(), inline: true });
    ...
    async handleLevel(i) {
        ...
        const embed = EmbedManager.leveling('Level Information', `**${user.tag}**`)
            .addFields({ name: '⭐ Current Level', value: level.toString(), inline: true }, { name: '💎 Current XP', value: xp.toString(), inline: true }, { name: '🎯 XP to Next', value: xpToNext.toString(), inline: true }, { name: '📈 Progress', value: `${progress}%`, inline: false });
        await i.editReply({ embeds: [embed] });
    }
    async handleXp(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.leveling('XP Information', `**${user.tag}**`)
            .addFields({ name: '💎 Total XP', value: xp.toString(), inline: true }, { name: '⭐ Level', value: level.toString(), inline: true }, { name: '🎯 XP to Next Level', value: xpToNext.toString(), inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleLeaderboard(i) {
        await i.deferReply();
        if (!i.guild) {
            await i.editReply({ content: '❌ This command can only be used in a server.' });
            return;
    ...
    async handleConfigToggle(i) {
        ...
        const embed = EmbedManager.leveling('Leveling System', `Leveling system is now ${isEnabled ? 'enabled' : 'disabled'}`)
            .addFields({ name: '🖥️ Server', value: i.guild.name, inline: true }, { name: '📊 Status', value: isEnabled ? '✅ Enabled' : '❌ Disabled', inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleConfigChannel(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.leveling('Level-up Channel', channel ? 'Channel updated' : 'Channel cleared')
            .addFields({ name: '📺 Channel', value: channel ? `<#${channel.id}>` : 'None', inline: true }, { name: '🖥️ Server', value: i.guild.name, inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleConfigMultiplier(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.leveling('XP Multiplier', 'Multiplier updated')
            .addFields({ name: '✖️ Multiplier', value: `${multiplier}x`, inline: true }, { name: '🖥️ Server', value: i.guild.name, inline: true }, { name: '📊 Effect', value: `Users will earn ${multiplier}x XP`, inline: false });
        await i.editReply({ embeds: [embed] });
    }
    async handleConfigMessage(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.leveling('Level-up Message', 'Message template updated')
            .addFields({ name: '📝 Template', value: message, inline: false }, { name: '🖥️ Server', value: i.guild.name, inline: true }, { name: '💡 Placeholders', value: '{user}, {level}, {xp}, {rank}', inline: false });
        await i.editReply({ embeds: [embed] });
    }
    async handleRolesAdd(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.leveling('Level Role Added', 'Role successfully added')
            .addFields({ name: '⭐ Level', value: level.toString(), inline: true }, { name: '🎨 Role', value: `<@&${role.id}>`, inline: true }, { name: '🖥️ Server', value: i.guild.name, inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleRolesRemove(i) {
        await i.deferReply();
        const level = i.options.getInteger('level', true);
        if (!i.guild || !i.member) {
            await i.editReply({ content: '❌ This command can only be used in a server.' });
        ...
        const embed = EmbedManager.leveling('Level Role Removed', 'Role successfully removed')
            .addFields({ name: '⭐ Level', value: level.toString(), inline: true }, { name: '🖥️ Server', value: i.guild.name, inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleRolesList(i) {
        await i.deferReply();
        if (!i.guild) {
            await i.editReply({ content: '❌ This command can only be used in a server.' });
            return;
    ...
    async handleRolesSync(i) {
        ...
        const embed = EmbedManager.leveling('Role Sync Complete', 'All member roles have been synced')
            .addFields({ name: '✅ Synced', value: syncedCount.toString(), inline: true }, { name: '❌ Failed', value: failedCount.toString(), inline: true }, { name: '🖥️ Server', value: i.guild.name, inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleStats(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.leveling('Leveling Statistics', `**${i.guild.name}**`)
            .addFields({ name: '💎 Total XP', value: totalXp.toString(), inline: true }, { name: '⭐ Total Levels', value: totalLevels.toString(), inline: true }, { name: '👥 Members', value: i.guild.memberCount.toString(), inline: true }, { name: '🟢 Active Users', value: activeUsers.toString(), inline: true }, { name: '📊 Average Level', value: averageLevel.toFixed(2), inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleReset(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.leveling('XP Reset', 'User XP has been reset')
            .addFields({ name: '👤 User', value: `<@${user.id}>`, inline: true }, { name: '💎 New XP', value: '0', inline: true }, { name: '⭐ New Level', value: '1', inline: true }, { name: '🖥️ Server', value: i.guild.name, inline: true });
        await i.editReply({ embeds: [embed] });
    }
