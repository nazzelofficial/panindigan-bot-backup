...
export class OcrCommand extends BaseCommand {
    constructor() {
        super({ name: 'ocr', description: 'Extract text from an image using OCR (Gold+)', category: 'utility', premiumTier: 'gold', cooldown: 10, guildOnly: false, ownerOnly: false, slashCommand: true, prefixCommand: true, aliases: ['readimage', 'extracttext'], examples: ['/ocr [attach an image]', 'p!ocr [attach an image]'] });
    }
    async extractText(imageUrl) {
        ...
        if (!parsedResults?.length)
            return null;
        return parsedResults.map((r) => r.ParsedText?.trim()).filter(Boolean).join('\n') || null;
    }
    async run(i, m) {
