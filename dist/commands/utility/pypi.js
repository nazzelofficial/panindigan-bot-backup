...
export class PypiCommand extends BaseCommand {
    constructor() {
        super({ name: 'pypi', description: 'Look up a Python package on PyPI', category: 'utility', premiumTier: 'free', cooldown: 5, guildOnly: false, ownerOnly: false, slashCommand: true, prefixCommand: true, aliases: ['pip', 'pypkg'], examples: ['/pypi requests', 'p!pypi numpy'] });
    }
    async run(i, m, pkg) {
        ...
        try {
            ...
            if (!resp.ok)
                return send(new EmbedBuilder().setColor(COLORS.error).setDescription(`❌ Package \`${pkg}\` not found on PyPI.`));
            ...
            const embed = new EmbedBuilder().setColor(0x3776AB).setTitle(`🐍 PyPI: ${info.name}`)
                ...
                .addFields({ name: '📌 Version', value: info.version ?? 'N/A', inline: true }, { name: '📜 License', value: info.license ?? 'N/A', inline: true }, { name: '👤 Author', value: info.author ?? 'N/A', inline: true }, { name: '🐍 Python', value: info.requires_python ?? 'N/A', inline: true }, { name: '📦 Install', value: `\`pip install ${info.name}\``, inline: false });
            await send(embed);
        }
        catch (err) {
