...
export class TagCommand extends BaseCommand {
    constructor() {
        super({ name: 'tag', description: 'Custom server tags — create, show, list, delete', category: 'utility', premiumTier: 'diamond', cooldown: 3, guildOnly: true, ownerOnly: false, slashCommand: true, prefixCommand: true, aliases: ['tags', 'snippet'], examples: ['/tag show rules', 'p!tag create rules Read the rules!', 'p!tag list'] });
    }
    async run(i, m, sub, name, content) {
        ...
        if (sub === 'show' || sub === 'get') {
            if (!name)
                return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Provide a tag name.'));
            const tag = await col.findOne({ guildId, name: name.toLowerCase() });
            if (!tag)
                return send(new EmbedBuilder().setColor(COLORS.error).setDescription(`❌ Tag \`${name}\` not found.`));
            ...
            if (!name || !content)
                return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Provide both a name and content.'));
            const exists = await col.findOne({ guildId, name: name.toLowerCase() });
            if (exists)
                return send(new EmbedBuilder().setColor(COLORS.error).setDescription(`❌ Tag \`${name}\` already exists. Delete it first.`));
