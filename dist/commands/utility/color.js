...
export class ColorCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addStringOption(opt => opt.setName('hex').setDescription('Hex color code (e.g. #5865F2 or 5865F2)').setRequired(true));
    }
    parseHex(input) {
    ...
    buildEmbed(hex, r, g, b, decimal) {
        return new EmbedBuilder()
            .setTitle(`${EMOJIS.utility} Color Preview`)
            .setColor(decimal)
            .addFields({ name: 'Hex', value: `\`${hex}\``, inline: true }, { name: 'RGB', value: `\`rgb(${r}, ${g}, ${b})\``, inline: true }, { name: 'Decimal', value: `\`${decimal}\``, inline: true })
