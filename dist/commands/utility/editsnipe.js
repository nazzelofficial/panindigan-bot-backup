    ...
    async getEditSnipe(channelId) {
        try {
            ...
            return new EmbedBuilder()
                ...
                .addFields({ name: 'Before', value: (data.before ?? 'N/A').slice(0, 1024), inline: false }, { name: 'After', value: (data.after ?? 'N/A').slice(0, 1024), inline: false }, { name: 'Author', value: data.author ?? 'Unknown', inline: true }, { name: 'Edited At', value: `<t:${Math.floor(editedAt.getTime() / 1000)}:R>`, inline: true })
                .setTimestamp();
        }
        catch (err) {
