...
export class UtilityCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return (new SlashCommandBuilder()
            ...
            .addSubcommand(s => s.setName('clear').setDescription('Clear all notes'))));
    }
    async executeSlash(i) {
        ...
        if (subcommandGroup === 'personal') {
            switch (subcommand) {
                case 'afk':
                    await this.handleAfk(i);
                    break;
                case 'birthday':
                    await this.handleBirthday(i);
                    break;
                case 'reminder':
                    await this.handleReminder(i);
                    break;
                case 'ticket':
                    await this.handleTicket(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'google':
                    await this.handleSearchGoogle(i);
                    break;
                case 'youtube':
                    await this.handleSearchYoutube(i);
                    break;
                case 'urban':
                    await this.handleSearchUrban(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'calc':
                    await this.handleToolsCalc(i);
                    break;
                case 'charcount':
                    await this.handleToolsCharCount(i);
                    break;
                case 'color':
                    await this.handleToolsColor(i);
                    break;
                case 'currency':
                    await this.handleToolsCurrency(i);
                    break;
                case 'qr':
                    await this.handleToolsQr(i);
                    break;
                case 'timestamp':
                    await this.handleToolsTimestamp(i);
                    break;
                case 'shorten':
                    await this.handleToolsShorten(i);
                    break;
            }
        }
            ...
            switch (subcommand) {
                case 'add':
                    await this.handleNotesAdd(i);
                    break;
                case 'list':
                    await this.handleNotesList(i);
                    break;
                case 'remove':
                    await this.handleNotesRemove(i);
                    break;
                case 'clear':
                    await this.handleNotesClear(i);
                    break;
            }
        }
    }
    async handleAfk(i) {
        ...
        const embed = EmbedManager.info('AFK Status', 'You are now AFK')
            .addFields({ name: '👤 User', value: i.user.tag, inline: true }, { name: '💭 Reason', value: reason, inline: false }, { name: '⏰ Set at', value: `<t:${Math.floor(Date.now() / 1000)}:R>`, inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleBirthday(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.info('Birthday Set', 'Your birthday has been saved')
            .addFields({ name: '🎂 Date', value: date, inline: true }, { name: '👤 User', value: i.user.tag, inline: true }, { name: '📅 Next Birthday', value: `<t:${Math.floor(nextBirthday.getTime() / 1000)}:R>`, inline: true }, { name: '⏰ Days Until', value: daysUntil.toString(), inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleReminder(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.info('Reminder Set', 'Your reminder has been scheduled')
            .addFields({ name: '💭 Message', value: message, inline: false }, { name: '⏰ Time', value: time, inline: true }, { name: '📅 Remind At', value: `<t:${Math.floor(reminderTime / 1000)}:R>`, inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleTicket(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.info('Ticket Created', 'Your support ticket has been created')
            .addFields({ name: '🎫 Ticket ID', value: ticketId, inline: true }, { name: '👤 User', value: i.user.tag, inline: true }, { name: '💭 Reason', value: reason, inline: false }, { name: '🖥️ Server', value: i.guild.name, inline: true })
            .setFooter({ text: 'Support staff will be with you shortly' });
        await i.editReply({ embeds: [embed] });
    }
    async handleSearchGoogle(i) {
        await i.deferReply();
        const query = i.options.getString('query', true);
        const googleUrl = `https://www.google.com/search?q=${encodeURIComponent(query)}`;
        const embed = EmbedManager.info('Google Search', 'Search results')
            .addFields({ name: '🔍 Query', value: query, inline: false }, { name: '🔗 Link', value: `[Click to search](${googleUrl})`, inline: false });
        await i.editReply({ embeds: [embed] });
    }
    async handleSearchYoutube(i) {
        await i.deferReply();
        const query = i.options.getString('query', true);
        const youtubeUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(query)}`;
        const embed = EmbedManager.info('YouTube Search', 'Search results')
            .addFields({ name: '🔍 Query', value: query, inline: false }, { name: '🔗 Link', value: `[Click to search](${youtubeUrl})`, inline: false });
        await i.editReply({ embeds: [embed] });
    }
    async handleSearchUrban(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.info('Urban Dictionary', 'Definition')
            .addFields({ name: '📖 Term', value: term, inline: false }, { name: '🔗 Link', value: `[View on Urban Dictionary](${urbanUrl})`, inline: false })
            .setFooter({ text: 'API integration coming soon' });
        await i.editReply({ embeds: [embed] });
    }
    async handleToolsCalc(i) {
        await i.deferReply();
        ...
        try {
            ...
            const embed = EmbedManager.info('Calculator', 'Calculation result')
                .addFields({ name: '📝 Expression', value: expr, inline: false }, { name: '➡️ Result', value: result.toString(), inline: true });
            await i.editReply({ embeds: [embed] });
        }
        catch (error) {
    ...
    async handleToolsCharCount(i) {
        ...
        const embed = EmbedManager.info('Character Count', 'Text statistics')
            .addFields({ name: '📝 Characters', value: charCount.toString(), inline: true }, { name: '📖 Words', value: wordCount.toString(), inline: true }, { name: '📄 Lines', value: lineCount.toString(), inline: true }, { name: '⌨️ Spaces', value: spaceCount.toString(), inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleToolsColor(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.info('Color Information', fullHex)
            ...
            .addFields({ name: '🎨 HEX', value: fullHex, inline: true }, { name: '🖼️ RGB', value: rgb, inline: true }, { name: '🌈 HSL', value: hsl, inline: true }, { name: '🔢 Decimal', value: parseInt(hexColor, 16).toString(), inline: true });
        await i.editReply({ embeds: [embed] });
    }
    rgbToHsl(r, g, b) {
        r /= 255;
    ...
    async handleToolsCurrency(i) {
        ...
        const embed = EmbedManager.info('Currency Conversion', 'Conversion result')
            .addFields({ name: '💵 Amount', value: amount.toString(), inline: true }, { name: '🔄 From', value: from, inline: true }, { name: '➡️ To', value: to, inline: true }, { name: '📊 Result', value: `${converted.toFixed(2)} ${to}`, inline: false })
            .setFooter({ text: 'Exchange rates are approximate. API integration coming soon.' });
        await i.editReply({ embeds: [embed] });
    }
    async handleToolsQr(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.info('QR Code', 'Scan to view')
            .setImage(qrApiUrl)
            .addFields({ name: '🔗 URL/Text', value: url.length > 100 ? url.substring(0, 100) + '...' : url, inline: false });
        await i.editReply({ embeds: [embed] });
    }
    async handleToolsTimestamp(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.info('Timestamp', 'Date information')
            .addFields({ name: '⏰ Unix', value: unix.toString(), inline: true }, { name: '📅 ISO', value: iso, inline: false }, { name: '🌍 Local', value: relative, inline: false }, { name: '💬 Discord Format', value: `<t:${unix}:R>`, inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleToolsShorten(i) {
        await i.deferReply();
        const url = i.options.getString('url', true);
        // Validate URL
        ...
        const embed = EmbedManager.info('URL Shortened', 'Your shortened URL')
            .addFields({ name: '🔗 Original', value: url.length > 50 ? url.substring(0, 50) + '...' : url, inline: false }, { name: '✂️ Shortened', value: shortUrl, inline: false })
            .setFooter({ text: 'API integration coming soon - this is a demo' });
        await i.editReply({ embeds: [embed] });
    }
    async handleNotesAdd(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.info('Note Added', 'Your note has been saved')
            .addFields({ name: '📝 ID', value: noteId.toString(), inline: true }, { name: '👤 User', value: i.user.tag, inline: true }, { name: '📄 Content', value: content.length > 100 ? content.substring(0, 100) + '...' : content, inline: false }, { name: '⏰ Created', value: `<t:${Math.floor(Date.now() / 1000)}:R>`, inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleNotesList(i) {
        await i.deferReply();
    ...
    async handleNotesRemove(i) {
        ...
        const embed = EmbedManager.info('Note Removed', 'Your note has been deleted')
            .addFields({ name: '📝 Note ID', value: id.toString(), inline: true }, { name: '👤 User', value: i.user.tag, inline: true });
        await i.editReply({ embeds: [embed] });
    }
    async handleNotesClear(i) {
        await i.deferReply();
        ...
        const embed = EmbedManager.info('Notes Cleared', 'All your notes have been deleted')
            .addFields({ name: '👤 User', value: i.user.tag, inline: true }, { name: '⏰ Cleared', value: `<t:${Math.floor(Date.now() / 1000)}:R>`, inline: true });
        await i.editReply({ embeds: [embed] });
    }
