...
const SCREENSHOT_SERVICES = [
    (url) => `https://api.screenshotmachine.com/?url=${encodeURIComponent(url)}&dimension=1280x720&format=jpg&cacheLimit=0`,
    (url) => `https://s.wordpress.com/mshots/v1/${encodeURIComponent(url)}?w=1280&h=720`,
    (url) => `https://image.thum.io/get/width/1280/crop/720/${url}`,
];
export class ScreenshotCommand extends BaseCommand {
    constructor() {
        super({ name: 'screenshot', description: 'Capture a screenshot of a website (Gold+) 📸', category: 'utility', premiumTier: 'gold', cooldown: 15, guildOnly: false, ownerOnly: false, slashCommand: true, prefixCommand: true, aliases: ['ss', 'snap', 'capture'], examples: ['/screenshot https://discord.com', 'p!screenshot https://google.com'] });
    }
    buildSlashCommand() {
        return (new SlashCommandBuilder().setName(this.name).setDescription(this.description)
            ...
            .addChoices({ name: '1280×720 (HD)', value: '1280x720' }, { name: '1920×1080 (Full HD)', value: '1920x1080' }, { name: '375×812 (Mobile)', value: '375x812' }))
    ...
    async run(i, m, url, size) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: '🌐 URL', value: `[${url}](${url})`, inline: false }, { name: '📐 Size', value: `${width}×${height}`, inline: true }, { name: '⚡ Service', value: usedService, inline: true })
    ...
    async executePrefix(m, _args) {
        if (!args[0]) {
            await m.reply('❌ Usage: `p!screenshot <url>`');
            return;
        }
