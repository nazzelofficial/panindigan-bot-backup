...
export class AliasCommand extends BaseCommand {
    constructor() {
        super({ name: 'alias', description: 'Custom command aliases — create, list, delete', category: 'utility', premiumTier: 'diamond', cooldown: 3, guildOnly: true, ownerOnly: false, slashCommand: true, prefixCommand: true, aliases: ['customalias'], examples: ['p!alias create pl play', 'p!alias list'] });
    }
    async run(i, m, sub, aliasName, target) {
        ...
        if (sub === 'create' || sub === 'add') {
            if (!aliasName || !target)
                return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Usage: `alias create <alias> <target_command>`'));
            const count = await col.countDocuments({ guildId });
            if (count >= 30)
                return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Max 30 aliases per server.'));
    ...
    async executePrefix(m, _args) { await this.run(null, m, args[0] ?? 'list', args[1] ?? '', args[2] ?? ''); }
}
