...
export class PdfCommand extends BaseCommand {
    constructor() {
        super({ name: 'pdf', description: 'Extract text and info from a PDF file (Gold+) 📄', category: 'utility', premiumTier: 'gold', cooldown: 15, guildOnly: false, ownerOnly: false, slashCommand: true, prefixCommand: true, aliases: ['pdfinfo', 'pdftext', 'readpdf'], examples: ['/pdf [attach a PDF]', 'p!pdf [attach a PDF]'] });
    }
    buildSlashCommand() {
        return (new SlashCommandBuilder().setName(this.name).setDescription(this.description)
            ...
            .addChoices({ name: 'Extract text', value: 'text' }, { name: 'File info only', value: 'info' }))
    ...
    async run(i, m, mode) {
        ...
        if (!attachment)
            return send({ content: '❌ Please attach a PDF file.' });
        if (!attachment.name?.toLowerCase().endsWith('.pdf'))
            return send({ content: '❌ Please attach a `.pdf` file.' });
        if (attachment.size > 10 * 1024 * 1024)
            return send({ content: '❌ PDF must be under 10MB.' });
        if (i && !i.deferred)
            await i.deferReply();
        ...
        if (resolvedMode === 'info') {
            embed.addFields({ name: '📁 Filename', value: attachment.name, inline: true }, { name: '📦 File Size', value: `${(attachment.size / 1024).toFixed(1)} KB`, inline: true }, { name: '🔗 URL', value: `[Download](${attachment.url})`, inline: true });
        ...
        try {
            ...
            if (!result) {
                ...
                embed.setColor(COLORS.warning)
                    ...
                    .addFields({ name: '📁 Filename', value: attachment.name, inline: true }, { name: '📦 File Size', value: `${(attachment.size / 1024).toFixed(1)} KB`, inline: true }, { name: '🔗 URL', value: `[Download](${attachment.url})`, inline: true });
                return send({ embeds: [embed] });
            }
            ...
            embed.setColor(COLORS.success)
                ...
                .addFields({ name: '📁 Filename', value: attachment.name, inline: true }, { name: '📄 Pages', value: `${numPages}`, inline: true }, { name: '📝 Characters', value: `${cleanText.length.toLocaleString()}`, inline: true }, { name: '📦 File Size', value: `${(attachment.size / 1024).toFixed(1)} KB`, inline: true });
