...
export class BulkroleCommand extends BaseCommand {
    ...
    async run(i, m, action, roleId, targets) {
        ...
        if (!action || !roleId)
            return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Usage: `bulkrole add/remove <@role> <@user...> or all`'));
        const role = guild.roles.cache.get(roleId.replace(/[<@&>]/g, ''));
        if (!role)
            return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Role not found.'));
        if (role.position >= (guild.members.me?.roles.highest.position ?? 0))
            return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ I cannot manage that role (too high in hierarchy).'));
        const isAll = targets.includes('all');
        await guild.members.fetch();
        const members = isAll ? [...guild.members.cache.filter(m => !m.user.bot).values()] : targets.map(t => guild.members.cache.get(t.replace(/[<@!>]/g, ''))).filter(Boolean);
        ...
        if (i)
            await i.followUp({ embeds: [result], flags: 64 });
        else
            await m.channel.send({ embeds: [result] });
    }
    async executeSlash(i) { await this.run(i, null, i.options.getString('action', true), i.options.getRole('role', true).id, []); }
    async executePrefix(m, _args) { await this.run(null, m, args[0], args[1], _args.slice(2)); }
}
