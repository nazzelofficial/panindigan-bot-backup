...
export class NotesCommand extends BaseCommand {
    ...
    async run(i, m, sub, content) {
        ...
        if (sub === 'create' || sub === 'add') {
            if (!content)
                return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Provide note content.'));
            const count = await col.countDocuments({ userId });
            if (count >= 25)
                return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ You have reached the maximum of 25 notes.'));
            ...
            if (!notes.length)
                return send(new EmbedBuilder().setColor(COLORS.default).setDescription('📝 You have no notes. Use `/notes create <text>` to add one.'));
            ...
            if (isNaN(idx) || !notes[idx])
                return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Invalid note number. Use `/notes list` to see your notes.'));
    ...
    async executePrefix(m, _args) { await this.run(null, m, args[0] ?? 'list', _args.slice(1).join(' ')); }
}
