...
export class BirthdayCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addSubcommand(sub => sub.setName('list').setDescription('List upcoming birthdays in this server'))
            .addSubcommand(sub => sub.setName('remove').setDescription('Remove your birthday from the bot'))
            .setDMPermission(false);
    }
    async handleSet(userId, guildId, dateStr, reply) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: '📅 Birthday', value: formatBirthday(parsed), inline: true }, {
            name: '⏳ Next Birthday',
    ...
    async handleView(targetUserId, targetUsername, guildId, reply) {
        const prisma = getPrismaClient();
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: '📅 Birthday', value: formatBirthday(user.birthday), inline: true }, {
            name: '⏳ Next Birthday',
    ...
    async handleList(guildId, reply) {
        const prisma = getPrismaClient();
    ...
    async handleRemove(userId, guildId, reply) {
        const prisma = getPrismaClient();
    ...
    async executePrefix(message, _args) {
        ...
        if (sub === 'set') {
            ...
            if (!dateStr) {
                return void message.reply(`${EMOJIS.error} Usage: \`p!birthday set YYYY-MM-DD\` (e.g. \`p!birthday set 1999-05-20\`)`);
            }
