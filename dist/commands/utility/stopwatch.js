...
export class StopwatchCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addChoices({ name: 'start', value: 'start' }, { name: 'stop', value: 'stop' }, { name: 'lap', value: 'lap' }));
    }
    formatElapsed(ms) {
    ...
    async handle(userId, action) {
        ...
        if (action === 'lap') {
            ...
            const lapData = JSON.parse(data.lapData ?? '[]');
        ...
        if (action === 'stop') {
            ...
            return new EmbedBuilder()
                ...
                .addFields({ name: 'Total Laps', value: `${data.laps ?? '0'}`, inline: true }, { name: 'Lap Times', value: lapData.length > 0 ? lapData.map((l, i) => `Lap ${i + 1}: \`${l}\``).join('\n').slice(0, 1024) : 'None', inline: false })
                .setTimestamp();
        }
