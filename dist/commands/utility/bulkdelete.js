...
export class BulkdeleteCommand extends BaseCommand {
    ...
    async run(i, m, userId, count) {
        ...
        if (!userId)
            return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Mention a user.'));
        ...
        if (!toDelete.length)
            return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ No recent messages found from that user.'));
    ...
    async executeSlash(i) { await this.run(i, null, i.options.getUser('user', true).id, i.options.getInteger('count') ?? 50); }
    async executePrefix(m, _args) { await this.run(null, m, args[0]?.replace(/[<@!>]/g, ''), parseInt(args[1]) || 50); }
}
