...
export class TicketCommand extends BaseCommand {
    ...
    async handleCreate(username, channel, userId) {
        try {
    ...
    async handleClaim(thread, claimerTag) {
        try {
    ...
    async executeSlash(interaction) {
        ...
        if (sub === 'create') {
            ...
            if (!channel?.isThread()) {
                ...
                embed = await this.handleClose(channel);
            }
        }
            ...
            if (!channel?.isThread()) {
                ...
                embed = await this.handleTranscript(channel);
            }
        }
        else if (sub === 'claim') {
            if (!channel?.isThread()) {
                ...
                embed = await this.handleClaim(channel, interaction.user.tag);
            }
        }
        else if (sub === 'reopen') {
            if (!channel?.isThread()) {
                ...
                embed = await this.handleReopen(channel);
            }
        }
        else {
    ...
    async executePrefix(message, _args) {
        ...
        if (sub === 'create') {
            ...
            if (!channel.isThread()) {
                ...
                embed = await this.handleClaim(channel, message.author.tag);
            }
        }
        else if (sub === 'reopen') {
            if (!channel.isThread()) {
