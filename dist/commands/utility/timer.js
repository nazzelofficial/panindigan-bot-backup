...
export class TimerCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addStringOption(opt => opt.setName('duration').setDescription('Duration (e.g. 30s, 5m, 1h)').setRequired(true))
            .addStringOption(opt => opt.setName('label').setDescription('Optional label for this timer').setRequired(false));
    }
    parseDuration(input) {
    ...
    async startTimer(channelId, userId, durationMs, label, sendMessage) {
        ...
        if (durationMs > 600000)
            return; // Don't use setTimeout for > 10 min to avoid memory issues
        setTimeout(async () => {
            try {
                const redis = await getRedisClient();
            ...
            if (durationMs <= 600000) {
                setTimeout(async () => {
                    const doneEmbed = new EmbedBuilder()
                        ...
                        .setTimestamp();
                    await m.channel.send({ embeds: [doneEmbed] }).catch(() => { });
                }, durationMs);
            }
        }
        catch (err) {
