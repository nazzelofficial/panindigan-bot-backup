...
export class MacroCommand extends BaseCommand {
    ...
    async run(i, m, sub, name, commands) {
        ...
        if (sub === 'create' || sub === 'add') {
            if (!name || !commands)
                return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Usage: `macro create <name> <cmd1 | cmd2 | ...>`'));
            ...
            if (!macro)
                return send(new EmbedBuilder().setColor(COLORS.error).setDescription(`❌ Macro \`${name}\` not found.`));
            await send(new EmbedBuilder().setColor(COLORS.default).setTitle(`▶️ Running Macro: ${name}`)
                .setDescription(`**Steps:**\n${macro.steps.map((s, idx) => `${idx + 1}. \`${s}\``).join('\n')}`));
        }
        else {
