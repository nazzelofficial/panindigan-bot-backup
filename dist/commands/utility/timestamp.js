...
export class TimestampCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addStringOption(opt => opt.setName('date').setDescription('Date string or unix timestamp (optional, defaults to now)').setRequired(false));
    }
    buildEmbed(unix) {
        return new EmbedBuilder()
            ...
            .addFields({ name: 'Short Time `<t:unix:t>`', value: `<t:${unix}:t>`, inline: true }, { name: 'Long Time `<t:unix:T>`', value: `<t:${unix}:T>`, inline: true }, { name: 'Short Date `<t:unix:d>`', value: `<t:${unix}:d>`, inline: true }, { name: 'Long Date `<t:unix:D>`', value: `<t:${unix}:D>`, inline: true }, { name: 'Short Date/Time `<t:unix:f>`', value: `<t:${unix}:f>`, inline: true }, { name: 'Long Date/Time `<t:unix:F>`', value: `<t:${unix}:F>`, inline: true }, { name: 'Relative `<t:unix:R>`', value: `<t:${unix}:R>`, inline: true })
            .setTimestamp();
    }
    parseInput(input) {
        if (!input)
