...
export class CoupleNicknameCommand extends BaseCommand {
    constructor() {
        super({ name: 'couplenickname', description: 'Set a shared premium couple nickname 💕', category: 'social', premiumTier: 'gold', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['couplenicke', 'setcouplenick', 'sharedname'], examples: ['/couplenickname ForeverUs', 'p!couplenickname The Dream Team'] });
    }
    buildSlashCommand() {
    ...
    async handle(sub, userId, guildId, nickname, send) {
        ...
        if (sub === 'set') {
            await prisma.couple?.update?.({ where: { id: profile.id }, data: { sharedNickname: nickname } }).catch(() => null);
            ...
            const currentNick = profile.sharedNickname;
            const embed = new EmbedBuilder()
            ...
            await prisma.couple?.update?.({ where: { id: profile.id }, data: { sharedNickname: null } }).catch(() => null);
