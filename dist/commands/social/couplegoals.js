...
export class CoupleGoalsCommand extends BaseCommand {
    constructor() {
        super({ name: 'couplegoals', description: 'Set and track couple goals with your partner 🎯', category: 'social', premiumTier: 'gold', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['goals', 'setgoal'], examples: ['/couplegoals set Visit Japan together', '/couplegoals list', 'p!couplegoals set Learn a new language'] });
    }
    buildSlashCommand() {
    ...
    async handle(sub, userId, guildId, goalText, goalNum, send) {
        const profile = await coupleProfileService.getProfile(userId, guildId);
        if (!profile) {
            await send({ content: '❌ Wala kang kasalukuyang couple status. Gumamit ng `/marry @user` para mag-propose!', ephemeral: true });
            return;
        }
        const prisma = getPrismaClient();
        const coupleData = await prisma.coupleGoal?.findMany?.({ where: { coupleId: profile.id } }).catch(() => null) || [];
        if (sub === 'set') {
            await prisma.coupleGoal?.create?.({ data: { coupleId: profile.id, goal: goalText, completed: false, createdBy: userId } }).catch(() => null);
            ...
            if (!coupleData.length) {
                ...
                const lines = coupleData.map((g, i) => `${i + 1}. ${g.completed ? '✅' : '⬜'} ${g.goal}`);
                embed.setDescription(lines.join('\n'));
                const done = coupleData.filter((g) => g.completed).length;
            ...
            if (!goal) {
                await send({ content: '❌ Goal not found.', ephemeral: true });
                return;
            }
            await prisma.coupleGoal?.update?.({ where: { id: goal.id }, data: { completed: true } }).catch(() => null);
            ...
            await prisma.coupleGoal?.delete?.({ where: { id: goal.id } }).catch(() => null);
