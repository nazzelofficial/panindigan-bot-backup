...
export class CoupleCommand extends BaseCommand {
    constructor() {
        super({ name: 'couple', description: 'View your couple profile 💑', category: 'social', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['couplestatus'], examples: ['/couple', 'p!couple'] });
    }
    async executeSlash(i) {
        const profile = await coupleProfileService.getProfile(i.user.id, i.guildId);
        if (!profile) {
            await i.reply({ content: '💔 You are not in a couple. Use `/marry @user` to propose!', ephemeral: true });
            return;
        }
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: '💕 Partners', value: `${i.user} & ${partner || `<@${partnerId}>`}`, inline: false }, { name: '📅 Together since', value: `<t:${Math.floor(new Date(profile.marriedAt).getTime() / 1000)}:D>`, inline: true }, { name: '⏰ Days together', value: `${days} day${days !== 1 ? 's' : ''}`, inline: true }, { name: '💬 Interactions', value: `${profile.interactions}`, inline: true }).setTimestamp();
        await i.reply({ embeds: [embed] });
    }
    async executePrefix(m, _args) {
        const profile = await coupleProfileService.getProfile(m.author.id, m.guildId);
        if (!profile) {
            await m.reply('💔 You are not in a couple. Use `p!marry @user` to propose!');
            return;
        }
