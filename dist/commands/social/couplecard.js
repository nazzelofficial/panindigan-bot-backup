...
export class CouplecardCommand extends BaseCommand {
    constructor() {
        super({ name: 'couplecard', description: 'Generate a premium couple image card with avatars and details 💑', category: 'social', premiumTier: 'gold', cooldown: 10, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['lovecard', 'couplepic'], examples: ['/couplecard', 'p!couplecard'] });
    }
    buildSlashCommand() {
    ...
    async handle(userId, guildId, send, client) {
        ...
        if (!profile) {
            await send({ content: '❌ You are not in a couple! Use `/marry @user` to start.', ephemeral: true });
            return;
        }
        ...
        try {
            ...
            await send({ content: '❌ Could not fetch user data.', ephemeral: true });
            return;
        }
        ...
        try {
            ...
            if (av1) {
                ctx.save();
                ctx.beginPath();
                ctx.arc(90, H / 2, aSize / 2, 0, Math.PI * 2);
                ctx.clip();
                ctx.drawImage(av1, 90 - aSize / 2, H / 2 - aSize / 2, aSize, aSize);
                ctx.restore();
            }
            if (av2) {
                ctx.save();
                ctx.beginPath();
                ctx.arc(W - 90, H / 2, aSize / 2, 0, Math.PI * 2);
                ctx.clip();
                ctx.drawImage(av2, W - 90 - aSize / 2, H / 2 - aSize / 2, aSize, aSize);
                ctx.restore();
            }
            // Center text
            ctx.fillStyle = '#fff';
            ...
            if (profile.sharedNickname) {
                ctx.font = 'italic 14px Arial';
                ctx.fillStyle = '#ffd6e7';
                ctx.fillText(`"${profile.sharedNickname}"`, W / 2, H / 2 + 62);
            }
            // Names
            ctx.fillStyle = '#fff';
