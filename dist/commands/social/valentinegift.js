...
export class ValentineGiftCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return (new SlashCommandBuilder()
            ...
            .addUserOption(o => o.setName('user').setDescription('Who to send the gift to').setRequired(true))
            .addStringOption(o => o.setName('gift')
            .setDescription('Choose a gift to send')
            .setRequired(true)
            .addChoices({ name: '🌹 Roses', value: 'roses' }, { name: '🍫 Chocolates', value: 'chocolates' }, { name: '🧸 Teddy Bear', value: 'teddy bear' }, { name: '💍 Ring', value: 'ring' }, { name: '⭐ Stars', value: 'stars' }))
    ...
    async executeSlash(i) {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`${i.user.toString()} sends ${target.toString()} a gift! 🎁\n\n` +
    ...
    async executePrefix(m, _args) {
        try {
            ...
            const embed = new EmbedBuilder()
                .setTitle(`${gift.emoji} A Valentine's Gift!`)
                .setDescription(`${m.author.toString()} sends ${target.toString()} a gift! 🎁\n\n` +
                `**${gift.emoji} ${giftKey.charAt(0).toUpperCase() + giftKey.slice(1)}**\n` +
                `*${gift.description}*\n\n` +
                `💌 *With love and warm wishes, from the heart* 💖`)
                .setColor(COLORS.default)
