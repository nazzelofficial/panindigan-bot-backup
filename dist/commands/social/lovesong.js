...
export class LovesongCommand extends BaseCommand {
    constructor() {
        super({ name: 'lovesong', description: 'Generate an AI-personalized love song for your partner 🎵', category: 'social', premiumTier: 'gold', cooldown: 30, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['songforpartner', 'aisong'], examples: ['/lovesong', 'p!lovesong'] });
    }
    buildSlashCommand() {
    ...
    async handle(userId, guildId, theme, send, client) {
        const profile = await coupleProfileService.getProfile(userId, guildId);
        if (!profile) {
            await send({ content: '❌ Wala kang kasalukuyang couple status. Use `/marry @user` first!', ephemeral: true });
            return;
        }
        ...
        try {
            const u = await client.users.fetch(partnerId);
    ...
    async executePrefix(m, _args) {
        ...
        await this.handle(m.author.id, m.guildId, _args.join(' ') || null, async (c) => { await thinking.edit(c.content || ''); if (c.embeds)
            await thinking.edit({ content: '', ...c }); }, m.client);
    }
}
