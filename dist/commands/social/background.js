...
export class BackgroundCommand extends BaseCommand {
    constructor() {
        super({ name: 'background', description: 'Set your profile background image (Diamond) 🎨', category: 'social', premiumTier: 'diamond', cooldown: 5, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['setbg', 'profilebg', 'bg'], examples: ['/background galaxy', '/background custom https://...', 'p!background sunset'] });
    }
    buildSlashCommand() {
        return (new SlashCommandBuilder().setName(this.name).setDescription(this.description)
            ...
            .addStringOption(o => o.setName('url').setDescription('Custom image URL (if theme is custom)').setRequired(false)));
    }
    async handle(userId, theme, customUrl, send) {
        const prisma = getPrismaClient();
        let bgValue = theme;
        if (theme === 'custom') {
            if (!customUrl) {
                await send({ content: '❌ Provide a URL for custom background.', ephemeral: true });
                return;
            }
            try {
                new URL(customUrl);
                ...
                await send({ content: '❌ Invalid URL.', ephemeral: true });
                return;
            }
            bgValue = customUrl;
        }
        await prisma.user.upsert({
            where: { userId },
            create: { userId, profileBackground: bgValue },
            update: { profileBackground: bgValue },
        }).catch(() => null);
