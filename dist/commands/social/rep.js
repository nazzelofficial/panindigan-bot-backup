...
export class RepCommand extends BaseCommand {
    constructor() {
        super({ name: 'rep', description: 'Give reputation points to a user ⭐', category: 'social', premiumTier: 'silver', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['reputation', 'vouch'], examples: ['/rep @user', 'p!rep @user'] });
    }
    buildSlashCommand() {
        return (new SlashCommandBuilder().setName(this.name).setDescription(this.description).addUserOption(o => o.setName('user').setDescription('Who to rep').setRequired(true)).setDMPermission(false));
    }
    async giveRep(giverId, targetId, guildId) {
        if (giverId === targetId)
            return { success: false, error: 'You cannot rep yourself!' };
    ...
    async executePrefix(m, _args) {
        ...
        if (!t) {
            await m.reply('❌ Mention who to give rep!');
            return;
        }
