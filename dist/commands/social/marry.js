...
export class MarryCommand extends BaseCommand {
    constructor() {
        super({ name: 'marry', description: 'Propose marriage to another member! 💍', category: 'social', premiumTier: 'free', cooldown: 10, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['propose', 'couplerequest'], examples: ['/marry @user', 'p!marry @user'] });
    }
    buildSlashCommand() {
        return (new SlashCommandBuilder().setName(this.name).setDescription(this.description).addUserOption(o => o.setName('user').setDescription('Who to propose to').setRequired(true)).setDMPermission(false));
    }
    async handle(proposerId, targetId, guildId, send) {
        if (proposerId === targetId) {
            await send({ content: '❌ Hindi ka makakapag-propose sa iyong sarili!', ephemeral: true });
            return;
        }
        ...
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`marry_accept:${proposerId}:${targetId}`).setLabel('💍 Tanggapin').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`marry_decline:${proposerId}:${targetId}`).setLabel('💔 Tanggihan').setStyle(ButtonStyle.Danger));
        ...
        if (!msg)
            return;
        const collector = msg.createMessageComponentCollector({ componentType: ComponentType.Button, time: 300000, filter: (i) => i.user.id === targetId });
        collector.on('collect', async (i) => {
            if (i.customId.startsWith('marry_accept')) {
                ...
                if (!acceptResult.success) {
                    await i.update({ content: `❌ ${acceptResult.error}`, components: [], embeds: [] });
                    return;
                }
    ...
    async executePrefix(m, _args) {
        ...
        if (!t) {
            await m.reply('❌ Mention mo ang taong gusto mong i-propose!');
            return;
        }
