...
export class ThrowCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return (new SlashCommandBuilder()
            ...
            .addUserOption(o => o.setName('user').setDescription('Who to throw at').setRequired(true)));
    }
    gif() { return GIFS[Math.floor(Math.random() * GIFS.length)]; }
    item() { return ITEMS[Math.floor(Math.random() * ITEMS.length)]; }
    ...
    async executePrefix(m, _args) {
        try {
            ...
            if (!t) {
                await m.reply('❌ Mention someone to throw at!');
                return;
            }
            const item = this.item();
            const embed = new EmbedBuilder()
