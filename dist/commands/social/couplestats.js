...
export class CoupleStatsCommand extends BaseCommand {
    constructor() {
        super({ name: 'couplestats', description: 'View advanced relationship stats and milestones 📊', category: 'social', premiumTier: 'gold', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['relationshipstats', 'lovestats'], examples: ['/couplestats', 'p!couplestats'] });
    }
    buildSlashCommand() {
    ...
    async handle(userId, guildId, send, client) {
        ...
        let partner;
        try {
            partner = await client.users.fetch(partnerId);
        }
        catch { /* ignored */ }
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: '📅 Together Since', value: `<t:${Math.floor(marriedAt.getTime() / 1000)}:F>`, inline: false }, { name: '⏰ Duration', value: `**${days}** days • **${weeks}** weeks • **${months}** months`, inline: false }, { name: '💬 Interactions', value: `${profile.interactions || 0} interactions recorded`, inline: true }, { name: '🎯 Next Milestone', value: nextMilestone ? `${nextMilestone} days (in **${daysUntilNext}** more days)` : '🌟 All milestones reached!', inline: true }, { name: '💕 Shared Nickname', value: profile.sharedNickname || 'Not set — use `/couplenickname`', inline: false })
            .setTimestamp();
