...
export class LovemeterCommand extends BaseCommand {
    constructor() {
        super({ name: 'lovemeter', description: 'Measure the love compatibility between two members 💘', category: 'social', premiumTier: 'free', cooldown: 5, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['lovecalc', 'lovecheck'], examples: ['/lovemeter @user1 @user2', 'p!lovemeter @user1 @user2'] });
    }
    buildSlashCommand() {
    ...
    getMessage(pct) {
        ...
        if (pct >= 75)
            return '💕 Strong connection! Malakas ang chemistry!';
        if (pct >= 60)
            return '💗 Good compatibility! May potential ito! 😊';
        if (pct >= 45)
            return '💛 Moderate compatibility. Give it time! ⏰';
        if (pct >= 30)
            return '🤍 Low compatibility. Pero love is unpredictable! 🤷';
        return '💔 Hmm… not the best match. Pero who knows? 🙈';
    }
    buildEmbed(user1, user2, pct) {
