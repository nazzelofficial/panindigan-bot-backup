...
export class HeartbreakCommand extends BaseCommand {
    constructor() {
        super({ name: 'heartbreak', description: 'Dramatic heartbreak message generator (for fun!) 💔', category: 'social', premiumTier: 'gold', cooldown: 3, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['broken', 'sadlove', 'heartbroken'], examples: ['/heartbreak', 'p!heartbreak'] });
    }
    gif() { return GIFS[Math.floor(Math.random() * GIFS.length)]; }
    msg() { return MESSAGES[Math.floor(Math.random() * MESSAGES.length)]; }
