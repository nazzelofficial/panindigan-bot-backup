...
export class PickuplineCommand extends BaseCommand {
    constructor() {
        super({ name: 'pickupline', description: 'Get a curated romantic pickup line 💘', category: 'social', premiumTier: 'gold', cooldown: 3, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['pickup', 'flirt', 'pul'], examples: ['/pickupline', '/pickupline @user', 'p!pickupline @user'] });
    }
    buildSlashCommand() {
        return (new SlashCommandBuilder().setName(this.name).setDescription(this.description)
            .addUserOption(o => o.setName('user').setDescription('Who to flirt with').setRequired(false)));
    }
    line() { return LINES[Math.floor(Math.random() * LINES.length)]; }
