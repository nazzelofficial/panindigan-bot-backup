...
export class CoupleRequestCommand extends BaseCommand {
    constructor() {
        super({ name: 'couplerequest', description: 'Send a couple request to another member 💕', category: 'social', premiumTier: 'free', cooldown: 10, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['cr', 'daterequest'], examples: ['/couplerequest @user', 'p!couplerequest @user'] });
    }
    buildSlashCommand() {
    ...
    async handle(senderId, targetId, guildId, send) {
        if (senderId === targetId) {
            await send({ content: '❌ Hindi ka makakapag-send ng request sa iyong sarili!', ephemeral: true });
            return;
        }
        ...
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`cr_accept:${senderId}:${targetId}`).setLabel('💕 Tanggapin').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`cr_decline:${senderId}:${targetId}`).setLabel('💔 Tanggihan').setStyle(ButtonStyle.Danger));
    ...
    async executePrefix(m, _args) {
        ...
        if (!t) {
            await m.reply('❌ Mention mo ang taong gusto mong maging couple!');
            return;
        }
