...
export class CustomBadgeCommand extends BaseCommand {
    constructor() {
        super({ name: 'custombadge', description: 'Create and equip a custom badge on your profile (Diamond only) 💎', category: 'social', premiumTier: 'diamond', cooldown: 10, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['makebadge', 'createbadge'], examples: ['/custombadge create 🌺 Floral Queen My floral badge', 'p!custombadge create 🔮 Mystic My custom badge'] });
    }
    buildSlashCommand() {
    ...
    async handle(sub, userId, emoji, name, desc, send) {
        const prisma = getPrismaClient();
        if (sub === 'create') {
            if (!emoji || !name) {
                await send({ content: '❌ Provide emoji and name for your custom badge.', ephemeral: true });
                return;
            }
            // Basic emoji validation
            const emojiRegex = /\p{Emoji}/u;
            if (!emojiRegex.test(emoji)) {
                await send({ content: '❌ Please use a valid emoji for your badge.', ephemeral: true });
                return;
            }
            const badgeData = { emoji, name, description: desc || `${name} — custom badge` };
            await prisma.user.upsert({
                where: { userId },
                create: { userId, customBadge: JSON.stringify(badgeData) },
                update: { customBadge: JSON.stringify(badgeData) },
            }).catch(() => null);
            ...
            const badgeData = userData?.customBadge ? JSON.parse(userData.customBadge) : null;
            const embed = new EmbedBuilder()
            ...
            await prisma.user.update({ where: { userId }, data: { customBadge: null } }).catch(() => null);
