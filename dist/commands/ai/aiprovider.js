...
const PROVIDERS = ['openai', 'anthropic', 'google', 'groq', 'auto'];
export class AiProviderCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .setRequired(true)
            .addChoices({ name: '🤖 OpenAI (GPT)', value: 'openai' }, { name: '🧠 Anthropic (Claude)', value: 'anthropic' }, { name: '✨ Google (Gemini)', value: 'google' }, { name: '⚡ Groq (Fast)', value: 'groq' }, { name: '🔄 Auto (Best Available)', value: 'auto' }))
    ...
    async executeSlash(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: `${info.emoji} Provider`, value: provider.charAt(0).toUpperCase() + provider.slice(1), inline: true }, { name: '📋 Description', value: info.description, inline: false }, { name: '🤖 Available Models', value: info.models, inline: false }, { name: '⏱️ Duration', value: 'Active for 24 hours', inline: true })
    ...
    async executePrefix(m, _args) {
        ...
        if (!provider || !PROVIDERS.includes(provider)) {
            return void m.reply(`${EMOJIS.error} Please specify a valid provider.\nAvailable: ${PROVIDERS.map(p => `\`${p}\``).join(', ')}`);
        }
        try {
            const redis = getRedisClient();
            if (redis) {
