...
export class SecurityCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addStringOption(o => o.setName('code').setDescription('Code to audit for security').setRequired(true).setMaxLength(2000));
    }
    ...
    async executePrefix(message, _args) {
        ...
        if (!code)
            return void message.reply(`${EMOJIS.error} Please provide code to audit.`);
        const thinking = await message.reply(`${EMOJIS.ai} Auditing security...`);
        try {
            const client = message.client;
            const response = await client.aiHandler.generateTaskResponse(code, 'Security audit: check for SQL injection, XSS, auth flaws, insecure data handling, hardcoded secrets, OWASP Top 10. Severity + fix for each finding.');
            const embed = new EmbedBuilder()
                .setTitle(`${EMOJIS.ai} 🔒 Security Audit`)
                .setColor(COLORS.error)
