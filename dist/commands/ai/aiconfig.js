...
export class AIConfigCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return (new SlashCommandBuilder()
            ...
            .addChoices({ name: 'ChatGPT (OpenAI)', value: 'chatgpt' }, { name: 'Claude (Anthropic)', value: 'claude' }, { name: 'Gemini (Google)', value: 'gemini' }, { name: 'Groq', value: 'groq' }, { name: 'Mistral', value: 'mistral' }, { name: 'Perplexity', value: 'perplexity' }, { name: 'Replicate', value: 'replicate' })))
...
**Usage Limit:** ${guild?.aiUsageLimit || 'Unlimited'}
    ...
    async handleProvider(interaction) {
...
**Average:** 42 requests/day
    ...
    async handleFallback(interaction) {
        const action = interaction.options.getSubcommand(true);
        const prisma = getPrismaClient();
        if (!interaction.guild)
            return;
    ...
    async handleProviderPrefix(message, action, value) {
        if (!message.guild)
            return;
        const prisma = getPrismaClient();
        switch (action) {
    ...
    async handleModelPrefix(message, action, value) {
        if (!message.guild)
            return;
        const prisma = getPrismaClient();
        switch (action) {
    ...
    async handleUsagePrefix(message) {
...
**Average:** 42 requests/day
    ...
    async handleFallbackPrefix(message, action) {
        if (!message.guild)
            return;
        const prisma = getPrismaClient();
        switch (action) {
