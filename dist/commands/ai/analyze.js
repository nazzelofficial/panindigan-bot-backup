...
export class AnalyzeCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addStringOption(o => o.setName('text').setDescription('Text to analyze deeply').setRequired(true).setMaxLength(2000))
...
Text: "${text}"`;
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: '📝 Input Text', value: text.length > 512 ? text.slice(0, 509) + '...' : text, inline: false }, { name: '📊 Analysis', value: response.content.slice(0, 3800) || 'No analysis returned.', inline: false })
    ...
    async executePrefix(m, _args) {
        ...
        if (!text)
            return void m.reply(`${EMOJIS.error} Please provide text to analyze.`);
        const thinking = await m.reply(`${EMOJIS.ai} Analyzing text deeply...`);
        try {
            const client = m.client;
            const response = await client.aiHandler.generateTaskResponse(text, 'Perform a deep text analysis: key themes, tone & mood, named entities, sentiment, writing style, and a brief summary.');
            const embed = new EmbedBuilder()
                .setTitle(`${EMOJIS.ai} 🔍 Deep Text Analysis`)
                .setColor(COLORS.info)
                .addFields({ name: '📝 Input', value: text.slice(0, 512), inline: false }, { name: '📊 Analysis', value: response.content.slice(0, 3800), inline: false })
