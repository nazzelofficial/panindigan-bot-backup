...
Text: "${text}"`;
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: '📝 Input Text', value: text.length > 512 ? text.slice(0, 509) + '...' : text, inline: false }, { name: '🏷️ Classification', value: response.content.slice(0, 3800) || 'No classification returned.', inline: false })
    ...
    async executePrefix(m, _args) {
        ...
        if (!text)
            return void m.reply(`${EMOJIS.error} Please provide text to classify.`);
        const thinking = await m.reply(`${EMOJIS.ai} Classifying text...`);
        try {
            const client = m.client;
            const response = await client.aiHandler.generateTaskResponse(text, 'Classify the following text into primary category, subcategories, tags, confidence score, and a brief reasoning.');
            const embed = new EmbedBuilder()
                .setTitle(`${EMOJIS.ai} 🏷️ Text Classification`)
                .setColor(COLORS.gold)
                .addFields({ name: '📝 Input', value: text.slice(0, 512), inline: false }, { name: '🏷️ Classification', value: response.content.slice(0, 3800), inline: false })
