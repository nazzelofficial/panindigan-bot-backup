...
export class ResumeCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addChoices({ name: 'Review and improve', value: 'review' }, { name: 'Generate bullet points', value: 'generate' }, { name: 'Write summary/objective', value: 'summary' }));
    }
    async executeSlash(interaction) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: `✨ ${action.charAt(0).toUpperCase() + action.slice(1)}`, value: response.content.slice(0, 4000), inline: false })
    ...
    async executePrefix(message, _args) {
        ...
        if (!content)
            return void message.reply(`${EMOJIS.error} Please provide your resume content.`);
        const thinking = await message.reply(`${EMOJIS.ai} Working on resume...`);
        try {
            const client = message.client;
            const response = await client.aiHandler.generateTaskResponse(content, 'Review and improve this resume section. Use action verbs, quantify achievements, remove weak language. Provide improved version and explain changes.');
            const embed = new EmbedBuilder()
