...
export class AiModelCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addStringOption(o => o.setName('model').setDescription('Model name to switch to (leave empty to see list)').setRequired(false))
    ...
    async executeSlash(i) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(COLORS.gold)
                .addFields({ name: '🤖 Model', value: `\`${modelName}\``, inline: true }, { name: '🔷 Provider', value: provider.charAt(0).toUpperCase() + provider.slice(1), inline: true }, { name: '⏱️ Duration', value: 'Active for 24 hours', inline: true })
