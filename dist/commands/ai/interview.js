...
export class InterviewCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addChoices({ name: 'Behavioral (STAR)', value: 'behavioral' }, { name: 'Technical', value: 'technical' }, { name: 'Mixed', value: 'mixed' }));
    }
    async executeSlash(interaction) {
        ...
        try {
            ...
            const response = await client.aiHandler.generateTaskResponse(role, `You are an interview coach. Generate 5 ${type} interview questions for a ${role} position. For each question: the question itself, what the interviewer is looking for, and a sample strong answer using the STAR method where applicable. Make them realistic and challenging.`);
    ...
    async executePrefix(message, _args) {
        ...
        if (!role)
            return void message.reply(`${EMOJIS.error} Please provide a job role.`);
        const thinking = await message.reply(`${EMOJIS.ai} Generating interview questions...`);
        try {
            const client = message.client;
            const response = await client.aiHandler.generateTaskResponse(role, 'Generate 5 interview questions with what the interviewer looks for and sample strong answers using STAR method.');
            const embed = new EmbedBuilder()
