...
export class RegexCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addChoices({ name: 'Generate regex', value: 'generate' }, { name: 'Explain this regex', value: 'explain' }));
    }
    async executeSlash(interaction) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: mode === 'explain' ? '🔍 Pattern' : '📋 Requirement', value: description.slice(0, 512), inline: false }, { name: '📝 Result', value: response.content.slice(0, 3500), inline: false })
    ...
    async executePrefix(message, _args) {
        ...
        if (!description)
            return void message.reply(`${EMOJIS.error} Please describe the pattern you need.`);
        const thinking = await message.reply(`${EMOJIS.ai} Generating regex...`);
        try {
            const client = message.client;
            const response = await client.aiHandler.generateTaskResponse(description, 'Generate a regex pattern: the pattern itself, explanation of each part, test examples, and JavaScript usage.');
            const embed = new EmbedBuilder()
