...
export class JokeCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addChoices({ name: 'Pun', value: 'pun' }, { name: 'Knock-knock', value: 'knock-knock' }, { name: 'One-liner', value: 'one-liner' }, { name: 'Dad joke', value: 'dad joke' }, { name: 'Programming', value: 'programming' }));
    }
    async executeSlash(interaction) {
        ...
        try {
            ...
            const response = await client.aiHandler.generateTaskResponse(`Topic: ${topic}`, `Tell a funny, clean ${type} joke${topic !== 'anything' ? ` about ${topic}` : ''}. Make it clever and actually funny. Keep it family-friendly. If it's a setup/punchline joke, format it clearly.`);
    ...
    async executePrefix(message, _args) {
        ...
        try {
            ...
            const response = await client.aiHandler.generateTaskResponse(`Topic: ${topic}`, `Tell a funny, clean joke about ${topic}. Make it clever and actually funny. Keep it family-friendly.`);
            const embed = new EmbedBuilder()
                .setTitle(`${EMOJIS.ai} 😂 Joke`)
                .setColor(COLORS.warning)
                .setDescription(response.content.slice(0, 4000))
