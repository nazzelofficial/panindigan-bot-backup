...
export class GiveawayTemplateCommand extends BaseCommand {
    ...
    async executeSlash(i) {
        ...
        if (sub === 'save') {
            ...
            if (!templates.length) {
                await i.reply({ content: '📭 No templates saved.', ephemeral: true });
                return;
            }
            const embed = new EmbedBuilder().setTitle('📋 Giveaway Templates').setColor(COLORS.gold)
                .setDescription(templates.map((t) => `**${t.name}**: ${t.prize} | ${t.duration} | ${t.winners} winner(s)`).join('\n'));
    ...
    async executePrefix(m, _args) {
        ...
        if (sub === 'list') {
            ...
            if (!templates.length) {
                await m.reply('📭 No templates.');
                return;
            }
            await m.reply(`📋 **Templates:**\n${templates.map((t) => `• **${t.name}**: ${t.prize} | ${t.duration}`).join('\n')}`);
        }
