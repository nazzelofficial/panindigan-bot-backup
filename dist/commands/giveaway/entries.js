...
export class GiveawayEntriesCommand extends BaseCommand {
    constructor() {
        super({ name: 'gentries', description: 'List all entries in a giveaway', category: 'giveaway', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['giveaway-entries', 'gw-entries'], examples: ['/gentries <id>', 'p!gentries <id>'] });
    }
    buildSlashCommand() {
        return (new SlashCommandBuilder().setName(this.name).setDescription(this.description)
            .addStringOption(o => o.setName('id').setDescription('Giveaway ID').setRequired(true)).setDMPermission(false));
    }
    async getEntries(guildId, id) {
    ...
    async executePrefix(m, _args) {
        if (!args[0]) {
            await m.reply('❌ Usage: `p!gentries <id>`');
            return;
        }
