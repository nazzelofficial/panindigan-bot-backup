...
export class GiveawayResumeCommand extends BaseCommand {
    constructor() {
        super({ name: 'gresume', description: 'Resume a paused giveaway', category: 'giveaway', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['giveaway-resume', 'gw-resume'], examples: ['/gresume <id>', 'p!gresume <id>'] });
    }
    buildSlashCommand() {
    ...
    async executeSlash(i) {
        ...
        if (!g) {
            await i.reply({ content: '❌ Giveaway not found.', ephemeral: true });
            return;
        }
    ...
    async executePrefix(m, _args) {
        if (!args[0]) {
            await m.reply('❌ Usage: `p!gresume <id>`');
            return;
        }
        const prisma = getPrismaClient();
