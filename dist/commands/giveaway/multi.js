...
export class GiveawayMultiCommand extends BaseCommand {
    ...
    async executeSlash(i) {
        ...
        if (!prizes.length) {
            await i.reply({ content: '❌ No prizes provided.', ephemeral: true });
            return;
        }
        const ms = this.parseDuration(duration);
        if (!ms) {
            await i.reply({ content: '❌ Invalid duration.', ephemeral: true });
            return;
        }
        ...
        const embed = new EmbedBuilder().setTitle('🎉 MULTI-PRIZE GIVEAWAY!').setColor(COLORS.gold)
            ...
            .setTimestamp(endsAt);
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`giveaway_enter:${g.id}`).setLabel('🎉 Enter Giveaway').setStyle(ButtonStyle.Primary));
    ...
    async executePrefix(m, _args) {
        if (args.length < 2) {
            await m.reply('❌ Usage: `p!gmulti "<prize1,prize2,prize3>" <duration>`\nExample: `p!gmulti "Nitro,VIP,Role" 1h`');
            return;
        }
        ...
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`giveaway_enter:${g.id}`).setLabel('🎉 Enter').setStyle(ButtonStyle.Primary));
