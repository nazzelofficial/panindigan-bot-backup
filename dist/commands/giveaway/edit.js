...
export class GiveawayEditCommand extends BaseCommand {
    constructor() {
        super({ name: 'gedit', description: 'Edit an active giveaway (prize, time, winners)', category: 'giveaway', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['giveaway-edit', 'gw-edit'], examples: ['/gedit <id>', 'p!gedit <id>'] });
    }
    buildSlashCommand() {
    ...
    async executeSlash(i) {
        ...
        if (winners)
            updateData.winnerCount = winners;
        if (addTime) {
            const units = { s: 1000, m: 60000, h: 3600000, d: 86400000 };
            const match = addTime.match(/^(\d+)([smhd])$/i);
            if (match)
                updateData.endsAt = new Date(new Date(g.endsAt).getTime() + parseInt(match[1]) * (units[match[2].toLowerCase()] || 0));
        }
    ...
    async executePrefix(m, _args) {
        if (args.length < 3) {
            await m.reply('❌ Usage: `p!gedit <id> prize <new prize>` or `p!gedit <id> winners <count>`');
            return;
        }
        ...
        const updateData = {};
        if (field === 'prize')
            updateData.prize = rest.join(' ');
        else if (field === 'winners')
            updateData.winnerCount = parseInt(rest[0]) || g.winnerCount;
        await prisma.giveaway.update({ where: { id }, data: updateData });
