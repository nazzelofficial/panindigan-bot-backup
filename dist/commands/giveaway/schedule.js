...
export class GiveawayScheduleCommand extends BaseCommand {
    constructor() {
        super({ name: 'gschedule', description: 'Schedule a giveaway to start at a future time', category: 'giveaway', premiumTier: 'free', cooldown: 10, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['giveaway-schedule', 'gw-schedule'], examples: ['/gschedule', 'p!gschedule Nitro 2d 1h 1'] });
    }
    buildSlashCommand() {
    ...
    async executeSlash(i) {
        ...
        if (!startMs) {
            await i.reply({ content: '❌ Invalid start_in format. Use: 1h, 2d, etc.', ephemeral: true });
            return;
        }
        if (!durationMs) {
            await i.reply({ content: '❌ Invalid duration format.', ephemeral: true });
            return;
        }
        ...
        const embed = new EmbedBuilder().setTitle('📅 Giveaway Scheduled').setColor(COLORS.gold)
            .addFields({ name: '🎁 Prize', value: prize, inline: true }, { name: '👥 Winners', value: `${winners}`, inline: true }, { name: '🕐 Starts', value: `<t:${Math.floor(startAt.getTime() / 1000)}:F>`, inline: false }, { name: '🏁 Ends', value: `<t:${Math.floor(endsAt.getTime() / 1000)}:F>`, inline: false }, { name: '🎫 ID', value: `\`${g.id}\``, inline: true }).setTimestamp();
        await i.reply({ embeds: [embed] });
    }
    async executePrefix(m, _args) {
        if (args.length < 4) {
            await m.reply('❌ Usage: `p!gschedule <prize> <start_in> <duration> [winners]`\nExample: `p!gschedule "Nitro" 2h 1d 1`');
            return;
        }
        ...
        if (!startMs || !durationMs) {
            await m.reply('❌ Invalid duration format.');
            return;
        }
        const startAt = new Date(Date.now() + startMs);
        const endsAt = new Date(startAt.getTime() + durationMs);
