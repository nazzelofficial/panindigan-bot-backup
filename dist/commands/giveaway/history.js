...
export class GiveawayHistoryCommand extends BaseCommand {
    constructor() {
        super({ name: 'ghistory', description: 'View history of past giveaways in this server', category: 'giveaway', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['giveaway-history', 'gw-history'], examples: ['/ghistory', 'p!ghistory'] });
    }
    async executeSlash(i) {
        ...
        if (!giveaways.length) {
            await i.editReply({ content: '📭 No completed giveaways found.' });
            return;
        }
    ...
    async executePrefix(m, _args) {
        ...
        if (!giveaways.length) {
            await m.reply('📭 No completed giveaways.');
            return;
        }
        const embed = new EmbedBuilder().setTitle('📜 Giveaway History').setColor(COLORS.default)
