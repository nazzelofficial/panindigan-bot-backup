...
export class GiveawayBonusCommand extends BaseCommand {
    constructor() {
        super({ name: 'gbonus', description: 'Give bonus entries to specific roles in a giveaway', category: 'giveaway', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['giveaway-bonus', 'gw-bonus'], examples: ['/gbonus <id> @role 2', 'p!gbonus <id> @role 2'] });
    }
    buildSlashCommand() {
    ...
    async executeSlash(i) {
        ...
        const bonusRoles = g.bonusRoles ? JSON.parse(g.bonusRoles || '{}') : {};
        bonusRoles[role.id] = entries;
        await prisma.giveaway.update({ where: { id }, data: { bonusRoles: JSON.stringify(bonusRoles) } });
        await i.reply({ content: `✅ Members with <@&${role.id}> will receive **${entries}** bonus entries!`, ephemeral: true });
    }
    async executePrefix(m, _args) {
        if (args.length < 3) {
            await m.reply('❌ Usage: `p!gbonus <id> @role <entries>`');
            return;
        }
