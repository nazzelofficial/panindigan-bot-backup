...
export class GiveawayCommand extends BaseCommand {
    ...
    async startGiveaway(guildId, channelId, prize, duration, winners, hostId, client) {
        ...
        if (!ms || ms < 10000)
            return '❌ Invalid duration. Use format like `1h`, `30m`, `2d`. Minimum 10 seconds.';
        if (ms > 30 * 86400000)
            return '❌ Maximum giveaway duration is 30 days.';
    ...
    async endGiveaway(messageId, guildId, client, forced) {
        ...
        if (!giveaway)
            return '❌ Giveaway not found or already ended.';
        ...
        return `✅ Giveaway ended! Winners: ${winners.length ? winners.map((w) => `<@${w}>`).join(', ') : 'None'}`;
    }
    async executeSlash(i) {
        ...
        if (sub === 'start') {
            ...
            if (!giveaways.length) {
                await i.editReply({ content: '❌ No active giveaways.' });
                return;
            }
            const embed = new EmbedBuilder().setTitle('🎉 Active Giveaways').setColor(COLORS.gold)
                .setDescription(giveaways.map((g) => `**${g.prize}** — <#${g.channelId}> ends <t:${Math.floor(new Date(g.endsAt).getTime() / 1000)}:R>`).join('\n'));
    ...
    async executePrefix(m, _args) {
        ...
        if (sub === 'start') {
            ...
            if (!prize) {
                await m.reply('❌ Please provide a prize.');
                return;
            }
