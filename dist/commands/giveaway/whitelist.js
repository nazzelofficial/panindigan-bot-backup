...
export class GiveawayWhitelistCommand extends BaseCommand {
    constructor() {
        super({ name: 'gwhitelist', description: 'Make a giveaway exclusive to specific roles only', category: 'giveaway', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['giveaway-whitelist', 'gw-whitelist'], examples: ['/gwhitelist <id> @role', 'p!gwhitelist <id> @role'] });
    }
    buildSlashCommand() {
    ...
    async executeSlash(i) {
        ...
        await prisma.giveaway.update({ where: { id }, data: { whitelistRoleId: role.id } });
        await i.reply({ content: `✅ Giveaway is now exclusive to <@&${role.id}> members only.`, ephemeral: true });
    }
    async executePrefix(m, _args) {
        if (!args[0]) {
            await m.reply('❌ Usage: `p!gwhitelist <id> @role`');
            return;
        }
        ...
        if (!g) {
            await m.reply('❌ Giveaway not found.');
            return;
        }
        await prisma.giveaway.update({ where: { id: args[0] }, data: { whitelistRoleId: role.id } });
