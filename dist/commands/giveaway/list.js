...
export class GiveawayListCommand extends BaseCommand {
    constructor() {
        super({ name: 'glist', description: 'List all active giveaways in the server', category: 'giveaway', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['giveaway-list', 'gw-list', 'giveaways'], examples: ['/glist', 'p!glist'] });
    }
    async executeSlash(i) {
        ...
        if (!giveaways.length) {
            await i.editReply({ content: '📭 No active giveaways in this server.' });
            return;
        }
    ...
    async executePrefix(m, _args) {
        ...
        if (!giveaways.length) {
            await m.reply('📭 No active giveaways.');
            return;
        }
        const embed = new EmbedBuilder().setTitle('🎉 Active Giveaways').setColor(COLORS.gold)
