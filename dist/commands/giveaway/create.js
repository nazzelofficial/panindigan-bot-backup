...
import { EmbedBuilder, SlashCommandBuilder, PermissionFlagsBits, ButtonBuilder, ButtonStyle, ActionRowBuilder } from 'discord.js';
import { COLORS } from '../../utils/Constants.js';
import { getPrismaClient } from '../../database/postgresql/client.js';
export class GiveawayCreateCommand extends BaseCommand {
    constructor() {
        super({ name: 'gcreate', description: 'Create and start a giveaway interactively', category: 'giveaway', premiumTier: 'free', cooldown: 10, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['giveaway-create', 'gw-create'], examples: ['/gcreate', 'p!gcreate'] });
    }
    buildSlashCommand() {
    ...
    async createGiveaway(guildId, channelId, prize, durationStr, winnerCount, hostId, requiredRoleId) {
        const ms = this.parseDuration(durationStr);
        if (!ms || ms < 10000)
            return { success: false, message: '❌ Invalid duration. Use format: 1h, 30m, 2d. Minimum 10 seconds.' };
        if (ms > 30 * 86400000)
    ...
    async executeSlash(i) {
        ...
        if (!result.success || !result.giveawayId) {
            await i.editReply({ content: result.message || '❌ Failed to create giveaway.' });
            return;
        }
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: '🎁 Prize', value: prize, inline: true }, { name: '👥 Winners', value: `${winners}`, inline: true }, { name: '🕐 Ends', value: `<t:${Math.floor(endsAt.getTime() / 1000)}:R>`, inline: true }, { name: '🎫 Hosted by', value: `<@${i.user.id}>`, inline: true }, ...(requiredRole ? [{ name: '🔒 Required Role', value: `<@&${requiredRole.id}>`, inline: true }] : []))
            .setFooter({ text: `Giveaway ID: ${result.giveawayId}` })
            .setTimestamp(endsAt);
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`giveaway_enter:${result.giveawayId}`).setLabel('🎉 Enter Giveaway').setStyle(ButtonStyle.Primary));
    ...
    async executePrefix(m, _args) {
        if (!args.length) {
            await m.reply('❌ Usage: `p!gcreate <prize> <duration> [winners]`\nExample: `p!gcreate Nitro 1h 3`');
            return;
        }
        const durationIdx = _args.findIndex(a => /^\d+[smhd]$/i.test(a));
        if (durationIdx === -1) {
            await m.reply('❌ Please include a valid duration (e.g. 1h, 30m, 2d).');
            return;
        }
        ...
        if (!result.success || !result.giveawayId) {
            await m.reply(result.message || '❌ Failed.');
            return;
        }
        const ms = this.parseDuration(duration);
        const endsAt = new Date(Date.now() + ms);
