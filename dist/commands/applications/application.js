...
import { EmbedBuilder, SlashCommandBuilder, PermissionFlagsBits, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, ButtonBuilder, ButtonStyle, } from 'discord.js';
import { COLORS } from '../../utils/Constants.js';
import { getPrismaClient } from '../../database/postgresql/client.js';
export class ApplicationCommand extends BaseCommand {
    constructor() {
        super({ name: 'application', description: 'Manage and submit applications', category: 'applications', premiumTier: 'silver', cooldown: 10, guildOnly: true, slashCommand: true, prefixCommand: false, aliases: ['apply', 'app'], examples: ['/application create', '/application list', '/application view <id>'] });
    }
    buildSlashCommand() {
    ...
    async executeSlash(i) {
        ...
        if (sub === 'create') {
            if (!i.memberPermissions?.has(PermissionFlagsBits.ManageGuild)) {
                await i.reply({ content: '❌ You need Manage Server permission to create forms.', ephemeral: true });
                return;
            }
            ...
            if (!form) {
                await i.reply({ content: `❌ Form **${formName}** not found.`, ephemeral: true });
                return;
            }
            const questions = form.questions || [];
            if (!questions.length) {
                await i.reply({ content: '❌ This form has no questions set up yet.', ephemeral: true });
                return;
            }
            ...
            questions.slice(0, 5).forEach((q, idx) => {
                modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId(`q_${idx}`).setLabel(q.question).setStyle(TextInputStyle.Paragraph).setRequired(q.required ?? true).setMaxLength(1000)));
            });
            ...
            if (form.reviewChannelId) {
                ...
                if (reviewCh?.isTextBased()) {
                    ...
                    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`app_accept:${application.id}`).setLabel('✅ Accept').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`app_deny:${application.id}`).setLabel('❌ Deny').setStyle(ButtonStyle.Danger));
            ...
            if (!i.memberPermissions?.has(PermissionFlagsBits.ManageGuild)) {
                await i.reply({ content: '❌ You need Manage Server to review applications.', ephemeral: true });
                return;
            }
            const id = i.options.getString('id', true);
