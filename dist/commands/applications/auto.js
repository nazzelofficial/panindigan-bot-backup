...
export class ApplicationAutoCommand extends BaseCommand {
    constructor() {
        super({ name: 'application-auto', description: 'Configure auto-accept/auto-deny rules for an application', category: 'applications', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['app-auto', 'appauto'], examples: ['/application-auto <id>'] });
    }
    buildSlashCommand() {
    ...
    async executeSlash(i) {
        ...
        await prisma.application.update({
            where: { id },
            data: { autoMode: mode, minLevel: minLevel || null, minAccountAge: minAge || null },
        });
        let msg = `✅ Application **${app.name}** set to **${mode}** mode.`;
        if (minLevel)
            msg += ` Min level: ${minLevel}.`;
        if (minAge)
            msg += ` Min account age: ${minAge} days.`;
        await i.reply({ content: msg, ephemeral: true });
    }
    async executePrefix(m, _args) {
        if (args.length < 2) {
            await m.reply('❌ Usage: `p!application-auto <id> accept|deny|manual`');
            return;
        }
        const [id, mode] = _args;
        const prisma = getPrismaClient();
        ...
        await prisma.application.update({ where: { id }, data: { autoMode: mode } });
