...
export class ApplicationResponsesCommand extends BaseCommand {
    constructor() {
        super({ name: 'application-responses', description: 'View a specific applicant\'s responses', category: 'applications', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['app-responses', 'appresponses', 'appreview'], examples: ['/application-responses <form-id>'] });
    }
    buildSlashCommand() {
    ...
    async getResponses(guildId, formId) {
        ...
        return new EmbedBuilder()
            ...
            .setColor(form.status === 'accepted' ? COLORS.success : form.status === 'denied' ? COLORS.error : COLORS.warning)
    ...
    async executePrefix(m, _args) {
        if (!args[0]) {
            await m.reply('❌ Usage: `p!application-responses <form-id>`');
            return;
        }
