...
import { ChatInputCommandInteraction, EmbedBuilder, PermissionFlagsBits, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder } from 'discord.js';
...
export class SetupCommand extends BaseCommand {
    ...
    async runSetup(interaction) {
        ...
        const collector = (interaction instanceof ChatInputCommandInteraction
            ...
            : await interaction.channel.messages.fetch(interaction.id)).createMessageComponentCollector({ time: 300000 });
    ...
    async setupLanguage(interaction) {
        if (!interaction.guild)
            return;
        const prisma = getPrismaClient();
        const embed = new EmbedBuilder()
    ...
    async setupComplete(interaction) {
        const embed = new EmbedBuilder()
