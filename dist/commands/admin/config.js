...
export class ConfigCommand extends BaseCommand {
    ...
    async handleGeneral(interaction) {
        const action = interaction.options.getSubcommand(true);
        const prisma = getPrismaClient();
        if (!interaction.guild)
            return;
    ...
    async handleIgnore(interaction) {
        const action = interaction.options.getSubcommand(true);
        const prisma = getPrismaClient();
        if (!interaction.guild)
            return;
    ...
    async handleRolesPrefix(message, action, value) {
    ...
    async handleChannelsPrefix(message, action, value) {
    ...
    async handleGeneralPrefix(message, action, value) {
        const prisma = getPrismaClient();
        if (!message.guild)
            return;
        let updateData = {};
    ...
    async handleIgnorePrefix(message, action, value) {
        const prisma = getPrismaClient();
        if (!message.guild)
            return;
