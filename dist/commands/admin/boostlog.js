...
export class BoostLogCommand extends BaseCommand {
    constructor() {
        super({ name: 'boostlog', description: 'Set up boost notification logging channel 🚀', category: 'admin', premiumTier: 'bronze', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['setboostlog', 'boostchannel', 'boostnotify'], examples: ['/boostlog #boosts', 'p!boostlog #boost-log'] });
    }
    buildSlashCommand() {
    ...
    async handle(guildId, channelId, customMsg, send) {
        const prisma = getPrismaClient();
        const defaultMsg = '🚀 {user} just boosted the server! Thank you for the boost! ❤️';
        await prisma.guild.upsert({
            where: { guildId },
            create: { guildId, boostLogChannelId: channelId, boostMessage: customMsg || defaultMsg },
            update: { boostLogChannelId: channelId, boostMessage: customMsg || defaultMsg },
        });
        const embed = new EmbedBuilder()
            .setTitle('🚀 Boost Log Updated')
            .setColor(COLORS.success)
            .addFields({ name: '📍 Channel', value: channelId ? `<#${channelId}>` : '❌ Disabled', inline: true }, { name: '💬 Message', value: customMsg || defaultMsg, inline: false })
