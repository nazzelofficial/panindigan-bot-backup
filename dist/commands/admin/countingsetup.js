...
export class CountingSetupCommand extends BaseCommand {
    constructor() {
        super({ name: 'countingsetup', description: 'Set up a counting channel for the server 🔢', category: 'admin', premiumTier: 'bronze', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['setupcounting', 'counting'], examples: ['/countingsetup #counting', '/countingsetup disable', 'p!countingsetup #counting'] });
    }
    buildSlashCommand() {
    ...
    async handle(sub, guildId, channelId, startNum, send) {
        const prisma = getPrismaClient();
        if (sub === 'set') {
            await prisma.guild.upsert({
                where: { guildId },
                create: { guildId, countingChannelId: channelId, countingCurrent: startNum },
                update: { countingChannelId: channelId, countingCurrent: startNum },
            });
            const embed = new EmbedBuilder()
                .setTitle('🔢 Counting Channel Set!')
                .setColor(COLORS.success)
                .addFields({ name: '📍 Channel', value: `<#${channelId}>`, inline: true }, { name: '🔢 Starting At', value: `${startNum}`, inline: true })
                .setDescription('Members must count up by 1 each message. Wrong numbers will be deleted!')
                .setTimestamp();
            await send({ embeds: [embed] });
        }
        else if (sub === 'disable') {
            await prisma.guild.upsert({ where: { guildId }, create: { guildId, countingChannelId: null }, update: { countingChannelId: null } });
            await send({ content: '✅ Counting channel disabled.', ephemeral: true });
        }
        else if (sub === 'info') {
            const guild = await prisma.guild.findUnique({ where: { guildId } });
            const chId = guild?.countingChannelId;
            const current = guild?.countingCurrent || 0;
            const embed = new EmbedBuilder()
                .setTitle('🔢 Counting Channel Info')
                .setColor(COLORS.info)
                .addFields({ name: '📍 Channel', value: chId ? `<#${chId}>` : 'Not set', inline: true }, { name: '🔢 Current Count', value: `${current}`, inline: true })
                .setTimestamp();
            await send({ embeds: [embed] });
        }
        else if (sub === 'reset') {
            await prisma.guild.upsert({ where: { guildId }, create: { guildId, countingCurrent: 0 }, update: { countingCurrent: 0 } });
