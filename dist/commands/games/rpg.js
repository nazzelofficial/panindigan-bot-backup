...
export class RPGCommand extends BaseCommand {
    ...
    async getOrCreateChar(userId, guildId) {
        const prisma = getPrismaClient();
        let char = await prisma.rpgCharacter?.findUnique?.({ where: { userId_guildId: { userId, guildId } } }).catch(() => null);
        return char;
    }
    async handleSubcommand(sub, userId, guildId, username, classArg) {
        if (sub === 'start') {
        ...
        if (sub === 'class' && classArg) {
            const cls = CLASSES[classArg];
            if (!cls)
                return new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Invalid class. Choose: warrior, mage, archer');
        ...
        if (sub === 'stats') {
            return new EmbedBuilder()
                ...
                .addFields({ name: '🧙 Class', value: 'Warrior (example)', inline: true }, { name: '📊 Level', value: '1', inline: true }, { name: '✨ XP', value: '0 / 100', inline: true }, { name: '❤️ HP', value: '150 / 150', inline: true }, { name: '⚔️ ATK', value: '20', inline: true }, { name: '🛡️ DEF', value: '15', inline: true }, { name: '💰 Gold', value: '100', inline: true })
        ...
        if (sub === 'fight') {
            ...
            return new EmbedBuilder()
                ...
                .addFields({ name: `${monster.emoji} ${monster.name}`, value: `HP: ${monster.hp} | ATK: ${monster.atk}`, inline: true }, { name: '👤 You', value: `ATK: ${playerAtk} | DEF: ${playerDef}`, inline: true })
                .setTimestamp();
        }
        if (sub === 'explore') {
            ...
            return new EmbedBuilder()
                ...
                .addFields({ name: '✨ XP Gained', value: `+${event.xp}`, inline: true }, { name: '💰 Gold Gained', value: `+${event.gold}`, inline: true })
                .setTimestamp();
        }
        if (sub === 'inventory') {
            return new EmbedBuilder()
