...
export class RiddleCommand extends BaseCommand {
    ...
    async runRiddle(reply, editReply, channel, userId, username) {
        const riddle = RIDDLES[Math.floor(Math.random() * RIDDLES.length)];
        let hintUsed = false;
        const hintRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('riddle_hint').setLabel('💡 Get Hint').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId('riddle_give_up').setLabel('🏳️ Give Up').setStyle(ButtonStyle.Danger));
    ...
    async executeSlash(interaction) {
        await this.runRiddle((c) => interaction.reply(c), (c) => interaction.editReply(c), interaction.channel, interaction.user.id, interaction.user.username);
    }
    async executePrefix(message) {
        let sent;
        await this.runRiddle(async (c) => { sent = await message.reply(c); return sent; }, async (c) => sent.edit(c), message.channel, message.author.id, message.author.username);
    }
}
