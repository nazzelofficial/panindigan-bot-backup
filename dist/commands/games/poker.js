...
export class PokerCommand extends BaseCommand {
    ...
    buildSlashCommand() {
        return new SlashCommandBuilder()
            ...
            .addIntegerOption(o => o.setName('bet').setDescription('Amount to bet (in Piso)').setRequired(false).setMinValue(10).setMaxValue(10000));
    }
    async executeSlash(interaction) {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: `👤 Your Hand (${interaction.user.username})`, value: `${handStr(playerHand)}\n${playerEval}`, inline: false }, { name: '🤖 Bot\'s Hand', value: `${handStr(botHand)}\n${botEval}`, inline: false }, { name: '🎲 Result', value: resultText, inline: false }, { name: '💰 Balance', value: `₱${newBalance.toLocaleString()}`, inline: true }, { name: '🎯 Bet', value: `₱${bet.toLocaleString()}`, inline: true })
    ...
    async executePrefix(message, _args) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: '👤 Your Hand', value: `${handStr(playerHand)}\n${playerEval}`, inline: false }, { name: '🤖 Bot Hand', value: `${handStr(botHand)}\n${botEval}`, inline: false }, { name: '🎲 Result', value: playerWins ? `🎉 You Win! +₱${bet}` : `😔 Bot Wins! -₱${bet}`, inline: false })
            .setTimestamp();
