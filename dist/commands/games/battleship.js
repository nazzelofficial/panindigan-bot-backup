...
function renderGrid(grid, reveal = false) {
    ...
    const rows = grid.map((row, i) => `**${i + 1}** ` + row.map(c => reveal ? c : (c === '🚢' ? '🌊' : c)).join(''));
    ...
    async runGame(reply, editReply, userId, channelCollect) {
        ...
        const buildEmbed = (ended = false) => {
            ...
            return new EmbedBuilder()
                ...
                .addFields({ name: `🎯 Enemy Waters (${remaining} ship parts left)`, value: renderGrid(botGrid), inline: false }, { name: `📊 Shots Fired: ${shots}/${maxShots}`, value: ended ? (remaining === 0 ? '🎉 You sank all ships!' : '💀 Out of shots!') : 'Type a coordinate (e.g. `A1`, `C3`) to fire!', inline: false })
                .setTimestamp();
        };
        const buildButtons = () => {
            const cols = ['A', 'B', 'C', 'D', 'E'];
            ...
            for (let r = 0; r < GRID_SIZE; r++) {
                const row = new ActionRowBuilder().addComponents(cols.map((c, ci) => {
        ...
        collector.on('end', async (col) => {
            if (col.size === 0)
                await editReply({ embeds: [buildEmbed(true).setFooter({ text: 'Game expired!' })], components: [] });
        });
    }
    async executeSlash(interaction) {
        await this.runGame((c) => interaction.reply(c), (c) => interaction.editReply(c), interaction.user.id, (o) => interaction.channel.createMessageComponentCollector(o));
    }
    async executePrefix(message) {
        await this.runGame((c) => message.reply(c), async (c) => { const m = await message.reply('...'); return m.edit(c); }, message.author.id, (o) => message.channel.createMessageComponentCollector(o));
    }
}
