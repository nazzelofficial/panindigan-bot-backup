...
function getColor(n) { return n === 0 ? '🟢' : RED_NUMBERS.includes(n) ? '🔴' : '⚫'; }
function isEven(n) { return n !== 0 && n % 2 === 0; }
function evaluateBet(bet, result) {
    if (bet === 'red')
        return { won: RED_NUMBERS.includes(result), multiplier: 2 };
    if (bet === 'black')
        return { won: !RED_NUMBERS.includes(result) && result !== 0, multiplier: 2 };
    if (bet === 'even')
        return { won: isEven(result), multiplier: 2 };
    if (bet === 'odd')
        return { won: !isEven(result) && result !== 0, multiplier: 2 };
    if (bet === '1-18')
    ...
    if (!isNaN(num))
        return { won: result === num, multiplier: 36 };
...
export class RouletteCommand extends BaseCommand {
    ...
    async runRoulette(reply, editReply, userId, channelCollect, presetBet) {
        const outsideBets = [
        ...
        const row1 = new ActionRowBuilder().addComponents(outsideBets.slice(0, 3).map(b => new ButtonBuilder().setCustomId(b.id).setLabel(b.label).setStyle(b.style)));
        const row2 = new ActionRowBuilder().addComponents(outsideBets.slice(3).map(b => new ButtonBuilder().setCustomId(b.id).setLabel(b.label).setStyle(b.style)));
        ...
        collector.on('end', async (col) => {
            if (col.size === 0)
                await editReply({ embeds: [promptEmbed.setFooter({ text: 'Time\'s up!' })], components: [] });
        });
    }
    async executeSlash(interaction) {
        await this.runRoulette((c) => interaction.reply(c), (c) => interaction.editReply(c), interaction.user.id, (o) => interaction.channel.createMessageComponentCollector(o));
    }
    async executePrefix(message, _args) {
        const bet = args[0];
        await this.runRoulette((c) => message.reply(c), async (c) => { const m = await message.reply('...'); return m.edit(c); }, message.author.id, (o) => message.channel.createMessageComponentCollector(o), bet);
    }
}
