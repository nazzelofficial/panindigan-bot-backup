...
export class UnoCommand extends BaseCommand {
    ...
    async executeSlash(interaction) {
        ...
        const buildGameEmbed = (playerCards, botCardCount, top, turn, message) => new EmbedBuilder()
            .setTitle(`🃏 UNO!`)
            .setColor(COLORS.warning)
            .setDescription(message || (turn === 'player' ? '**Your turn!** Choose a card to play.' : '**Bot\'s turn...**'))
            .addFields({ name: '📋 Top Card', value: top.display, inline: true }, { name: '🤖 Bot Cards', value: `**${botCardCount}** cards`, inline: true }, { name: `👤 Your Hand (${playerCards.length} cards)`, value: playerCards.map((c, i) => `\`${i + 1}.\` ${c.display}`).join('\n') || 'No cards', inline: false })
            .setFooter({ text: 'UNO! | Click a card number button to play' })
            .setTimestamp();
        const buildCardButtons = (cards, top) => {
    ...
    async executePrefix(message, _args) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: '📋 Top Card', value: topCard.display, inline: false }, { name: '🎴 Your Hand', value: playerHand.map(c => c.display).join('\n'), inline: false })
