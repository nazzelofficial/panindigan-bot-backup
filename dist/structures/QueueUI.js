...
export function createComprehensiveQueueEmbed(currentTrack, queue, state, guildName, options = {}) {
    const { showNowPlaying = true, showUpcoming = true, showStats = true, showControls = true, } = options;
...
export function createMiniQueueEmbed(currentTrack, queue, guildName) {
...
export function createQueueActionComponents(guildId, state) {
    return [
...
export function createQueueManagementComponents(prefix) {
    return [
...
export async function sendComprehensiveQueue(source, currentTrack, queue, state, guildName, guildId, options = {}) {
...
// ─── Send Mini Queue ─────────────────────────────────────────────────────────────
export async function sendMiniQueue(source, currentTrack, queue, guildName) {
...
// ─── Queue Added Embed ───────────────────────────────────────────────────────────
export function createQueueAddedEmbed(track, position, queueSize) {
    return EmbedManager.success('Added to Queue', `**[${track.title}](${track.url})** has been added to position **${position}**.\n\n` +
        `Artist: ${track.artist}\n` +
        `Duration: ${MusicUI.formatDuration(track.duration)}\n` +
        `Queue size: ${queueSize} songs`);
}
// ─── Queue Removed Embed ─────────────────────────────────────────────────────────
export function createQueueRemovedEmbed(track, position, queueSize) {
    return EmbedManager.success('Removed from Queue', `**[${track.title}](${track.url})** has been removed from position **${position}**.\n\n` +
        `Artist: ${track.artist}\n` +
        `Queue size: ${queueSize} songs`);
}
// ─── Queue Cleared Embed ─────────────────────────────────────────────────────────
export function createQueueClearedEmbed(count) {
    return EmbedManager.success('Queue Cleared', `${count} songs have been removed from the queue.\n\n**Next action:**\n• Use /play to add new songs`);
}
// ─── Queue Shuffled Embed ─────────────────────────────────────────────────────
export function createQueueShuffledEmbed() {
    return EmbedManager.success('Queue Shuffled', 'The queue has been shuffled randomly.\n\n**Next action:**\n• Use /queue to view the new order');
}
// ─── Queue Saved Embed ─────────────────────────────────────────────────────────
export function createQueueSavedEmbed(name, count) {
    return EmbedManager.success('Queue Saved', `Playlist **${name}** has been saved with ${count} songs.\n\n**Next action:**\n• Use /playlist load to load this playlist`);
}
// ─── Export Queue UI ────────────────────────────────────────────────────────────
...
//# sourceMappingURL=QueueUI.js.map