...
export function createMusicPlayerEmbed(track, state, queueInfo, guildName) {
...
function calculateProgress(position, duration) {
    if (duration === 0)
        return 0;
...
// ─── Format Time Display ───────────────────────────────────────────────────────────
function formatTimeDisplay(position, duration) {
    return {
...
function formatControls(state) {
    const controls = [];
    if (state.paused)
        controls.push('⏸️ Paused');
    else
        controls.push('▶️ Playing');
    if (state.loop === 'track')
        controls.push('🔁 Loop Track');
    else if (state.loop === 'queue')
        controls.push('🔁 Loop Queue');
    if (state.shuffle)
        controls.push('🔀 Shuffle');
    if (state.autoplay)
        controls.push('🤖 Autoplay');
    if (state.nightcore)
        controls.push('🎛️ Nightcore');
    if (state.bassboost)
        controls.push('🎛️ Bassboost');
    if (state.vaporwave)
        controls.push('🎛️ Vaporwave');
    return controls.join(' • ') || 'No active effects';
}
// ─── Music Player Components ───────────────────────────────────────────────────────
export function createMusicPlayerComponents(guildId, state) {
    return [
...
export function createMusicFilterComponents(guildId) {
    return ButtonManager.musicFilter(guildId);
}
// ─── Queue Embed ─────────────────────────────────────────────────────────────────
export function createQueueEmbed(currentTrack, queue, state, guildName, page = 0, pageSize = 10) {
...
export async function sendMusicPlayer(source, track, state, queueInfo, guildName, guildId) {
...
export async function updateMusicPlayer(message, track, state, queueInfo, guildName, guildId) {
    const embed = createMusicPlayerEmbed(track, state, queueInfo, guildName);
    const components = createMusicPlayerComponents(guildId, state);
    await message.edit({ embeds: [embed], components });
}
// ─── Send Queue Response ───────────────────────────────────────────────────────────
export async function sendQueue(source, currentTrack, queue, state, guildName, guildId, page = 0) {
...
export function createSearchResultsEmbed(query, results, guildName) {
...
// ─── No Results Embed ─────────────────────────────────────────────────────────────
export function createNoResultsEmbed(query) {
...
export function createNotInVoiceEmbed() {
    return EmbedManager.error('Not in Voice Channel', 'You need to be in a voice channel to use music commands.\n\n**How to fix:**\n1. Join a voice channel\n2. Run the command again');
}
// ─── No Track Playing Embed ───────────────────────────────────────────────────────
export function createNoTrackPlayingEmbed() {
    return EmbedManager.error('No Track Playing', 'There is no track currently playing.\n\n**How to fix:**\n• Use `/play` to add a song to the queue\n• Use `/search` to find a song');
}
// ─── Queue Empty Embed ────────────────────────────────────────────────────────────
export function createQueueEmptyEmbed() {
    return EmbedManager.info('Queue is Empty', 'There are no songs in the queue.\n\n**Next action:**\n• Use `/play` to add a song\n• Use `/search` to find music');
}
// ─── Added to Queue Embed ────────────────────────────────────────────────────────
export function createAddedToQueueEmbed(track, position) {
    return EmbedManager.success('Added to Queue', `**[${track.title}](${track.url})** has been added to the queue at position **${position}**.\n\nArtist: ${track.artist}\nDuration: ${formatDuration(track.duration)}`);
}
// ─── Track Started Embed ─────────────────────────────────────────────────────────
export function createTrackStartedEmbed(track) {
    return EmbedManager.music('Now Playing', `**[${track.title}](${track.url})**\n\nArtist: ${track.artist}\nDuration: ${formatDuration(track.duration)}\nRequested by: <@${track.requester}>`).setThumbnail(track.thumbnail || null);
}
// ─── Track Ended Embed ───────────────────────────────────────────────────────────
export function createTrackEndedEmbed(track) {
    return EmbedManager.music('Track Ended', `**[${track.title}](${track.url})** has finished playing.`);
}
// ─── Export Music UI ────────────────────────────────────────────────────────────
