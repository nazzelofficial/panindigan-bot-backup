...
export class PanindiganClient extends Client {
    ...
    async initializeMusic() {
        ...
        if (!nodes) {
            loggers.music.warn('No Lavalink nodes configured — music commands will be unavailable. ' +
        ...
        this.kazagumo.on('playerEnd', (player) => {
            ...
            if (!player.queue.current && player.queue.size === 0) {
                const timeout = parseInt(process.env.MUSIC_INACTIVITY_TIMEOUT_MS ?? String(this.config.music.inactivityTimeoutMs ?? 300_000), 10);
                setTimeout(() => {
                    if (!player.queue.current && player.queue.size === 0) {
                        player.destroy();
...
//# sourceMappingURL=PanindiganClient.js.map