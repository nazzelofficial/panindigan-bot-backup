...
export async function initializePrisma(): Promise<void> {
  ...
  if (!postgresUrl) {
    throw new Error(
      `PostgreSQL URL not found in environment variable: ${config.databases.postgresql.urlEnv}`,
    );
  }
...
export function getPrismaClient(): PrismaClient {
  if (!prisma) {
    throw new Error(
      'PrismaClient is not initialized. Call initializePrisma() during startup before using the database.',
    );
  }
  return prisma;
}
