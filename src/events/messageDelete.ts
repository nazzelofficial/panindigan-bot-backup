...
export const event: Event = {
  name: 'messageDelete',
  once: false,
  async execute(message: Message | PartialMessage, client: PanindiganClient) {
    ...
    try {
      ...
      await redis.set(
        `${config.databases.redis.keyPrefix}snipe:${message.guild.id}:${message.channel.id}`,
        snipeData,
        { EX: 60 }
      );
    ...
    try {
      ...
      const logChannel = message.guild.channels.cache.get(guild.logChannelId) as TextChannel;
