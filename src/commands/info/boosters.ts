...
export class BoostersCommand extends BaseCommand {
  ...
  private async send(guild: any, reply: (e: EmbedBuilder) => Promise<any>) {
    await guild.members.fetch();
    const boosters = guild.members.cache.filter((m: any) => m.premiumSince);
    ...
    const sorted = [...boosters.values()].sort((a: any, b: any) =>
      new Date(a.premiumSince).getTime() - new Date(b.premiumSince).getTime()
    );

    const list = sorted.map((m: any, i: number) => {
