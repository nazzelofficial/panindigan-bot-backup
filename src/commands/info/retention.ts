...
export class RetentionCommand extends BaseCommand {
  ...
  private async buildEmbed(guild: any): Promise<EmbedBuilder> {
    ...
    const DAY = 86400000;
    const ranges = [
      { label: '< 1 day', filter: (m: any) => now - m.joinedAt.getTime() < DAY },
      { label: '1–7 days', filter: (m: any) => { const d = now - m.joinedAt.getTime(); return d >= DAY && d < 7 * DAY; } },
      { label: '1–4 weeks', filter: (m: any) => { const d = now - m.joinedAt.getTime(); return d >= 7 * DAY && d < 30 * DAY; } },
      { label: '1–6 months', filter: (m: any) => { const d = now - m.joinedAt.getTime(); return d >= 30 * DAY && d < 180 * DAY; } },
      { label: '6–12 months', filter: (m: any) => { const d = now - m.joinedAt.getTime(); return d >= 180 * DAY && d < 365 * DAY; } },
      { label: '> 1 year', filter: (m: any) => now - m.joinedAt.getTime() >= 365 * DAY },
    ];
