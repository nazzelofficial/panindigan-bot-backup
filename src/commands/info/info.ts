...
export class InfoCommand extends BaseCommand {
  ...
  public async executeSlash(i: ChatInputCommandInteraction): Promise<void> {
    ...
    switch (subcommand) {
      case 'user': await this.handleUser(i); break;
      case 'avatar': await this.handleAvatar(i); break;
      case 'banner': await this.handleBanner(i); break;
      case 'server': await this.handleServer(i); break;
      case 'roles': await this.handleRoles(i); break;
      case 'channels': await this.handleChannels(i); break;
      case 'members': await this.handleMembers(i); break;
      case 'bans': await this.handleBans(i); break;
      case 'emojis': await this.handleEmojis(i); break;
      case 'roleinfo': await this.handleRoleInfo(i); break;
      case 'channelinfo': await this.handleChannelInfo(i); break;
      case 'bot': await this.handleBot(i); break;
      case 'ping': await this.handlePing(i); break;
      case 'uptime': await this.handleUptime(i); break;
      case 'stats': await this.handleStats(i); break;
      case 'id': await this.handleId(i); break;
      case 'snowflake': await this.handleSnowflake(i); break;
      case 'permissions': await this.handlePermissions(i); break;
  ...
  private async handleUser(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
  ...
  private async handleRoles(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
    if (!i.guild) return;
  ...
  private async handleChannels(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
    if (!i.guild) return;
  ...
  private async handleMembers(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
    if (!i.guild) return;
  ...
  private async handleBans(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
    if (!i.guild) return;
  ...
  private async handleEmojis(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
    if (!i.guild) return;
    ...
    const embed = EmbedManager.info('Server Emojis', `Total emojis: ${emojis.size}`)
      .addFields(
        ...
        { name: '🖼️ Static', value: `${staticEmojis}`, inline: true },
      )
      .setDescription(emojiList || 'No emojis');
    await i.editReply({ embeds: [embed] });
  }

  private async handleRoleInfo(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
  ...
  private async handleChannelInfo(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
    ...
    if (channel.isVoiceBased()) {
      fields.push(
        { name: '👥 Connected', value: `${channel.members.size}`, inline: true },
  ...
  private async handleBot(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
    ...
    const embed = EmbedManager.info('Bot Information', `**${client.user?.tag}**`)
      .addFields(
        ...
        { name: '📅 Started', value: `<t:${Math.floor(client.readyTimestamp / 1000)}:R>`, inline: true },
  ...
  private async handlePing(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
  ...
  private async handleUptime(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
  ...
  private async handleId(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
  ...
  private async handleSnowflake(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
  ...
  private async handlePermissions(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
    const user = i.options.getUser('user') || i.user;
  ...
  private async handleEmojiInfo(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
    ...
    const embed = EmbedManager.info('Emoji Information', foundEmoji.name || 'Unknown')
      .addFields(
        ...
        { name: '🎬 Animated', value: foundEmoji.animated ? 'Yes' : 'No', inline: true },
        { name: '🖥️ Server', value: foundEmoji.guild?.name || 'Unknown', inline: true },
        { name: '📅 Created', value: `<t:${Math.floor(foundEmoji.createdTimestamp / 1000)}:R>`, inline: true },
      )
      .setThumbnail(foundEmoji.url);
    await i.editReply({ embeds: [embed] });
  }

  private async handleWeather(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
  ...
  private async handleTime(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
  ...
  private async handleUrban(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
  ...
  private async handleGithub(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
  ...
  private async handleCountry(i: ChatInputCommandInteraction): Promise<void> {
    await i.deferReply();
