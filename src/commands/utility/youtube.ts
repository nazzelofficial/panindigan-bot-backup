...
export class YoutubeCommand extends BaseCommand {
  constructor() {
    super({ name: 'youtube', description: 'Search YouTube for a video', category: 'utility', premiumTier: 'free', cooldown: 5, guildOnly: false, ownerOnly: false, slashCommand: true, prefixCommand: true, aliases: ['yt', 'ytsearch'], examples: ['/youtube lofi hip hop', 'p!youtube coding music'] } as CommandOptions);
  }
  private async run(i: ChatInputCommandInteraction | null, m: Message | null, query: string): Promise<void> {
    ...
    if (!query) return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Provide a search query.'));
    ...
    try {
      ...
      if (!item) return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ No results found.'));
