...
export class AutorespondCommand extends BaseCommand {
  constructor() {
    super({ name: 'autorespond', description: 'Auto-response triggers — add, list, delete', category: 'utility', premiumTier: 'diamond', cooldown: 3, guildOnly: true, ownerOnly: false, slashCommand: true, prefixCommand: true, aliases: ['ar', 'autoresponse'], examples: ['/autorespond add hello Hi there!', 'p!autorespond list'] } as CommandOptions);
  }
  private async run(i: ChatInputCommandInteraction | null, m: Message | null, sub: string, trigger: string, response: string): Promise<void> {
    ...
    if (sub === 'add' || sub === 'create') {
      if (!trigger || !response) return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Provide both trigger and response.'));
      const count = await col.countDocuments({ guildId });
      if (count >= 50) return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Maximum 50 auto-responses per server.'));
      ...
      if (!trigger) return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Provide a trigger to remove.'));
