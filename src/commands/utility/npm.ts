...
export class NpmCommand extends BaseCommand {
  constructor() {
    super({ name: 'npm', description: 'Look up an NPM package', category: 'utility', premiumTier: 'free', cooldown: 5, guildOnly: false, ownerOnly: false, slashCommand: true, prefixCommand: true, aliases: ['npmpkg', 'package'], examples: ['/npm discord.js', 'p!npm express'] } as CommandOptions);
  }
  private async run(i: ChatInputCommandInteraction | null, m: Message | null, pkg: string): Promise<void> {
    ...
    try {
      ...
      if (!resp.ok) return send(new EmbedBuilder().setColor(COLORS.error).setDescription(`❌ Package \`${pkg}\` not found on npm.`));
      ...
      const embed = new EmbedBuilder().setColor(0xCC3534).setTitle(`📦 npm: ${data.name}`)
        ...
        .addFields(
          ...
          { name: '📅 Modified', value: data.time?.modified ? new Date(data.time.modified).toDateString() : 'N/A', inline: true },
          { name: '📜 License', value: ver?.license ?? data.license ?? 'N/A', inline: true },
          { name: '🔗 Repo', value: ver?.repository?.url?.replace('git+', '').replace('.git', '') ?? 'N/A', inline: false },
        );
      await send(embed);
    } catch (err: any) {
