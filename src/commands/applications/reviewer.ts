...
export class ApplicationReviewerCommand extends BaseCommand {
  constructor() {
    super({ name: 'application-reviewer', description: 'Set the reviewer role for an application', category: 'applications', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['app-reviewer', 'appreviewer'], examples: ['/application-reviewer <id> @Role'] } as CommandOptions);
  }
  ...
  public async executeSlash(i: ChatInputCommandInteraction): Promise<void> {
    ...
    await prisma.application.update({ where: { id }, data: { reviewerRoleId: role.id } as any });
