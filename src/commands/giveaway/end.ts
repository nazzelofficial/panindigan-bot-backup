...
export class GiveawayEndCommand extends BaseCommand {
  constructor() {
    super({ name: 'gend', description: 'End a giveaway early and pick winners', category: 'giveaway', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['giveaway-end', 'gw-end'], examples: ['/gend <id>', 'p!gend <id>'] } as CommandOptions);
  }
  ...
  private async endGiveaway(guildId: string, giveawayId: string, channelFetch: (id: string) => any): Promise<string> {
    const prisma = getPrismaClient();
    const giveaway = await prisma.giveaway.findFirst({ where: { id: giveawayId, guildId, active: true } });
    if (!giveaway) return '❌ Active giveaway not found with that ID.';
    ...
    if (channel?.isTextBased()) {
      ...
      await channel.send({ content: `🎉 Congratulations ${winnerIds.map((id: string) => `<@${id}>`).join(', ')}! You won **${giveaway.prize}**!`, embeds: [embed] });
    }
