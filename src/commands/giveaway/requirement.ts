...
export class GiveawayRequirementCommand extends BaseCommand {
  constructor() {
    super({ name: 'grequire', description: 'Set role or level requirements for a giveaway', category: 'giveaway', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['grequirement', 'gw-require'], examples: ['/grequire role <id> @role', 'p!grequire level <id> 10'] } as CommandOptions);
  }
  ...
  public async executeSlash(i: ChatInputCommandInteraction): Promise<void> {
    ...
    if (sub === 'role') {
      ...
      await prisma.giveaway.update({ where: { id }, data: { requiredLevel: level } as any });
      await i.reply({ content: `✅ Giveaway now requires level **${level}** to enter.`, ephemeral: true });
    } else {
      await prisma.giveaway.update({ where: { id }, data: { requiredRoleId: null, requiredLevel: null } as any });
