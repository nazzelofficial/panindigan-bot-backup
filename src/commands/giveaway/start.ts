...
export class GiveawayStartCommand extends BaseCommand {
  constructor() {
    super({ name: 'giveaway-start', description: 'Start an existing giveaway immediately 🎁', category: 'giveaway', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageGuild], aliases: ['gstart', 'startgiveaway'], examples: ['/giveaway-start <id>', 'p!giveaway start <id>'] } as CommandOptions);
  }
  ...
  private async handle(giveawayId: string, guildId: string, send: (c: any) => Promise<any>, client: any): Promise<void> {
    ...
    if (!giveaway) { await send({ content: '❌ Giveaway not found or already ended.', ephemeral: true }); return; }
    if (giveaway.status === 'active') { await send({ content: '❌ This giveaway is already active!', ephemeral: true }); return; }
    if (giveaway.status === 'ended') { await send({ content: '❌ This giveaway has already ended.', ephemeral: true }); return; }

    // Update status to active and set startedAt
    await (prisma as any).giveaway?.update?.({
      where: { id: giveawayId },
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '🏆 Prize', value: giveaway.prize || 'Not specified', inline: true },
