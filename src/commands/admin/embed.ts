...
import { ChatInputCommandInteraction, Message, EmbedBuilder, SlashCommandBuilder, PermissionFlagsBits, ColorResolvable } from 'discord.js';
import { COLORS } from '../../utils/Constants';

export class EmbedCommand extends BaseCommand {
  constructor() {
    super({ name: 'embed', description: 'Create a simple embed message', category: 'admin', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, userPermissions: [PermissionFlagsBits.ManageMessages], aliases: ['createembed', 'sendembed'], examples: ['/embed title:"Hello" description:"World"', 'p!embed Hello World'] } as CommandOptions);
  }
  ...
  private parseColor(colorStr: string | null): number {
    if (!colorStr) return COLORS.default;
  ...
  public async executeSlash(i: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      if (ch?.isTextBased()) {
        ...
        if (ch.id !== i.channelId) await i.reply({ content: `✅ Embed sent to ${ch}!`, ephemeral: true });
        else await i.reply({ content: '✅ Embed sent!', ephemeral: true });
      } else {
