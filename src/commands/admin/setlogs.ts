...
export class SetLogsCommand extends BaseCommand {
  ...
  private async handle(guildId: string, logType: string, channelId: string | null, send: (c: any) => Promise<any>): Promise<void> {
    ...
    const currentSettings = (existing as any)?.logChannels ? JSON.parse((existing as any).logChannels) : {};

    if (logType === 'all') {
      for (const t of LOG_TYPES.filter(t => t.value !== 'all')) currentSettings[t.value] = channelId;
    } else {
    ...
    await prisma.guild.upsert({
      where: { guildId },
      create: { guildId, logChannels: JSON.stringify(currentSettings) } as any,
      update: { logChannels: JSON.stringify(currentSettings) } as any,
    });
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: 'Event Type', value: typeName, inline: true },
        { name: 'Channel', value: channelId ? `<#${channelId}>` : '❌ Disabled', inline: true },
      )
      .setTimestamp();

    if (logType === 'all') embed.setFooter({ text: 'All event log channels updated' });
