...
export class PlayMoodCommand extends BaseCommand {
  constructor() {
    const options: CommandOptions = {
      name: 'playmood',
      description: 'Play songs based on mood',
      category: 'music',
      cooldown: 5,
      ...
      prefixCommand: true,
      aliases: ['mood'],
      examples: ['/playmood happy', 'p!playmood sad'],
    };
    super(options);
  }
  ...
  public async executeSlash(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (!mood) {
      await interaction.reply({ content: '❌ Please provide a mood (e.g., happy, sad, energetic, chill).', ephemeral: true });
      return;
    }
    ...
    if (!voiceChannel) {
      await interaction.reply({ content: '❌ You need to be in a voice channel to play mood songs.', ephemeral: true });
      return;
    }
    ...
    try {
      ...
      const tracks = await musicManager.searchMood(mood);

      if (!tracks || tracks.length === 0) {
        await interaction.editReply({ content: '❌ No songs found for that mood.' });
        return;
      }
      ...
      const embed = new EmbedBuilder()
        .setTitle(`${EMOJIS.music} Mood Songs Added to Queue`)
      ...
      await interaction.editReply({ content: '❌ Failed to play mood songs.' });
    }
  }
  ...
  public async executePrefix(message: Message, _args: string[]): Promise<void> {
    ...
    if (!voiceChannel) {
      await message.reply('❌ You need to be in a voice channel to play mood songs.');
      return;
    }

    try {
      await message.reply('🎵 Searching for mood songs...');
      ...
      if (!tracks || tracks.length === 0) {
        await message.edit('❌ No songs found for that mood.');
        return;
      }
      ...
      await message.edit('❌ Failed to play mood songs.');
    }
  }
}

export default PlayMoodCommand;
