...
export class ResumeCommand extends BaseCommand {
  ...
  public async executeSlash(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (!voiceChannel) {
      await interaction.reply({ content: '❌ You need to be in a voice channel to resume music.', ephemeral: true });
      return;
    }
  ...
  public async executePrefix(message: Message): Promise<void> {
    ...
    if (!voiceChannel) {
      await message.reply('❌ You need to be in a voice channel to resume music.');
      return;
    }
    ...
    try {
      ...
      if (!player.paused) {
        await message.reply('❌ The music is already playing.');
        return;
      }
