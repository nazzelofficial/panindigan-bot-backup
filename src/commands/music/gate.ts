...
export class GateCommand extends BaseCommand {
  constructor() {
    const options: CommandOptions = {
      name: 'gate',
      description: 'Apply gate filter (noise gate)',
      category: 'music',
      cooldown: 3,
  ...
  public async executeSlash(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (!voiceChannel) {
      await interaction.reply({ content: '❌ You need to be in a voice channel to apply gate.', ephemeral: true });
      return;
    }
    ...
    try {
      ...
      await musicManager.setFilter(interaction.guild.id, 'gate');

      const embed = new EmbedBuilder()
        .setTitle(`${EMOJIS.music} Gate Applied`)
      ...
      await interaction.reply({ content: '❌ Failed to apply gate.', ephemeral: true });
    }
  }
  ...
  public async executePrefix(message: Message): Promise<void> {
    ...
    if (!voiceChannel) {
      await message.reply('❌ You need to be in a voice channel to apply gate.');
      return;
    }
    ...
    try {
      ...
      await musicManager.setFilter(message.guild.id, 'gate');
      ...
      await message.reply('❌ Failed to apply gate.');
    }
  }
}
