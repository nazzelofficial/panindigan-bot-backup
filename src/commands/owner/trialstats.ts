...
export class TrialstatsCommand extends BaseCommand {
  constructor() {
    super({ name: 'trialstats', description: 'Show trial activations and conversion stats', category: 'owner', premiumTier: 'free', cooldown: 0, guildOnly: false, ownerOnly: true, slashCommand: true, prefixCommand: true, aliases: ['trials'], examples: ['p!trialstats'] } as CommandOptions);
  }
  private async run(i: ChatInputCommandInteraction | null, m: Message | null): Promise<void> {
    ...
    try {
      ...
      const embed = new EmbedBuilder().setColor(COLORS.default).setTitle('📊 Trial Statistics')
        .addFields(
          { name: '🎯 Total Trials', value: total.toString(), inline: true },
          { name: '✅ Converted', value: `${converted} (${rate}%)`, inline: true },
          { name: '📅 Last 7 Days', value: last7.toString(), inline: true },
        );
      await send(embed);
    } catch (err: any) {
