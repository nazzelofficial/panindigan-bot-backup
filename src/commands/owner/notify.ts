...
export class NotifyCommand extends BaseCommand {
  ...
  public buildSlashCommand(): SlashCommandBuilder {
    return new SlashCommandBuilder()
      ...
      .addStringOption(o => o.setName('message').setDescription('Notification message').setRequired(true)) as SlashCommandBuilder;
  }

  private buildPlanEmbed(type: string, notifMessage: string, guildCount: number): EmbedBuilder {
    ...
    return new EmbedBuilder()
      ...
      .addFields(
        { name: 'Target Group', value: typeLabels[type] ?? type, inline: true },
        { name: 'Guild Count', value: String(guildCount), inline: true },
        { name: 'Target Description', value: targetDesc[type] ?? 'Unknown group', inline: false },
        { name: 'Message Preview', value: notifMessage.slice(0, 1024), inline: false },
      )
