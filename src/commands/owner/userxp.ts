...
export class UserXpCommand extends BaseCommand {
  ...
  public buildSlashCommand(): SlashCommandBuilder {
    return new SlashCommandBuilder()
      ...
      .addIntegerOption(o => o.setName('amount').setDescription('XP amount to set').setRequired(true)) as SlashCommandBuilder;
  }
  ...
  public async executeSlash(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      const updated = await (prisma as any).userLevel.upsert({
        where: { userId },
        update: { xp: amount },
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'Level', value: String(updated.level ?? 0), inline: true },
        )
        .setTimestamp();
