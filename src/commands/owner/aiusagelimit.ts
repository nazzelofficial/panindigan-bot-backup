...
export class AiusagelimitCommand extends BaseCommand {
  constructor() {
    super({ name: 'aiusagelimit', description: 'Set AI usage limit for a guild', category: 'owner', premiumTier: 'free', cooldown: 0, guildOnly: false, ownerOnly: true, slashCommand: true, prefixCommand: true, aliases: ['ailimit'], examples: ['p!aiusagelimit 123456789 100'] } as CommandOptions);
  }
  private async run(i: ChatInputCommandInteraction | null, m: Message | null, guildId: string, limit: number): Promise<void> {
    const send = async (e: EmbedBuilder) => { if (i) await i.reply({ embeds: [e], flags: 64 }); else await m!.reply({ embeds: [e] }); };
    if (!guildId || !limit) return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Usage: `aiusagelimit <guild_id> <daily_limit>`'));
