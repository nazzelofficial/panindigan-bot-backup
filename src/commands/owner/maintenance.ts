...
export class MaintenanceCommand extends BaseCommand {
  ...
  public buildSlashCommand(): SlashCommandBuilder {
    return new SlashCommandBuilder()
      ...
      .addStringOption(o => o.setName('reason').setDescription('Reason for maintenance').setRequired(false)) as SlashCommandBuilder;
  }

  private async toggleMaintenance(state: string, reason?: string): Promise<void> {
  ...
  private async getMaintenanceStatus(): Promise<{ enabled: boolean; reason: string | null }> {
    const redis = await getRedisClient();
