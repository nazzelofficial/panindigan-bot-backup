...
export class ConfiggetCommand extends BaseCommand {
  constructor() {
    super({ name: 'configget', description: 'Get a value from config.json', category: 'owner', premiumTier: 'free', cooldown: 0, guildOnly: false, ownerOnly: true, slashCommand: true, prefixCommand: true, aliases: ['cfget'], examples: ['p!configget PREFIX'] } as CommandOptions);
  }
  private async run(i: ChatInputCommandInteraction | null, m: Message | null, key: string): Promise<void> {
    ...
    if (!key) return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ Provide a config key.'));
    try {
      const configPath = 'config.json';
      if (!fs.existsSync(configPath)) return send(new EmbedBuilder().setColor(COLORS.error).setDescription('❌ config.json not found.'));
      const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
      const value = config[key];
      if (value === undefined) return send(new EmbedBuilder().setColor(COLORS.error).setDescription(`❌ Key \`${key}\` not found in config.`));
