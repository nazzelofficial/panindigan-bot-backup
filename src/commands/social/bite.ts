...
export class BiteCommand extends BaseCommand {
  ...
  public async executePrefix(m: Message, args: string[]): Promise<void> { const t = m.mentions.users.first(); if (!t) { await m.reply('❌ Mention someone!'); return; } await m.reply({ embeds: [new EmbedBuilder().setDescription(`🦷 **${m.author.username}** bites **${t.username}**!`).setImage(this.gif()).setColor(COLORS.default)] }); }
}
