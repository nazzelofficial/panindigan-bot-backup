...
export class SmugCommand extends BaseCommand {
  ...
  public async executePrefix(m: Message, _args: string[]): Promise<void> { await m.reply({ embeds: [new EmbedBuilder().setDescription(`😏 **${m.author.username}** looks smug!`).setImage(GIFS[0]).setColor(COLORS.default)] }); }
}
