...
export class KillCommand extends BaseCommand {
  ...
  public buildSlashCommand(): SlashCommandBuilder {
    return (new SlashCommandBuilder()
      ...
      .addUserOption(o => o.setName('user').setDescription('Who to dramatically "kill"').setRequired(true))) as SlashCommandBuilder;
  }

  private gif() { return GIFS[Math.floor(Math.random() * GIFS.length)]; }
  private scenario() { return SCENARIOS[Math.floor(Math.random() * SCENARIOS.length)]; }
  ...
  public async executePrefix(m: Message, args: string[]): Promise<void> {
    try {
      ...
      if (!t) { await m.reply('❌ Mention someone to dramatically take down!'); return; }
      if (t.id === m.author.id) { await m.reply('💀 You tried to kill yourself dramatically... but tripped over your shoelaces instead.'); return; }
      const embed = new EmbedBuilder()
