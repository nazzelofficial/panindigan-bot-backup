...
export class CouplebgCommand extends BaseCommand {
  constructor() {
    super({ name: 'couplebg', description: 'Set a custom background for your premium couple profile card 🎨', category: 'social', premiumTier: 'gold', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['couplebackground', 'setcouplebg'], examples: ['/couplebg sunset', 'p!couplebg stars'] } as CommandOptions);
  }
  ...
  private async handle(userId: string, guildId: string, theme: string, send: (c: any) => Promise<any>): Promise<void> {
    ...
    if (!bg) { await send({ content: '❌ Invalid background theme.', ephemeral: true }); return; }

    const prisma = getPrismaClient();
    await (prisma as any).couple?.update?.({ where: { id: profile.id }, data: { backgroundUrl: bg.url } }).catch(() => null);
