...
export class ShootCommand extends BaseCommand {
  constructor() {
    super({ name: 'shoot', description: 'Shoot someone (for fun)! 🔫', category: 'social', premiumTier: 'gold', cooldown: 3, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['bang', 'pew'], examples: ['/shoot @user', 'p!shoot @user'] } as CommandOptions);
  }

  public buildSlashCommand(): SlashCommandBuilder {
    return (new SlashCommandBuilder().setName(this.name).setDescription(this.description)
      .addUserOption(o => o.setName('user').setDescription('Who to shoot (playfully!)').setRequired(true))) as SlashCommandBuilder;
  }
  ...
  public async executePrefix(m: Message, _args: string[]): Promise<void> {
    ...
    if (!t) { await m.reply('❌ Mention someone to shoot!'); return; }
    const embed = new EmbedBuilder()
