...
export class CryCommand extends BaseCommand {
  ...
  public async executePrefix(m: Message, _args: string[]): Promise<void> { await m.reply({ embeds: [new EmbedBuilder().setDescription(`😢 **${m.author.username}** is crying...`).setImage(this.gif()).setColor(COLORS.info)] }); }
}
