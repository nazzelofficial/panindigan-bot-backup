...
export class WaveCommand extends BaseCommand {
  ...
  public async executePrefix(m: Message, args: string[]): Promise<void> { const t = m.mentions.users.first(); if (!t) { await m.reply('❌ Mention someone!'); return; } await m.reply({ embeds: [new EmbedBuilder().setDescription(`👋 **${m.author.username}** waves at **${t.username}**!`).setImage(GIFS[0]).setColor(COLORS.default)] }); }
}
