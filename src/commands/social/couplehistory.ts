...
export class CoupleHistoryCommand extends BaseCommand {
  constructor() {
    super({ name: 'couplehistory', description: 'View complete couple milestone history 📖', category: 'social', premiumTier: 'gold', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['relationshiphistory', 'milestones'], examples: ['/couplehistory', 'p!couplehistory'] } as CommandOptions);
  }
  ...
  private async handle(userId: string, guildId: string, send: (c: any) => Promise<any>): Promise<void> {
    ...
    const history = await coupleHistoryService.getHistory(userId, partnerId, guildId).catch(() => []) as any[];
    ...
    const allEvents = [
      ...
      ...history.map((h: any) => ({ label: h.event === 'divorced' ? '💔 Separated' : `📝 ${h.event}`, ts: new Date(h.createdAt), type: 'event' })),
