...
export class BoopCommand extends BaseCommand {
  ...
  public async executePrefix(m: Message, args: string[]): Promise<void> { const t = m.mentions.users.first(); if (!t) { await m.reply('❌ Mention someone!'); return; } await m.reply({ embeds: [new EmbedBuilder().setDescription(`👆 **${m.author.username}** boops **${t.username}** on the nose!`).setImage(GIFS[0]).setColor(COLORS.default)] }); }
}
