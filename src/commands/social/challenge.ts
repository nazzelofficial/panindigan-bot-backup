...
export class ChallengeCommand extends BaseCommand {
  constructor() {
    super({ name: 'challenge', description: 'Send a friendly challenge to another member 🎯', category: 'social', premiumTier: 'gold', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['dare2', 'friendchallenge'], examples: ['/challenge @user', 'p!challenge @user'] } as CommandOptions);
  }
  ...
  private async handle(senderId: string, targetId: string, senderName: string, send: (content: any) => Promise<any>): Promise<void> {
    if (senderId === targetId) { await send({ content: '❌ You cannot challenge yourself!', ephemeral: true }); return; }

    const challenge = CHALLENGES[Math.floor(Math.random() * CHALLENGES.length)];

    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId(`ch_accept:${targetId}`).setLabel('✅ Accept').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId(`ch_decline:${targetId}`).setLabel('❌ Decline').setStyle(ButtonStyle.Danger),
    );
    ...
    const collector = msg.createMessageComponentCollector({ componentType: ComponentType.Button, time: 120000, filter: (i: any) => i.user.id === targetId });
  ...
  public async executePrefix(m: Message, _args: string[]): Promise<void> {
    ...
    if (!t) { await m.reply('❌ Mention someone to challenge!'); return; }
