...
const BADGES: Record<string, { emoji: string; name: string; desc: string; price: number | null }> = {
...
export class BadgeCommand extends BaseCommand {
  constructor() {
    super({ name: 'badge', description: 'View all your earned and available badges (Diamond) 🏅', category: 'social', premiumTier: 'diamond', cooldown: 5, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['badges', 'mybadges'], examples: ['/badge', '/badge @user', 'p!badge'] } as CommandOptions);
  }

  public buildSlashCommand(): SlashCommandBuilder {
    return (new SlashCommandBuilder().setName(this.name).setDescription(this.description)
      .addUserOption(o => o.setName('user').setDescription('View another user\'s badges').setRequired(false))) as SlashCommandBuilder;
  }
  ...
  private async handle(userId: string, send: (c: any) => Promise<any>, client: any): Promise<void> {
    ...
    const earned: string[] = (userData as any)?.badges || [];
