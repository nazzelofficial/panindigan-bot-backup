...
export class HighFiveCommand extends BaseCommand {
  ...
  public async executePrefix(m: Message, _args: string[]): Promise<void> { const t = m.mentions.users.first(); if (!t) { await m.reply('❌ Mention someone!'); return; } await m.reply({ embeds: [new EmbedBuilder().setDescription(`✋ **${m.author.username}** high fives **${t.username}**!`).setImage(this.gif()).setColor(COLORS.success)] }); }
}
