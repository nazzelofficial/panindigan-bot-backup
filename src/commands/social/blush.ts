...
export class BlushCommand extends BaseCommand {
  ...
  public async executePrefix(m: Message, args: string[]): Promise<void> { await m.reply({ embeds: [new EmbedBuilder().setDescription(`😊 **${m.author.username}** is blushing!`).setImage(GIFS[0]).setColor(COLORS.default)] }); }
}
