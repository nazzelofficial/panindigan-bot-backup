...
export class AkinatorCommand extends BaseCommand {
  ...
  private getRow(gameOver: boolean): ActionRowBuilder<ButtonBuilder> {
    return new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId('aki_yes').setLabel('✅ Yes').setStyle(ButtonStyle.Success).setDisabled(gameOver),
      new ButtonBuilder().setCustomId('aki_no').setLabel('❌ No').setStyle(ButtonStyle.Danger).setDisabled(gameOver),
      new ButtonBuilder().setCustomId('aki_maybe').setLabel('🤔 Maybe').setStyle(ButtonStyle.Secondary).setDisabled(gameOver),
      new ButtonBuilder().setCustomId('aki_reveal').setLabel('🎭 Reveal Answer').setStyle(ButtonStyle.Primary).setDisabled(gameOver),
    );
  }

  private buildEmbed(state: GameState): EmbedBuilder {
  ...
  private async runGame(channelId: string, userId: string, replyFn: (embed: EmbedBuilder, row: ActionRowBuilder<ButtonBuilder>) => Promise<any>): Promise<void> {
    if (activeGames.has(channelId)) {
      return;
    }
    const secret = SECRET_ITEMS[Math.floor(Math.random() * SECRET_ITEMS.length)];
    const state: GameState = { secret, questionIndex: 0, answers: [], revealed: false };
    ...
    collector.on('collect', async (btn: ButtonInteraction) => {
      ...
      const answerMap: Record<string, string> = { aki_yes: 'Yes', aki_no: 'No', aki_maybe: 'Maybe' };
