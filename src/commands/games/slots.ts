...
function evaluate(reels: string[]): { result: string; multiplier: number } {
  const [a, b, c] = reels;
  if (a === b && b === c) return { result: `JACKPOT! Three ${a}`, multiplier: PAYOUTS[a] * 3 };
  if (a === b || b === c || a === c) {
    const sym = a === b ? a : c;
  ...
  private async runSlots(
    ...
    const resultEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '🏆 Status', value: won ? `Won! (${multiplier}x)` : 'Lost', inline: true },
      )
