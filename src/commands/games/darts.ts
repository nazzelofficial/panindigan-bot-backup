...
function throwDart(): { zone: (typeof ZONES)[number]; segment: number; score: number; desc: string } {
  ...
  if ((zone as any).triple && !zone.miss) { score = segment * 3; desc = `Triple ${segment} 🟡`; }
  else if ((zone as any).double && !zone.miss) { score = segment * 2; desc = `Double ${segment} 🟢`; }
  else if (!zone.miss && !zone.points) { desc = `${segment} points`; }
...
export class DartsCommand extends BaseCommand {
  ...
  private async runDarts(username: string): Promise<EmbedBuilder> {
    ...
    return new EmbedBuilder()
      ...
      .setColor(rating.color as number)
      .addFields(
        ...throws.map((t, i) => ({ name: `🎯 Dart ${i + 1}`, value: `${t.desc} → **${t.score} pts**`, inline: true })),
        { name: '📊 Total Score', value: `**${total} / 180 pts**`, inline: false },
