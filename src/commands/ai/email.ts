...
Keep the email between 150–400 words. Write the full email, ready to send.`;
  ...
  public async executeSlash(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          { name: '📌 Topic', value: topic.slice(0, 256), inline: true },
          { name: '🎭 Tone', value: tone.charAt(0).toUpperCase() + tone.slice(1), inline: true },
          ...(recipient ? [{ name: '👤 Recipient', value: recipient.slice(0, 100), inline: true }] : []),
          { name: '✉️ Generated Email', value: `\`\`\`\n${truncated}\n\`\`\`` },
        )
  ...
  public async executePrefix(message: Message, args: string[]): Promise<void> {
    ...
    if (!topic) {
      return void message.reply(
        `${EMOJIS.error} Please provide a topic. Example: \`p!email professional Request a meeting\``,
      );
    }
