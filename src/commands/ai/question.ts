...
Keep each question concise (under 150 characters ideally).`;
    ...
    if (lines.length > 0) return lines.map(l => l.replace(/^\d+\.\s*/, '').trim());
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '🔢 Count', value: `${Math.min(questions.length, count)}`, inline: true },
          {
  ...
  public async executePrefix(message: Message, _args: string[]): Promise<void> {
    ...
    if (!topic) {
      return void message.reply(
        `${EMOJIS.error} Please provide a topic. Example: \`p!question debate climate change\``,
      );
    }
