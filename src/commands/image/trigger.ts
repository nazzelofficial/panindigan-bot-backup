...
export class TriggerCommand extends BaseCommand {
  constructor() {
    super({ name: 'trigger', description: 'Put a "triggered" effect on someone\'s avatar', category: 'image', premiumTier: 'free', cooldown: 5, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['triggered'], examples: ['/trigger @user', 'p!trigger @user'] } as CommandOptions);
  }

  public buildSlashCommand(): SlashCommandBuilder {
    return (new SlashCommandBuilder().setName(this.name).setDescription(this.description).addUserOption(o => o.setName('user').setDescription('User to trigger').setRequired(false)).setDMPermission(false)) as SlashCommandBuilder;
  }
