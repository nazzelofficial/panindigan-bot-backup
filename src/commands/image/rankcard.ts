...
export class RankcardCommand extends BaseCommand {
  constructor() {
    super({ name: 'rankcard', description: 'Preview your rank card with current stats', category: 'image', premiumTier: 'free', cooldown: 10, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['rankcard-preview', 'myrank'], examples: ['/rankcard @user', 'p!rankcard'] } as CommandOptions);
  }

  public buildSlashCommand(): SlashCommandBuilder {
    return (new SlashCommandBuilder().setName(this.name).setDescription(this.description).addUserOption(o => o.setName('user').setDescription('User').setRequired(false)).setDMPermission(false)) as SlashCommandBuilder;
  }

  public async executeSlash(i: ChatInputCommandInteraction): Promise<void> {
    const target = i.options.getMember('user') as any || i.member as any;
    const userId = target?.user?.id || i.user.id;
    await i.deferReply();
    const prisma = getPrismaClient();
    const userData = await prisma.user.findFirst({ where: { userId, guildId: i.guildId! } });
    if (!userData) { await i.editReply({ content: '❌ No data found for this user.' }); return; }
  ...
  public async executePrefix(m: Message, _args: string[]): Promise<void> {
    ...
    if (!userData) { await m.reply('❌ No data found.'); return; }
    try {
