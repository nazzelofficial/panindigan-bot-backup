...
export class StickerCommand extends BaseCommand {
  constructor() {
    super({ name: 'sticker', description: 'Create a custom Discord sticker from an avatar (Diamond) 🎨', category: 'image', premiumTier: 'diamond', cooldown: 15, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['makesticker', 'createsticker'], examples: ['/sticker create @user', 'p!sticker create @user My Sticker'] } as CommandOptions);
  }
  ...
  private async handle(avatarUrl: string, text: string | null, style: string, send: (c: any) => Promise<any>): Promise<void> {
    const size = 512;
  ...
  public async executeSlash(i: ChatInputCommandInteraction): Promise<void> {
    ...
    if (sub !== 'create') { await i.reply({ content: '❌ Use `/sticker create`', ephemeral: true }); return; }
