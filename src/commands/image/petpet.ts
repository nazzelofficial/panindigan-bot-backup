...
export class PetpetCommand extends BaseCommand {
  constructor() {
    super({ name: 'petpet', description: 'Generate an animated petpet GIF of a user', category: 'image', premiumTier: 'free', cooldown: 8, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['pet', 'headpat-gif'], examples: ['/petpet @user', 'p!petpet @user'] } as CommandOptions);
  }

  public buildSlashCommand(): SlashCommandBuilder {
    return (new SlashCommandBuilder().setName(this.name).setDescription(this.description).addUserOption(o => o.setName('user').setDescription('User to pet').setRequired(false)).setDMPermission(false)) as SlashCommandBuilder;
  }
