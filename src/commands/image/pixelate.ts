...
export class PixelateCommand extends BaseCommand {
  constructor() {
    super({ name: 'pixelate', description: 'Pixelate a user\'s avatar', category: 'image', premiumTier: 'free', cooldown: 5, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['pixel', 'mosaic'], examples: ['/pixelate @user 8', 'p!pixelate @user 16'] } as CommandOptions);
  }
  ...
  private async pixelate(avatarUrl: string, pixelSize: number): Promise<Buffer> {
