...
export class BalanceCommand extends BaseCommand {
  constructor() {
    const options: CommandOptions = {
      ...
      examples: ['/balance', '/balance @user', 'p!balance', 'p!balance @user'],
    };
    super(options);
  }
  ...
  public async executeSlash(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`${EMOJIS.economy} ${targetUser.tag}'s Balance`)
        ...
        .addFields([
          ...
          { name: 'Total', value: `${total.toLocaleString()} ${guild.currencySymbol || '💰'}`, inline: false },
        ])
        .setTimestamp();
