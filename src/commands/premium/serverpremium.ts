...
export class ServerPremiumCommand extends BaseCommand {
  constructor() {
    super({ name: 'serverpremium', description: 'View this server\'s premium status and perks', category: 'premium', premiumTier: 'free', cooldown: 5, guildOnly: true, slashCommand: true, prefixCommand: true, aliases: ['guildpremium', 'serverstatus'], examples: ['/serverpremium'] } as CommandOptions);
  }
  ...
  public async executeSlash(i: ChatInputCommandInteraction): Promise<void> {
    ...
    const tierColors: Record<string, number> = { bronze: COLORS.bronze, silver: COLORS.silver, gold: COLORS.gold, diamond: COLORS.diamond };
    const embed = new EmbedBuilder()
      .setTitle(`✨ Server Status: ${premium.tier?.toUpperCase()}`)
      .setColor(tierColors[premium.tier || 'bronze'] || COLORS.gold)
      .addFields(
        { name: '🏆 Tier', value: premium.tier?.toUpperCase() || 'Unknown', inline: true },
        { name: '👤 Activated by', value: `<@${premium.userId}>`, inline: true },
        { name: '📅 Since', value: `<t:${Math.floor(new Date(premium.activatedAt || Date.now()).getTime() / 1000)}:D>`, inline: true },
        { name: '📊 Total Members', value: `${i.guild?.memberCount || 0}`, inline: true },
      )
      .setTimestamp();
    await i.reply({ embeds: [embed] });
  }
  ...
  public async executePrefix(m: Message): Promise<void> {
    ...
    if (!premium) { await m.reply('🆓 This server is on the **Free** plan.'); return; }
