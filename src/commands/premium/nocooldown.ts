...
export class NoCooldownCommand extends BaseCommand {
  constructor() {
    super({ name: 'nocooldown', description: 'Clear all your active cooldowns (Diamond perk)', category: 'premium', premiumTier: 'diamond', cooldown: 300, guildOnly: false, slashCommand: true, prefixCommand: true, aliases: ['clearcooldowns', 'resetcooldowns'], examples: ['/nocooldown'] } as CommandOptions);
  }
  ...
  public async executeSlash(i: ChatInputCommandInteraction): Promise<void> {
    ...
    if (redis) {
      ...
      if (keys.length) { await redis.del(...keys); cleared = keys.length; }
    }
