...
import {
  ContextMenuCommandInteraction, EmbedBuilder, ApplicationCommandType,
} from 'discord.js';
...
export class MessageInfoContextCommand extends BaseCommand {
  ...
  public async executeContext(i: ContextMenuCommandInteraction): Promise<void> {
    ...
    try {
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '📝 Content', value: message.content.slice(0, 100) + (message.content.length > 100 ? '...' : ''), inline: false },
        )
        .setTimestamp();
