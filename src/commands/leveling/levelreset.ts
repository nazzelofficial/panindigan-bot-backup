...
import { ChatInputCommandInteraction, Message, EmbedBuilder, SlashCommandBuilder, PermissionFlagsBits, ActionRowBuilder, ButtonBuilder, ButtonStyle } from 'discord.js';
...
export class LevelresetCommand extends BaseCommand {
  ...
  public buildSlashCommand(): SlashCommandBuilder {
    return (new SlashCommandBuilder()
      ...
      .addStringOption(o =>
        o.setName('target').setDescription('Type "all" to reset everyone, or mention a user').setRequired(false)
      )
      .addUserOption(o =>
        o.setName('user').setDescription('Specific user to reset XP for').setRequired(false)
      )
  ...
  public async executePrefix(message: Message, args: string[]): Promise<void> {
    ...
    if (target?.toLowerCase() === 'all') {
      const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId('confirm_reset_all_pfx').setLabel('Confirm Reset All').setStyle(ButtonStyle.Danger),
        new ButtonBuilder().setCustomId('cancel_reset_all_pfx').setLabel('Cancel').setStyle(ButtonStyle.Secondary),
      );
      const confirmEmbed = new EmbedBuilder()
        .setTitle(`${EMOJIS.warning} Confirm Mass XP Reset`)
        .setColor(COLORS.error)
