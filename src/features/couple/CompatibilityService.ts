export class CompatibilityService {
  ...
  getCompatibilityMessage(score: number): { emoji: string; label: string; description: string } {
    if (score >= 95) return { emoji: '💎', label: 'Soulmates', description: 'Perfect match! Tadhana talaga kayo.' };
    if (score >= 85) return { emoji: '❤️', label: 'Perfect Match', description: 'Para kayong ginawa para sa isa\'t isa!' };
    if (score >= 75) return { emoji: '💕', label: 'Great Couple', description: 'Maganda ang chemistry ninyo!' };
    if (score >= 60) return { emoji: '💙', label: 'Good Match', description: 'May konektasyon kayo!' };
    if (score >= 45) return { emoji: '💛', label: 'It\'s Possible', description: 'Kailangan lang ng mas maraming effort.' };
    if (score >= 30) return { emoji: '🧡', label: 'Complicated', description: 'Medyo challenging, pero pwede pa rin.' };
    return { emoji: '💔', label: 'Opposites', description: 'Hindi talaga kayo tugma... pero love wins daw!' };
  }

  getCompatibilityBar(score: number, size: number = 20): string {
